...
type BasicClient interface {
	...
	ListAllPods(ctx context.Context, namespace string) (*corev1.PodList, error)
...
func (c *unifiedClient) ListAllPods(ctx context.Context, namespace string) (*corev1.PodList, error) {
	if namespace == "" {
		return nil, fmt.Errorf("namespace is required for ListAllPods operation")
	}
	...
	pods, err := c.clientset.CoreV1().Pods(namespace).List(ctx, metav1.ListOptions{})
	if err != nil {
		return nil, fmt.Errorf("failed to list all pods in namespace %s: %w", namespace, err)
	}

	c.log.WithFields(logrus.Fields{
		"namespace": namespace,
		"count":     len(pods.Items),
	}).Debug("Listed all pods in namespace")
...
// Additional Kubernetes client methods
