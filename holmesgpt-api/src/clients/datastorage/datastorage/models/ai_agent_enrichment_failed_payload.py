...
class AIAgentEnrichmentFailedPayload(BaseModel):
    """
    ...
    reason: StrictStr = Field(description="Failure reason from EnrichmentFailure (e.g., rca_incomplete)")
    detail: StrictStr = Field(description="Detailed failure context including retry information")
    affected_resource_kind: StrictStr = Field(description="Kind of the resource that was being enriched when failure occurred")
    affected_resource_name: StrictStr = Field(description="Name of the resource that was being enriched when failure occurred")
    affected_resource_namespace: Optional[StrictStr] = Field(default=None, description="Namespace of the resource (empty for cluster-scoped)")
    __properties: ClassVar[List[str]] = ["event_type", "event_id", "incident_id", "reason", "detail", "affected_resource_kind", "affected_resource_name", "affected_resource_namespace"]

    @field_validator('event_type')
    def event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('aiagent.enrichment.failed'):
            raise ValueError("must be one of enum values ('aiagent.enrichment.failed')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "affected_resource_kind": obj.get("affected_resource_kind"),
            "affected_resource_name": obj.get("affected_resource_name"),
            "affected_resource_namespace": obj.get("affected_resource_namespace")
        })
        return _obj
