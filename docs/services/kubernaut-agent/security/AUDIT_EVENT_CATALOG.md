...
| `aiagent.response.failed` | `EventTypeResponseFailed` | AU-2, AU-3, CC7.2 | Any non-cancellation LLM turn failure, including upstream timeouts, provider errors, or stream failures | `error_message`, `error_details` (`message`, `code`, `component`, `retry_possible`, optional `stack_trace`), `phase`, `duration_seconds` |
...
**Reconstruction note (Issue #1818; superseded by DD-AA-KA-001 Amendment Gap 3, BR-AA-KA-065.12):** a session created by `handleStart`'s reattach path (`reattachOrCreateFallback`/`createFallbackSession` in `internal/kubernautagent/mcp/tools/investigate_start.go` and `investigate_autonomous.go`, entered when no `StatusRunning` autonomous session exists for the RR) carries a `mode` tag of `interactive_reattached` in the session's `Metadata`, seeded with the real RCA `InvestigationResult` copied from an autonomous investigation that completed and raced ahead of the interactive request. The `mode` tag is not itself a field on the `aiagent.session.started` event's `Data`; reconstructing this case for a given `correlation_id` requires resolving the event's `session_id` against the session store (`Manager.GetSession`) to inspect `Metadata["mode"]` and `Result`. The `mode=interactive_fallback` genuine-placeholder case (no prior investigation, no real RCA anywhere for the RR) no longer creates a session at all as of Gap 3: `reattachOrCreateFallback` returns exhausted, and `handleStart` fails closed with `ErrCodeNoInvestigationAvailable` instead — a placeholder session with nothing backing it is a dead end (AF has exactly two terminal flows, interactive remediation and autonomous fix, both of which require a real investigation behind them), so it is no longer fabricated. Before the original #1818 fix, every fallback session was unconditionally `interactive_fallback` with a hardcoded placeholder `RCASummary`, silently discarding real RCA content the autonomous investigation had already produced — a BR-AUDIT-005/CC8.1 reconstruction gap (querying by `correlation_id` alone would surface a real, completed investigation session alongside a disconnected placeholder session, not one coherent narrative). Covered by UT-KA-1818-001..004 (unit, updated for Gap 3) and IT-KA-1818-001/002 (integration, including an explicit audit-reconstruction-by-`correlation_id` proof) in `internal/kubernautagent/mcp/tools/`.
| `aiagent.session.cancelled` | `EventTypeSessionCancelled` | AU-2, CC8.1 | Emitted by `session.Manager.terminateSession` (operator-initiated, e.g. an interactive session explicitly cancelled) **or** `ForceCancelByRemediationID` (#2170; automated -- an `AgentSession` deletion observed by the Dispatcher's `Reconciler` (`reconcileDelete`/`cancelOnDelete`, #2231/DD-AA-KA-001 Amendment) or a self-enforced `Spec.TimesOutAt` deadline expiry (`cancelOnTimeout`)) | `session_id`, `reason` |
