...
#
# Usage: bash deploy/demo/scenarios/crashloop/recording/record.sh
set -euo pipefail
...
ALERT_CACHE="${RECORDING_DIR}/.alert-cache.json"
...
bash "${RECORDING_DIR}/tape-setup.sh"
echo "    Setup complete."

# ── Step 2: Start pods1 + rr1 (background) ────
echo "==> [2/11] Starting focused tapes: pods1 (-> CrashLoopBackOff) + rr1 (-> Analyzing)..."
vhs "${RECORDING_DIR}/crashloop-pods1.tape" &
PODS1_PID=$!
vhs "${RECORDING_DIR}/crashloop-rr1.tape" &
RR1_PID=$!
sleep 5

# ── Step 3: Inject bad config ─────────────────
echo "==> [3/11] Injecting bad nginx config..."
bash "${SCENARIO_DIR}/inject-bad-config.sh"  # scenario infra — update path when available
...
vhs "${RECORDING_DIR}/crashloop-rr2.tape" &
RR2_PID=$!
...
vhs "${RECORDING_DIR}/crashloop-rr3.tape" &
RR3_PID=$!
vhs "${RECORDING_DIR}/crashloop-pods2.tape" &
PODS2_PID=$!
sleep 5
...
sleep 3
bash "${REPO_ROOT}/deploy/demo/scripts/approve-rar.sh"
echo "    RAR approved."
...
vhs "${RECORDING_DIR}/crashloop-rr4.tape" &
RR4_PID=$!
...
export ALERT_CACHE
vhs "${RECORDING_DIR}/crashloop-screens.tape"
echo "    Screens tape finished."

# ── Cleanup ───────────────────────────────────
echo ""
echo "==> Cleaning up scenario resources..."
bash "${SCENARIO_DIR}/cleanup.sh" 2>/dev/null || true  # scenario infra — update path when available
...
echo ""
echo "  Raw files:"
echo "    ${RECORDING_DIR}/crashloop-screens-raw.mp4"
echo "    ${RECORDING_DIR}/crashloop-rr1-raw.mp4"
echo "    ${RECORDING_DIR}/crashloop-rr2-raw.mp4"
echo "    ${RECORDING_DIR}/crashloop-rr3-raw.mp4"
echo "    ${RECORDING_DIR}/crashloop-rr4-raw.mp4"
echo "    ${RECORDING_DIR}/crashloop-pods1-raw.mp4"
echo "    ${RECORDING_DIR}/crashloop-pods2-raw.mp4"
echo ""
echo "  Next: bash deploy/demo/scripts/splice-demo.sh crashloop recording"
echo "════════════════════════════════════════════"
