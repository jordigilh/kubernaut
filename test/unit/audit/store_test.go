...
func (m *MockDLQClient) EnqueueAuditEvent(ctx context.Context, event *audit.AuditEvent, originalError error) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *MockDLQClient) GetEvents() []*audit.AuditEvent {
	m.mu.Lock()
	defer m.mu.Unlock()
