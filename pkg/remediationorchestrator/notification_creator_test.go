	...
	Describe("CreateManualReviewNotification", func() {
		...
		Context("BR-ORCH-036: Priority Mapping by SubReason", func() {
			...
			DescribeTable("should map SubReason to correct priority",
				...
				// #2019/#2020: A workflow was already discovered and presented -- unlike
				// NoMatchingWorkflows (nothing found), a concrete recommendation is sitting
				// unanswered. AC-6/CM-3: route as High so the pending decision isn't lost in
				// a Medium-priority queue.
				Entry("UT-RO-2020-001: DecisionExpired → High", "DecisionExpired", notificationv1.NotificationPriorityHigh),
			)
		})
