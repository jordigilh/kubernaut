...
Kubernaut v1.0 requires a mechanism to execute operator-defined remediation workflows with parameters provided by the LLM. The current approach involves maintaining a generic "playbook-executor" Pipeline in the Remediation Execution Engine that runs container images.
...
**APPROVED**: Use Tekton Pipeline OCI Bundles for workflow execution in v1.0.
...
- Generic Pipeline must handle all workflow types
