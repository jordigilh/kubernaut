...
func certNotReadyConfig() MockScenarioConfig {
	return MockScenarioConfig{
		...
		WorkflowName: "fix-certificate-v1", WorkflowID: uuid.DeterministicUUID("fix-certificate-v1"), ActionType: "FixCertificate",
		WorkflowTitle: "Fix Certificate - Recreate CA Secret", Confidence: 0.92,
		Rationale:    "Certificate is stuck in NotReady due to missing CA Secret; recreating the secret from the ClusterIssuer spec is the targeted fix",
		...
		Contributing:         []string{"missing_ca_secret", "cluster_issuer_dependency", "cert_manager_reconciliation_failure"},
		ExecutionEngine:      "job",
