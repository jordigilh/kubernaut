			...
			It("should delay aggregation until threshold is reached, then create single CRD", func() {
				...
				for i := 1; i < bufferThreshold; i++ {
					alert := createPrometheusWebhookPayload(PrometheusAlertPayload{
						...
						Severity:  "critical",
						Message:   fmt.Sprintf("Pod payment-api-%d is crash looping", i),
					})

					resp, err := sendPrometheusAlert(httpClient, gatewayURL, alert)
					Expect(err).ToNot(HaveOccurred(), fmt.Sprintf("Alert %d should be accepted", i))
					Expect(resp.StatusCode).To(Equal(http.StatusAccepted), fmt.Sprintf("Alert %d should return 202 Accepted (buffered)", i))
					resp.Body.Close()

					logger.Info("Alert buffered", zap.Int("alert_number", i), zap.Int("status_code", resp.StatusCode))
				}
				...
				thresholdAlert := createPrometheusWebhookPayload(PrometheusAlertPayload{
					...
					Severity:  "critical",
					Message:   fmt.Sprintf("Pod payment-api-%d is crash looping", bufferThreshold),
				})

				resp, err := sendPrometheusAlert(httpClient, gatewayURL, thresholdAlert)
				Expect(err).ToNot(HaveOccurred())
				Expect(resp.StatusCode).To(Equal(http.StatusAccepted), "Threshold alert should be accepted")
				resp.Body.Close()
				...
				Eventually(func() int {
					_ = k8sClient.List(testCtx, crdList, client.InNamespace(testNamespace))
				...
				logger.Info("Verifying CRD aggregation details...",
					...
					zap.Int("alert_count", createdCRD.Spec.AlertCount),
					zap.Bool("is_storm", createdCRD.Spec.IsStorm),
				)

				Expect(createdCRD.Spec.AlertCount).To(Equal(bufferThreshold), "Aggregated CRD should contain all buffered alerts")
			...
			It("should extend window lifetime and aggregate all alerts", func() {
				...
				for i := 1; i <= totalAlerts; i++ {
					alert := createPrometheusWebhookPayload(PrometheusAlertPayload{
						...
						Severity:  "warning",
						Message:   fmt.Sprintf("Node worker-node-%d has memory pressure", i),
					})
				...
				Expect(createdCRD.Spec.AlertCount).To(Equal(totalAlerts), "All alerts should be aggregated (sliding window kept open)")
			...
			It("should close window and create separate CRDs for new storms", func() {
				...
				for i := 1; i <= bufferThreshold; i++ {
					alert := createPrometheusWebhookPayload(PrometheusAlertPayload{
						AlertName: alertName,
						Namespace: testNamespace,
						NodeName:  fmt.Sprintf("storage-node-%d", i),
						Severity:  "warning",
						Message:   fmt.Sprintf("Node storage-node-%d has low disk space", i),
					})
				...
				for i := 1; i <= bufferThreshold; i++ {
					alert := createPrometheusWebhookPayload(PrometheusAlertPayload{
						AlertName: alertName,
						Namespace: testNamespace,
						NodeName:  fmt.Sprintf("storage-node-%d", i+10), // Different nodes
						Severity:  "warning",
						Message:   fmt.Sprintf("Node storage-node-%d has low disk space", i+10),
					})
				...
				Expect(crdList.Items[0].Spec.AlertCount).To(Equal(bufferThreshold))
				Expect(crdList.Items[1].Spec.AlertCount).To(Equal(bufferThreshold))
			})
		})
	})
			...
			It("should isolate buffer limits per namespace", func() {
				...
				for i := 1; i <= bufferThreshold; i++ {
					alert := createPrometheusWebhookPayload(PrometheusAlertPayload{
						...
						Severity:  "critical",
						Message:   fmt.Sprintf("Service api-server-%d is unavailable", i),
					})
				...
				for i := 1; i <= bufferThreshold; i++ {
					alert := createPrometheusWebhookPayload(PrometheusAlertPayload{
						...
						Severity:  "warning",
						Message:   fmt.Sprintf("Service test-service-%d is unavailable", i),
					})
				...
				crdList := &remediationv1alpha1.RemediationRequestList{}
				Eventually(func() int {
					_ = k8sClient.List(testCtx, crdList)
					return len(crdList.Items)
				...
				for _, crd := range crdList.Items {
					if crd.Namespace == namespace1 {
						namespace1CRDs++
						Expect(crd.Spec.AlertCount).To(Equal(bufferThreshold))
