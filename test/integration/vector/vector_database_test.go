...
var _ = Describe("Vector Database Integration", Ordered, func() {
	...
	BeforeAll(func() {
		...
		stateManager = shared.NewIsolatedTestSuiteV2("Vector Database Integration").
			WithLogger(logger).
	...
	Context("Memory Vector Database", func() {
		...
		It("should find similar patterns", func() {
			...
			similarPatterns, err := memoryDB.FindSimilarPatterns(ctx, searchPattern, 5, 0.5)

			Expect(err).ToNot(HaveOccurred())
			Expect(len(similarPatterns)).To(BeNumerically(">=", 1))
