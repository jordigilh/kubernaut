...
8. A single, provider-agnostic reasoning-depth knob (`LLMReasoningConfig.Effort`: `""`/`none`/`minimal`/`low`/`medium`/`high`/`xhigh`/`max`) controls how hard the model thinks, mapped into each client's own wire dialect (issue #1604):
   ...
   - Real OpenAI/Azure o-series and GPT-5 models (including GPT-5.6+ variants): passed through verbatim as Chat Completions' `reasoning_effort` field.
...
9. Reasoning/effort can be tuned independently per investigation phase (`phaseModels.<phase>.reasoning`) and for the shadow/alignment-checker LLM (`ai.alignmentCheck.llm.reasoning`), both merged via the same `LLMOverrideConfig` used for identity fields (issue #1616). Reasoning is explicitly a tuning field, not identity (DD-LLM-008): a phase-scoped reasoning-only hot-reload is never subject to the restart-required identity lock, and is validated with the same effort-vocabulary/Anthropic-contradiction rules as the base field, against the override's own effective provider.
10. Where captured, reasoning content is additionally streamed live from KA to AF via a dedicated MCP/A2A event type distinct from KA's pre-existing orchestration-narration event (issues #1634, #1635; [DD-LLM-009](../architecture/decisions/DD-LLM-009-reasoning-content-live-stream-event-type.md)). This is purely additive: audit surfacing per AC6 remains the durable record of record and is unaffected; the live channel is best-effort operator visibility only. Redacted reasoning (`ReasoningBlock.Redacted == true`, `Text == ""`) produces a content-free live signal (`metadata.redacted=true`, no visible text) rather than no event at all — the audit trail remains the only record of the actual (withheld) content, matching AC6 (issue #1716, DD-LLM-009 redaction sub-decision revisited).
