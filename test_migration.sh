...
echo "🧪 Testing ADR-033 Migration Script (002_adr033_multidimensional_tracking.sql)"
echo "=============================================================================="
...
echo ""
echo "📋 Step 2: Creating base schema (resource_action_traces table)..."
...
echo ""
echo "🚀 Step 4: Running migration (FIRST RUN)..."
export GOOSE_DRIVER=postgres
export GOOSE_DBSTRING="host=localhost port=5434 user=$DB_USER password=$DB_PASSWORD dbname=$DB_NAME sslmode=disable"
...
RECORD_COUNT=$(podman exec -i $CONTAINER_NAME psql -U $DB_USER -d $DB_NAME -t -c "SELECT COUNT(*) FROM resource_action_traces;")
...
RECORD_COUNT_AFTER=$(podman exec -i $CONTAINER_NAME psql -U $DB_USER -d $DB_NAME -t -c "SELECT COUNT(*) FROM resource_action_traces;")
RECORD_COUNT_AFTER=$(echo $RECORD_COUNT_AFTER | tr -d ' ')
...
RECORD_COUNT_ROLLBACK=$(podman exec -i $CONTAINER_NAME psql -U $DB_USER -d $DB_NAME -t -c "SELECT COUNT(*) FROM resource_action_traces;")
RECORD_COUNT_ROLLBACK=$(echo $RECORD_COUNT_ROLLBACK | tr -d ' ')
