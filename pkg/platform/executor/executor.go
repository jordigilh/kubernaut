...
func (e *executor) registerBuiltinActions() error {
	...
	if err := e.registry.Register("scale_deployment", e.executeScaleDeployment); err != nil {
		return fmt.Errorf("failed to register scale_deployment action: %w", err)
	}
	if err := e.registry.Register("restart_pod", e.executeRestartPod); err != nil {
		return fmt.Errorf("failed to register restart_pod action: %w", err)
	}
	if err := e.registry.Register("increase_resources", e.executeIncreaseResources); err != nil {
		return fmt.Errorf("failed to register increase_resources action: %w", err)
	}

	// Register all actions with error handling
	actions := map[string]ActionHandler{
		"notify_only":           e.executeNotifyOnly,
		"rollback_deployment":   e.executeRollbackDeployment,
		"expand_pvc":            e.executeExpandPVC,
		"drain_node":            e.executeDrainNode,
		"quarantine_pod":        e.executeQuarantinePod,
		"collect_diagnostics":   e.executeCollectDiagnostics,
		"cleanup_storage":       e.executeCleanupStorage,
		"backup_data":           e.executeBackupData,
		"compact_storage":       e.executeCompactStorage,
		"cordon_node":           e.executeCordonNode,
		"update_hpa":            e.executeUpdateHPA,
		"restart_daemonset":     e.executeRestartDaemonSet,
		"rotate_secrets":        e.executeRotateSecrets,
		"audit_logs":            e.executeAuditLogs,
		"update_network_policy": e.executeUpdateNetworkPolicy,
		"restart_network":       e.executeRestartNetwork,
		"reset_service_mesh":    e.executeResetServiceMesh,
		"failover_database":     e.executeFailoverDatabase,
		"repair_database":       e.executeRepairDatabase,
		"scale_statefulset":     e.executeScaleStatefulSet,
		"enable_debug_mode":     e.executeEnableDebugMode,
		"create_heap_dump":      e.executeCreateHeapDump,
		"optimize_resources":    e.executeOptimizeResources,
		"migrate_workload":      e.executeMigrateWorkload,
	}

	for actionName, handler := range actions {
		if err := e.registry.Register(actionName, handler); err != nil {
			return fmt.Errorf("failed to register %s action: %w", actionName, err)
		}
	}
