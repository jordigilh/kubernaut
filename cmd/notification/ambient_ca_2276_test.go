...
func generateAmbientCATestCert(t *testing.T) string {
	...
	template := &x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "notification-ambient-ca-2276-test"},
...
// IT-NOT-2276-001: a tlsCaFile configured via YAML is wired through
// bootstrapAmbientCATrust -- the exact production function run() calls
// immediately after loadNotificationConfig, before buildDeliveryServices'
// delivery-channel HTTP clients -- into ambient env vars (Issue #2276,
// SC-8).
func TestBootstrapAmbientCATrust_Notification_InjectsFromConfig(t *testing.T) {
	resetAmbientCAEnv(t)
	realCA := generateAmbientCATestCert(t)
	sharedtls.SetSystemCertFileCandidatesForTesting([]string{"/nonexistent/system-bundle.pem"})

	cfg := &notificationconfig.Config{TLSCAFile: realCA}
...
// IT-NOT-2276-002: an unset tlsCaFile leaves ambient env vars untouched
// (fail-open parity).
func TestBootstrapAmbientCATrust_Notification_NoopWhenUnset(t *testing.T) {
	resetAmbientCAEnv(t)
