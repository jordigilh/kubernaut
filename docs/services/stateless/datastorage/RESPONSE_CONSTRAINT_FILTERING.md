...
**What's Needed**:
The current search API has **hardcoded filter fields** (5 mandatory labels per DD-WORKFLOW-001 v1.8). To support DetectedLabels and CustomLabels (pass-through), we need to add JSONB filter parameters.
...
**Current Support** (per DD-WORKFLOW-001 v1.8):
...
**Filter Fields** (`pkg/datastorage/models/workflow.go`) - snake_case per DD-WORKFLOW-001 v1.8:
```go
type WorkflowSearchFilters struct {
    // 5 MANDATORY (structured columns per DD-WORKFLOW-001 v1.8)
...
**Implementation Approach** (per DD-WORKFLOW-001 v1.8 - structured columns + JSONB):
```sql
...
| Document | Relevance |
|----------|-----------|
| [DD-WORKFLOW-001 v1.8](../../../architecture/decisions/DD-WORKFLOW-001-mandatory-label-schema.md) | Mandatory label schema (snake_case API fields) |
