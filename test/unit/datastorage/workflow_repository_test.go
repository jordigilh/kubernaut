...
var _ = Describe("WorkflowRepository", func() {
	...
	BeforeEach(func() {
		...
		// Create repository with mock embedding client (proper dependency injection)
		// Using interface allows for easy mocking without nil checks
		mockEmbeddingClient := testutil.NewMockEmbeddingClient()
		repo = repository.NewWorkflowRepository(db, logger, mockEmbeddingClient)
	})
	...
	// ========================================
	// TEST: NULL/Empty Fields Edge Case
	// ========================================
	// BR-STORAGE-014: Workflow CRUD with NULL/empty optional fields
	// Edge Case: Optional fields (description, labels) may be null or empty
	// Confidence Impact: +2% (validates null safety in buildSearchText)
	Context("when creating workflows with NULL or empty fields", func() {
		It("should handle nil description gracefully", func() {
			// ARRANGE: Workflow with nil description
			workflow := &models.RemediationWorkflow{
				WorkflowID:           "test-nil-description",
				Version:              "v1.0.0",
				Name:                 "Workflow with No Description",
				Description:          "", // Empty description
				Content:              "apiVersion: v1\nkind: Pod",
				ContentHash:          "hash-nil-desc",
				Labels:               []byte(`{"signal-type":"OOMKilled","severity":"critical"}`),
				Status:               "active",
				IsLatestVersion:      true,
				TotalExecutions:      0,
				SuccessfulExecutions: 0,
			...
			// EXPECT: INSERT query with embedding from mock
			mock.ExpectExec("INSERT INTO remediation_workflow_catalog").
				WithArgs(
					workflow.WorkflowID,
					workflow.Version,
					workflow.Name,
					workflow.Description, // Empty string
					nil,                  // owner
					nil,                  // maintainer
					workflow.Content,
					workflow.ContentHash,
					workflow.Labels,
					sqlmock.AnyArg(), // embedding (generated by mock)
					workflow.Status,
					workflow.IsLatestVersion,
					nil, // previous_version
					nil, // version_notes
					nil, // change_summary
					nil, // approved_by
					nil, // approved_at
					nil, // expected_success_rate
					nil, // expected_duration_seconds
					nil, // created_by
				).
			...
			// ASSERT: No errors (null safety works with empty description)
			Expect(err).ToNot(HaveOccurred(), "Should handle empty description without crashing")
			Expect(workflow.Embedding).ToNot(BeNil(), "Mock should generate embedding")
			Expect(len(workflow.Embedding.Slice())).To(Equal(768), "Should be 768 dimensions")
			Expect(mock.ExpectationsWereMet()).To(Succeed())
		})

		It("should handle empty content gracefully", func() {
			// ARRANGE: Workflow with empty content
			workflow := &models.RemediationWorkflow{
				WorkflowID:           "test-empty-content",
				Version:              "v1.0.0",
				Name:                 "Workflow with Empty Content",
				Description:          "This workflow has no content yet",
				Content:              "", // Empty content
				ContentHash:          "hash-empty-content",
				Labels:               []byte(`{"signal-type":"OOMKilled","severity":"critical"}`),
				Status:               "active",
				IsLatestVersion:      true,
				TotalExecutions:      0,
			...
			// EXPECT: INSERT query with nil embedding (no embedding client)
			mock.ExpectExec("INSERT INTO remediation_workflow_catalog").
				WithArgs(
					workflow.WorkflowID,
					workflow.Version,
			...
			// ASSERT: No errors (null safety works with empty content)
			Expect(err).ToNot(HaveOccurred(), "Should handle empty content without crashing")
			Expect(workflow.Embedding).ToNot(BeNil(), "Mock should generate embedding")
			Expect(len(workflow.Embedding.Slice())).To(Equal(768), "Should be 768 dimensions")
			Expect(mock.ExpectationsWereMet()).To(Succeed())
		})

		It("should handle workflow with only name (minimal fields)", func() {
			// ARRANGE: Workflow with only required fields
			workflow := &models.RemediationWorkflow{
				WorkflowID:           "test-minimal-fields",
				Version:              "v1.0.0",
				Name:                 "Minimal Workflow",
				Description:          "", // Empty
				Content:              "", // Empty
				ContentHash:          "hash-minimal",
				Labels:               []byte(`{}`), // Empty labels
				Status:               "active",
				IsLatestVersion:      true,
				TotalExecutions:      0,
				SuccessfulExecutions: 0,
			...
			// ASSERT: No errors (minimal fields work)
			Expect(err).ToNot(HaveOccurred(), "Should handle minimal fields without crashing")
			Expect(workflow.Embedding).ToNot(BeNil(), "Mock should generate embedding")
			Expect(len(workflow.Embedding.Slice())).To(Equal(768), "Should be 768 dimensions")
			Expect(mock.ExpectationsWereMet()).To(Succeed())
		})
