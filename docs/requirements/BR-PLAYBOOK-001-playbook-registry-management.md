...
ADR-033 introduces the **Remediation Playbook Catalog** with a Hybrid Model (90% catalog selection + 9% chaining + 1% manual). The Playbook Catalog Service must provide a centralized registry to store, retrieve, and manage remediation playbooks and their versions.

**Current Limitations**:
- ❌ No centralized playbook registry (playbooks scattered across code/configs)
...
- ❌ No playbook metadata (description, author, tags, status)
- ❌ Cannot deprecate old playbook versions

**Impact**:
- AI cannot implement ADR-033 Hybrid Model (90% catalog selection)
- No single source of truth for remediation playbooks
...
**Provide a centralized playbook registry with versioning, metadata, and query APIs to enable ADR-033 catalog-based remediation.**

### **Success Criteria**
1. ✅ Playbook Catalog Service provides REST API to create/read/update playbooks
...
7. ✅ 100% of remediation playbooks are registered in catalog (target: 20+ playbooks)
...
**Current Flow** (Without BR-PLAYBOOK-001):
```
1. AI receives pod-oom-killer alert
2. No centralized playbook registry exists
...
3. Playbook Catalog validates and stores playbook
...
4. Playbook Catalog updates status
...
**Requirement**: Playbook Catalog Service SHALL store playbooks with comprehensive metadata.
...
### **FR-PLAYBOOK-001-02: Create Playbook API**

**Requirement**: Playbook Catalog Service SHALL provide REST API to register new playbooks.
...
**Requirement**: Playbook Catalog Service SHALL provide API to query playbooks by incident type and status.
...
**Requirement**: Playbook Catalog Service SHALL provide API to list all versions of a playbook.
...
**Requirement**: Playbook Catalog Service SHALL provide API to update playbook status (activate, deprecate).
...
- ✅ ADR-033: Remediation Playbook Catalog (defines playbook catalog architecture)
- ✅ PostgreSQL database (for playbook storage)

### **Downstream Impacts**
- ✅ BR-AI-057: AI queries playbook catalog for incident-type-specific playbooks
- ✅ BR-REMEDIATION-016: RemediationExecutor populates playbook_id/version in audits
- ✅ BR-PLAYBOOK-002: Playbook versioning and deprecation (extends this BR)
...
**Approach**: Use Git as playbook registry (YAML files in repo)
...
- BR-AI-057: AI uses playbook catalog for selection
