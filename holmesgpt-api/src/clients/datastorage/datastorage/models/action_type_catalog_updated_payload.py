...
class ActionTypeCatalogUpdatedPayload(BaseModel):
    """
    DS audit payload when an action type description is updated (SOC2: old+new)
    """ # noqa: E501
    event_type: StrictStr
    action_type: StrictStr
    old_description: ActionTypeDescriptionPayload
    new_description: ActionTypeDescriptionPayload
    updated_by: StrictStr
    updated_fields: List[StrictStr]
    __properties: ClassVar[List[str]] = ["event_type", "action_type", "old_description", "new_description", "updated_by", "updated_fields"]

    @field_validator('event_type')
    def event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('datastorage.actiontype.updated'):
            raise ValueError("must be one of enum values ('datastorage.actiontype.updated')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of old_description
        if self.old_description:
            _dict['old_description'] = self.old_description.to_dict()
        # override the default output from pydantic by calling `to_dict()` of new_description
        if self.new_description:
            _dict['new_description'] = self.new_description.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "old_description": ActionTypeDescriptionPayload.from_dict(obj.get("old_description")) if obj.get("old_description") is not None else None,
            "new_description": ActionTypeDescriptionPayload.from_dict(obj.get("new_description")) if obj.get("new_description") is not None else None,
