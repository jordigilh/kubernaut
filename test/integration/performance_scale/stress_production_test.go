...
var _ = Describe("Stress Testing and Production Scenario Simulation", Ordered, func() {
	...
	BeforeAll(func() {
		...
		if err != nil {
			Skip(fmt.Sprintf("Skipping database-dependent tests - database unavailable: %v", err))
		}

		err = testUtils.InitializeFreshDatabase()
		if err != nil {
			Skip(fmt.Sprintf("Skipping database-dependent tests - database initialization failed: %v", err))
		}
