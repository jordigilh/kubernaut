...
| 2.0 | 2025-11-27 | **CRITICAL**: Moved audit generation from Kubernaut Agent (KA) to Data Storage Service for richer context (all workflow metadata, success metrics, version history) |
| 1.0 | 2025-11-27 | Initial design with Kubernaut Agent (KA) as audit generator |
...
|-------|-------------|--------|
| **BR-AUDIT-021** | Mandatory remediation_id propagation from Kubernaut Agent (KA) | ✅ Implemented |
...
**V1.0 (Kubernaut Agent (KA))**: Limited context - only sees search response

**V2.0 (Data Storage Service)**: Full context - has access to ALL workflow data

### **Context Comparison**

| Audit Data Point | Kubernaut Agent (KA) | Data Storage Service |
|------------------|---------------|---------------------|
| Query & filters | ✅ | ✅ |
| remediation_id | ✅ | ✅ |
| Returned workflows | ✅ | ✅ |
...
Data Storage Service
  ...
  ↓ Return search results to Kubernaut Agent (KA)
  ↓
Kubernaut Agent (KA)
...
### **Step 1: Kubernaut Agent (KA) - Pass remediation_id in JSON Body**
...
- [ ] **Phase 1**: Update Kubernaut Agent (KA) (1 hour)
...
- [ ] **Phase 3**: Update tests (2 hours)
  - [ ] Remove Kubernaut Agent (KA) audit tests
...
**Note**: `finalScore` is included in the audit event for debugging and tuning (operators need to understand why certain workflows rank higher). It is **not** exposed to the LLM -- KA strips it before rendering.
