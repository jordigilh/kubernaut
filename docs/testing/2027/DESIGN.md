# Design: Ambiguous Severity-Triage Correlation for Agent-Initiated Investigations

**Issue**: [#2027](https://github.com/jordigilh/kubernaut/issues/2027) (`release/v1.5`) — cloned for `main`/v1.6 as [#2028](https://github.com/jordigilh/kubernaut/issues/2028)
**Status**: 🟡 Design agreed — implementation plan (Wiring Manifest + confidence score) not yet written
**Related**: [#2018](https://github.com/jordigilh/kubernaut/issues/2018)/[#2021](https://github.com/jordigilh/kubernaut/issues/2021) (specificity-vs-state ranking fix — confirmed correct, NOT the cause of this recurrence), [#2022](https://github.com/jordigilh/kubernaut/issues/2022) (`kubernaut.ai/managed` scope pattern, referenced below), [#2023](https://github.com/jordigilh/kubernaut/issues/2023) (harness-signal + prompt-instruction dual pattern this design reuses)
...
## 2. Root Cause — Live-Cluster-Verified (NOT a #2018 Regression)

### 2.1 #2018/#2021's fix is confirmed correct

`severity.Triager.bestOverallMatch` (`pkg/apifrontend/severity/triage.go`) was read line-by-line and
traced through its final selection switch:
...
(firing > pending) — `resourcePending`/`nsPending` are checked **before** `clusterFiring`. This is
independently confirmed by two already-passing tests that assert exactly the scenario the reporter
believed should have won: `UT-AF-2018-001` (pending resource-specific alert beats firing
cluster-scoped) and `UT-AF-2018-002` (pending namespace-scoped alert beats firing cluster-scoped),
both in `pkg/apifrontend/severity/triage_test.go`.

**This refutes the issue's Hypothesis #1** ("pending alerts still filtered out of candidacy before
ranking runs").

### 2.2 Actual root cause: a timing race, confirmed against live Prometheus data

Live query against `prometheus-k8s` (`openshift-monitoring` — the `console-e2e-alerts`
`PrometheusRule` is evaluated there, not by `prometheus-user-workload` as first assumed) for the
`ALERTS{namespace="console-e2e-lifecycle-3-633843"}` historical series:
...
At the exact moment Tier 1 took its single Prometheus snapshot, **the target's own alert did not
exist yet — not pending, not firing, zero instances.** There was no higher-specificity candidate at
all for `bestOverallMatch` to rank; it correctly fell back to the only candidate present.

Cross-checked against apifrontend logs (`kubectl logs apifrontend-... --since-time=2026-08-08T23:14:00Z`)
for the initiating call:
...
**No alert was named in the request at all** — this was a plain resource-based call
(`kubernaut_investigate(namespace=console-e2e-lifecycle-3-633843, kind=Deployment, name=worker)`,
user `console-e2e-test` via the console). The alert identity was invented entirely by
`severity.Triager` — a **deterministic Go decision inside `HandleCreateRR`**, not an LLM choice, and
made before any KA/AA session or LLM reasoning ever sees the resource. (Minor, unrelated
observation: the agent's first tool-call attempt omitted `api_version` and failed validation,
self-corrected 2.6s later — not connected to the mis-correlation.)

**Root cause**: AF's Tier 1 takes a single, non-retrying Prometheus snapshot with no tolerance for
the inherent latency of Prometheus's own alerting pipeline (metric scrape interval + rule evaluation
interval + the rule's `for` duration — ~33-63s in this fixture). When an agent-initiated
investigation is triggered near-instantly after a failure begins (faster than Prometheus can
possibly notice), Tier 1 has zero resource/namespace-scoped candidates and, under the current
design, treats a completely unrelated cluster-scoped alert as equally valid evidence — with no
notion of confidence to say otherwise.

This is a **new, distinct bug class from #2018**: an absence of a candidate at correlation time, not
a misranking of candidates that do exist. `#2021`'s own original triage had already flagged the
alternative not chosen at the time — "gate the cluster-scoped fallback out of the automatic/
interactive investigation path entirely, pending a product decision" — and #2027 is the evidence
that deferred decision needs resolving now.

## 3. Design Discussion Trail

Several directions were discussed and superseded before reaching the agreed design — recorded here
...
| 4 | **Agreed**: surface the ambiguity back through the tool result to the calling agent, in the same conversational turn, and instruct it to ask the live user for clarification | Reuses the existing conversational session (no new synchronous-pause plumbing); asks the person who actually has the context, in real time; naturally extensible (see §5) |

Key clarifying facts established during discussion:
- The decision is made by **deterministic Go code** (`bestOverallMatch`), not an LLM — there is
  nothing for a smarter model to have done differently; the ambiguity must be surfaced structurally.
- The **signal-initiated path** (Alertmanager → Gateway → RO) is out of scope: Gateway already
  rejects cluster-scoped alerts outright today, so this race cannot manifest there. This fix is
  scoped to the **agent-initiated** path (`kubernaut_investigate`/`kubernaut_remediate`/
  `kubernaut_investigate_alert`) specifically, where a live conversational session exists to ask
  through.
- A close, existing precedent already exists for "the caller supplies a definitive signal identity
  instead of letting Triager guess": `CreateRRArgs.SignalNameOverride`
  (`pkg/apifrontend/tools/af_create_rr.go`), used today by `kubernaut_investigate_alert` where the
  alert name is already known from the incoming signal. The re-call-with-confirmation flow below is
  designed to extend this same pattern, not invent a parallel one.
...
- **Out of scope**: the fully automatic signal-initiated path (Alertmanager → Gateway → RO). Gateway
  ...
  occur there today. No change needed for that path as part of this fix.
...
## 6. Preliminary FedRAMP Control Mapping
...
- [ ] TDD RED/GREEN/REFACTOR per the pyramid invariant (UT for `bestOverallMatch` confidence + the
      ...
      round trip).
- [ ] Comment on [#2027](https://github.com/jordigilh/kubernaut/issues/2027) with this direction so
      QE can adjust E2E scenarios accordingly (e.g. expect a clarification prompt rather than a
      silent misattribution when an investigation is triggered faster than Prometheus's alerting
      pipeline can catch up).
