...
7. Disabling `fleetMetadataCache.enabled` or `apifrontend.enabled`, or removing an entry
   from `additionalClusterRoles`, actually removes the corresponding
...
8. `gateway.enabled` (added 2026-08-16, Issue #2162) lets an operator fully disable the Gateway
   ...
   existing installs).
9. `additionalClusterRoleBindings` (added 2026-08-16, Issue #2160) is renamed to
...
- **FR-6** (added 2026-08-16, Issue #2159): cluster-scoped RBAC (`ClusterRole`/
  ...
  `apifrontend.enabled`, or a service's `additionalClusterRoles` list) MUST be
...
- **FR-8** (added 2026-08-16, Issue #2160, combined into the #2162 PR): `additionalClusterRoleBindings`
  (`global`/`gateway`/`effectivenessmonitor`/`kubernautAgent`) is renamed to `additionalClusterRoles`
  across `values.schema.json`, `values.yaml`, templates, tests, and docs — the field holds ClusterRole
  *names* to bind, not `ClusterRoleBinding` objects, and the original name was misleading about its
