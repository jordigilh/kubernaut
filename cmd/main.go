...
import (
	...
	aianalysiscontroller "github.com/jordigilh/kubernaut/internal/controller/aianalysis"
	kubernetesexecutioncontroller "github.com/jordigilh/kubernaut/internal/controller/kubernetesexecution"
	remediationorchestratorcontroller "github.com/jordigilh/kubernaut/internal/controller/remediationorchestrator"
	remediationprocessingcontroller "github.com/jordigilh/kubernaut/internal/controller/remediationprocessing"
...
func init() {
	...
	utilruntime.Must(remediationorchestratorv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	if err := (&remediationprocessingcontroller.RemediationProcessingReconciler{
	...
	if err := (&workflowexecutioncontroller.WorkflowExecutionReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&kubernetesexecutioncontroller.KubernetesExecutionReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
	...
	if err := (&remediationorchestratorcontroller.RemediationOrchestratorReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
