...
but not that it is actually consumed correctly at any of the two production
console-gate enforcement points on this branch (`console_access.go`'s `GET
/a2a/access` handler, `mcp_bridge.go`'s per-tool `Check`). IT coverage was
