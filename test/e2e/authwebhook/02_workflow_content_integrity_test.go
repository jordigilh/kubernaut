...
func buildRemediationWorkflowCRD(crdName, workflowID, version, description string) *rwv1alpha1.RemediationWorkflow {
...
var _ = Describe("Workflow Content Integrity E2E Tests (BR-WORKFLOW-006)", func() {
	...
	Describe("E2E-INTEGRITY-001: First CRD registration populates status", func() {
		It("should set .status.workflowId and .status.catalogStatus after CRD creation", func() {
			...
			wfID := fmt.Sprintf("e2e-wf-001-%s", suffix)

			rw := buildRemediationWorkflowCRD(crdName, wfID, "1.0.0", "First registration E2E test")
	...
	Describe("E2E-INTEGRITY-002: CRD delete triggers DS disable", func() {
		It("should disable the workflow in DS when the CRD is deleted", func() {
			...
			wfID := fmt.Sprintf("e2e-wf-002-%s", suffix)

			rw := buildRemediationWorkflowCRD(crdName, wfID, "1.0.0", "Delete triggers disable E2E")
			Expect(k8sClient.Create(ctx, rw)).To(Succeed())
	...
	Describe("E2E-INTEGRITY-003: Delete + recreate same content re-enables with same UUID", func() {
		It("should re-enable the workflow with the original UUID", func() {
			...
			wfID := fmt.Sprintf("e2e-wf-003-%s", suffix)

			rw := buildRemediationWorkflowCRD(crdName, wfID, "1.0.0", "Re-enable same content E2E")
			Expect(k8sClient.Create(ctx, rw)).To(Succeed())
			...
			rw2 := buildRemediationWorkflowCRD(crdName, wfID, "1.0.0", "Re-enable same content E2E")
	...
	Describe("E2E-INTEGRITY-004: Delete + recreate different content creates new UUID", func() {
		It("should create a new workflow record with a different UUID", func() {
			...
			wfID := fmt.Sprintf("e2e-wf-004-%s", suffix)

			rw := buildRemediationWorkflowCRD(crdName, wfID, "1.0.0", "Original content before delete")
			Expect(k8sClient.Create(ctx, rw)).To(Succeed())
			...
			rw2 := buildRemediationWorkflowCRD(crdName, wfID, "1.0.0", "Modified content after delete")
			Expect(k8sClient.Create(ctx, rw2)).To(Succeed())
	...
	Describe("E2E-INTEGRITY-005: Two CRDs same workflow identity triggers supersede", func() {
		It("should supersede the old workflow and return a new UUID for the second CRD", func() {
			...
			wfID := fmt.Sprintf("e2e-wf-005-%s", suffix)

			// CRD-A: first registration
			rwA := buildRemediationWorkflowCRD(crdNameA, wfID, "1.0.0", "Version A of workflow")
			...
			rwB := buildRemediationWorkflowCRD(crdNameB, wfID, "1.0.0", "Version B of workflow - supersedes A")
	...
	Describe("E2E-INTEGRITY-006: CRD status includes supersede metadata", func() {
		It("should populate .status.superseded and .status.supersededId on the new CRD", func() {
			...
			wfID := fmt.Sprintf("e2e-wf-006-%s", suffix)

			// CRD-A: first registration
			rwA := buildRemediationWorkflowCRD(crdNameA, wfID, "1.0.0", "Original for supersede metadata test")
			Expect(k8sClient.Create(ctx, rwA)).To(Succeed())
			...
			// CRD-B: supersedes A
			rwB := buildRemediationWorkflowCRD(crdNameB, wfID, "1.0.0", "Supersede metadata test - new content")
			Expect(k8sClient.Create(ctx, rwB)).To(Succeed())
