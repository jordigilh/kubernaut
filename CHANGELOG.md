...
- **`authwebhook` init container flaky on transient API server connectivity blips (CI smoke test hardening)** — The `patch-cabundle` init container issued a single, non-retrying `kubectl get`/`kubectl patch` call to re-apply the webhook `caBundle` on pod start. Under Kind clusters with heavy Service/Endpoint churn (repeated helm install/upgrade/uninstall cycles), a transient iptables/conntrack sync delay can make the in-cluster `kubernetes.default` ClusterIP briefly unreachable (`dial tcp 10.96.0.1:443: i/o timeout`), which the container had no way to ride out other than kubelet's slow, escalating restart backoff (multi-minute waits). Added an in-script `wait_for_api_server` retry loop (30 attempts, 2s interval, 60s budget) so the container recovers from the same transient condition in seconds instead of minutes.
