...
var _ = Describe("Category 2 & 3: Multi-Channel Delivery and Retry/Circuit Breaker", Label("integration", "multi-channel", "retry"), func() {
	...
	Context("Category 2: Multi-Channel Delivery Integration Tests", func() {
		...
		It("should handle partial channel failure gracefully (Slack fails, Console succeeds)", func() {
			...
			// CORRECTNESS VALIDATION: Slack retries according to policy (5 attempts)
			// Default retry policy: MaxAttempts = 5
			Expect(notif.Status.FailedDeliveries).To(Equal(5),
				"Slack should fail all 5 retry attempts (always-fail mode)")

			// BEHAVIOR VALIDATION: Total attempts = Console (1 success) + Slack (5 failed retries)
			Expect(notif.Status.DeliveryAttempts).To(HaveLen(6),
				"Should have exactly 6 attempts: Console (1) + Slack retries (5)")
			...
			Expect(slackAttempts).To(Equal(5), "Exactly 5 slack retry attempts")
		...
		It("should handle all channels failing gracefully", func() {
			...
			Eventually(func() notificationv1alpha1.NotificationPhase {
				...
				return notif.Status.Phase
			}, 20*time.Second, 500*time.Millisecond).Should(Equal(notificationv1alpha1.NotificationPhaseFailed),
				"Should mark as Failed when all channels fail")
			...
			Expect(notif.Status.FailedDeliveries).To(Equal(5),
				"Should fail all 5 retry attempts for Slack channel (always-fail mode with 503)")
