...
('ScaleReplicas', '{"what": "Horizontally scale a workload by adjusting the replica count.", "whenToUse": "Root cause is insufficient capacity to handle current load and the workload supports horizontal scaling.", "whenNotToUse": "", "preconditions": "Evidence of increased incoming traffic or load correlating with the resource exhaustion."}'),
('RestartPod', '{"what": "Kill and recreate one or more pods.", "whenToUse": "Root cause is a transient runtime state issue (corrupted cache, leaked connections, stuck threads) that a fresh process would resolve.", "whenNotToUse": "", "preconditions": "Evidence that the issue is transient (e.g., pod was healthy before, no recent code deployment)."}'),
('IncreaseCPULimits', '{"what": "Increase CPU resource limits on containers.", "whenToUse": "CPU throttling is caused by resource limits being too low relative to the workload actual requirements, not by a code-level issue.", "whenNotToUse": "", "preconditions": "Container is actively CPU-throttled (not just using high CPU), and CPU usage pattern is consistent with legitimate workload."}'),
('IncreaseMemoryLimits', '{"what": "Increase memory resource limits on containers.", "whenToUse": "OOM kills are caused by memory limits being too low relative to the workload actual requirements.", "whenNotToUse": "", "preconditions": "Memory usage shows a stable pattern consistent with legitimate workload, not unbounded growth over time."}'),
('RollbackDeployment', '{"what": "Revert a deployment to its previous stable revision.", "whenToUse": "Root cause is a recent deployment that introduced a regression, and the previous revision was healthy.", "whenNotToUse": "", "preconditions": "A previous healthy revision exists (verify via rollout history) and the issue started after the most recent deployment."}'),
('DrainNode', '{"what": "Drain and cordon a Kubernetes node, evicting all pods and preventing new scheduling.", "whenToUse": "Root cause is a node-level issue (hardware degradation, kernel problems, disk pressure) affecting multiple workloads on the node, and pods must be moved to healthy nodes.", "whenNotToUse": "Only a single pod is affected on the node. This indicates a pod-level issue, not node-level -- use a pod-targeted action instead. If pods don''t need to be evicted yet, use CordonNode instead.", "preconditions": "Confirmed that multiple workloads on the same node are affected, indicating node-scoped impact."}'),
('CordonNode', '{"what": "Cordon a Kubernetes node to prevent new pod scheduling without evicting existing pods.", "whenToUse": "Root cause is an emerging node-level issue that warrants preventing new pods from being scheduled, but existing pods are still running and do not need immediate eviction.", "whenNotToUse": "If existing pods on the node are already failing or need to be moved to healthy nodes, use DrainNode instead.", "preconditions": "Evidence of degrading node health (intermittent errors, rising resource pressure) but existing workloads still functional."}'),
('RestartDeployment', '{"what": "Perform a rolling restart of all pods in a workload (Deployment or StatefulSet).", "whenToUse": "Root cause is a workload-wide state issue affecting all or most pods, such as stale configuration, expired certificates, or corrupted shared state that requires all pods to be refreshed.", "whenNotToUse": "", "preconditions": "Evidence that the issue affects multiple pods in the same workload (not just a single pod), and a fresh set of pods would resolve the issue."}'),
('CleanupNode', '{"what": "Reclaim disk space on a node by purging temporary files, old logs, and unused container images.", "whenToUse": "Node disk pressure is caused by accumulated ephemeral data (temp files, old container logs, unused images), not by legitimate workload storage growth.", "whenNotToUse": "If disk usage is from legitimate workload data (persistent volumes, application databases). Cleanup would not help and could cause data loss. Use DrainNode instead if the node needs to be decommissioned.", "preconditions": "Evidence that disk usage is dominated by ephemeral/reclaimable data (container image cache, log files, tmp directories), not persistent workload data."}'),
('DeletePod', '{"what": "Delete one or more specific pods without waiting for graceful termination.", "whenToUse": "Pods are stuck in a terminal state (Terminating, Unknown) and cannot be restarted through normal means.", "whenNotToUse": "Do not use as a general restart mechanism. Use RestartPod instead for transient runtime issues.", "preconditions": "Pod is genuinely stuck and not responding to graceful termination (verify via pod events and state duration)."}'),
('GitRevertCommit', '{"what": "Revert a bad commit in a Git repository managed by GitOps (ArgoCD/Flux).", "whenToUse": "Root cause is a recent Git commit that introduced a regression in a GitOps-managed environment, and the ArgoCD/Flux controller will reconcile the reverted state.", "whenNotToUse": "When the environment is not GitOps-managed. Use RollbackDeployment instead for direct kubectl-managed workloads.", "preconditions": "GitOps tooling (ArgoCD or Flux) is active, the source Git repository is accessible, and a previous healthy commit exists."}'),
('ProvisionNode', '{"what": "Request provisioning of a new Kubernetes node to increase cluster capacity.", "whenToUse": "Pods are Pending due to insufficient cluster-wide resources (CPU, memory) and no existing node has capacity.", "whenNotToUse": "When scheduling failures are caused by taints, affinity rules, or PDB constraints rather than resource exhaustion.", "preconditions": "Confirmed that all schedulable nodes are at or near capacity and pending pods have resource requests that cannot be satisfied."}'),
('GracefulRestart', '{"what": "Perform a graceful rolling restart of a workload to reset its runtime state.", "whenToUse": "Predictive analysis indicates an impending failure (memory leak, resource exhaustion) that can be prevented by proactively restarting before the crash occurs.", "whenNotToUse": "When the issue is caused by a code bug or misconfiguration that will recur after restart.", "preconditions": "Evidence of gradual resource degradation (trending metrics) and the workload supports rolling restarts without data loss."}'),
('CleanupPVC', '{"what": "Remove old or unnecessary files from a PersistentVolumeClaim to reclaim disk space.", "whenToUse": "PVC usage has exceeded a critical threshold due to accumulated logs, temp files, or cache data.", "whenNotToUse": "When disk usage is from essential application data. Deleting it would cause data loss.", "preconditions": "PVC is mounted and accessible, and the files to be cleaned are identified as non-essential (logs, temp, cache)."}'),
('RemoveTaint', '{"what": "Remove a taint from a Kubernetes node to allow pod scheduling.", "whenToUse": "Pods are Pending because the target node has a taint that prevents scheduling, and the taint is no longer necessary.", "whenNotToUse": "When the taint was applied intentionally for maintenance or isolation purposes.", "preconditions": "The taint condition has been resolved and the node is healthy for general workload scheduling."}'),
('PatchHPA', '{"what": "Patch a HorizontalPodAutoscaler to increase maxReplicas or adjust scaling thresholds.", "whenToUse": "HPA has scaled to maxReplicas but the workload still cannot handle the load, indicating the max ceiling is too low.", "whenNotToUse": "When the scaling issue is caused by a code-level performance regression. Fix the code instead of adding more replicas.", "preconditions": "HPA is at maxReplicas, CPU/memory utilization remains above target, and the cluster has capacity for additional replicas."}'),
('RelaxPDB', '{"what": "Temporarily relax a PodDisruptionBudget to unblock a pending node drain, then restore it.", "whenToUse": "A node drain is blocked because a PDB prevents evicting the required number of pods.", "whenNotToUse": "When the PDB is correctly preventing unsafe evictions and the drain should be postponed.", "preconditions": "Confirmed that the drain is intentional and the PDB is the sole blocker. The workload can tolerate temporary reduced availability."}'),
('ProactiveRollback', '{"what": "Proactively roll back a deployment based on predictive SLO burn rate analysis.", "whenToUse": "Predictive analysis shows the current error rate will exhaust the SLO error budget before the next maintenance window.", "whenNotToUse": "When the error rate is within normal bounds or the SLO budget has sufficient headroom.", "preconditions": "A previous stable deployment revision exists, and the error spike correlates with a recent deployment change."}'),
('CordonDrainNode', '{"what": "Cordon a node to prevent new scheduling, then drain existing pods to other nodes.", "whenToUse": "A node is in NotReady state or showing signs of hardware/OS-level failure, and pods need to be relocated.", "whenNotToUse": "When only a single pod is affected. Use a pod-targeted action instead.", "preconditions": "Other healthy nodes have capacity to absorb the drained pods, and the node issue is confirmed as node-scoped."}'),
('FixCertificate', '{"what": "Recreate a missing or corrupted CA Secret backing a cert-manager ClusterIssuer to restore certificate issuance.", "whenToUse": "A cert-manager Certificate is stuck in NotReady because the CA Secret has been deleted or corrupted, and the ClusterIssuer cannot sign.", "whenNotToUse": "When the Certificate failure is caused by DNS validation issues, incorrect certificate spec, or an expired root CA that needs rotation rather than recreation.", "preconditions": "cert-manager is installed, the ClusterIssuer exists, and the failure is specifically due to a missing or corrupted CA Secret."}'),
('HelmRollback', '{"what": "Roll back a Helm release to its previous healthy revision.", "whenToUse": "A Helm-managed workload is crashing after a helm upgrade introduced a bad configuration, and the previous Helm revision was healthy.", "whenNotToUse": "When the workload is not managed by Helm (use RollbackDeployment or GracefulRestart instead), or when the crash is caused by an external dependency failure.", "preconditions": "The Helm release has at least one previous healthy revision in its history, and the Helm tiller/controller has access to roll back."}'),
('FixAuthorizationPolicy', '{"what": "Remove or fix a Linkerd AuthorizationPolicy that is blocking legitimate traffic.", "whenToUse": "A Linkerd-meshed workload has high error rates (403 Forbidden) because a restrictive AuthorizationPolicy is denying all unauthenticated or unrecognized traffic.", "whenNotToUse": "When the 403 errors are intentional security policy enforcement, or when the issue is caused by something other than an AuthorizationPolicy (e.g., network partition).", "preconditions": "Linkerd is installed, the workload is meshed with a Linkerd sidecar, and an AuthorizationPolicy has been identified as the traffic blocker."}'),
('FixStatefulSetPVC', '{"what": "Recreate a missing PVC for a StatefulSet and restart the stuck pod.", "whenToUse": "A StatefulSet pod is stuck in Pending because its PVC has been accidentally deleted or the backing PV is no longer available.", "whenNotToUse": "When the PVC exists but the storage backend itself is down (fix the storage backend instead), or when data recovery is needed before recreating.", "preconditions": "The StatefulSet exists with a volumeClaimTemplate, the missing PVC name follows the StatefulSet naming convention, and a StorageClass is available to provision a new PV."}'),
('FixNetworkPolicy', '{"what": "Remove a deny-all NetworkPolicy that is blocking legitimate ingress traffic and causing health check failures.", "whenToUse": "A NetworkPolicy is blocking all ingress traffic to a workload, causing liveness and readiness probe failures that lead to pod restarts or unavailability.", "whenNotToUse": "When the NetworkPolicy is intentionally restrictive for security purposes and the outage has a different root cause.", "preconditions": "A deny-all NetworkPolicy exists in the namespace, and the workload health checks are confirmed to be failing due to blocked network traffic rather than application errors."}')
