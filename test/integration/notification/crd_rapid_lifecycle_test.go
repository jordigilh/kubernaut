	...
	Context("BR-NOT-053: Idempotency Under Rapid Lifecycle Changes", func() {
		It("should handle rapid create-delete cycles without duplicate deliveries", func() {
			...
			for cycle := 0; cycle < 10; cycle++ {
				...
				// Small delay to allow controller to start reconciliation
		...
		It("should handle rapid create-delete-create with same CRD name", func() {
			...
			for cycle := 0; cycle < 5; cycle++ {
				...
				By(fmt.Sprintf("Cycle %d: Allowing brief processing window", cycle+1))
				time.Sleep(100 * time.Millisecond) // Slightly longer to allow delivery attempt
				...
				// Small delay before next create to ensure deletion is processed
				time.Sleep(50 * time.Millisecond)
			}
