...
- **Labels**: `severity`, `environment`, and `component` are now arrays (e.g., `severity: [critical, high]`, `component: [apps/v1/Deployment]`). `priority` remains a scalar string. Issue #790: `component` changed from scalar to array. Issue #1051: `component` uses GVK format (apiVersion/Kind).
...
spec:
  ...
  labels:
    ...
    component: [string]     # REQUIRED - array: K8s resource GVK(s) (apps/v1/Deployment, v1/Pod, v1/Node)
