...
var _ = Describe("SignalProcessing Component Integration", func() {
	...
	Context("K8sEnricher - Real K8s API Interaction", func() {
		...
		// Node enrichment - Note: ENVTEST may not have real nodes
		It("BR-SP-001: should enrich Node context when available", func() {
			Skip("ENVTEST does not provide real nodes - covered by E2E tests with Kind cluster")
		})
	...
	Context("Environment Classifier - Real ConfigMap Interaction", func() {
		...
		// Hot-reload policy change - placeholder for Day 11
		It("BR-SP-072: should detect hot-reload policy changes", func() {
			Skip("Hot-reload testing requires file watcher - covered by hot_reloader_test.go")
		})
	})
	...
	Context("OwnerChain Builder - Real K8s Traversal", func() {
		...
		// Cross-namespace owner - Note: K8s doesn't support cross-namespace owner refs
		It("BR-SP-100: should handle cross-namespace owner scenario", func() {
			Skip("Kubernetes does not support cross-namespace owner references - scenario not applicable")
		})
	})
