...
## Application-Level Hardening (v1.3)

> **Design Decision**: [DD-AUTH-014](../../architecture/decisions/DD-AUTH-014-middleware-based-sar-authentication.md)
> **Tracking**: [Issue #673](https://github.com/jordigilh/kubernaut/issues/673)

Starting with v1.3, the Gateway includes application-level security controls in addition to the network-level security described above:

| Control | Description |
|---|---|
| **SAR Authentication** | All signal endpoints require a valid Bearer token validated via Kubernetes TokenReview + SubjectAccessReview (DD-AUTH-014) |
| **Request Body Limit** | `http.MaxBytesReader` enforces a 256KB cap on request bodies before reading into memory, preventing memory exhaustion |
| **ReadHeaderTimeout** | 5-second header read timeout mitigates Slowloris-class attacks (gosec G112) |
| **Generic Error Responses** | Authentication/authorization failures return generic messages; internal error details are logged server-side only |
| **Identity Header Stripping** | Client-supplied `X-Auth-Request-User` headers are stripped before authentication; the header is only set by the middleware after successful TokenReview |
| **Least-Privilege RBAC** | Gateway ClusterRole excludes `secrets` access; scope validation uses metadata-only informers |
| **Metrics Port Alignment** | Prometheus scraping targets port 8080 (the actual Gateway listen port), with a dedicated NetworkPolicy rule |
| **K8s API Timeout** | Per-handler 15s deadline on Kubernetes API operations (BR-GATEWAY-102); returns 504 Gateway Timeout with generic message on timeout |
| **K8s Timeout Validation** | Configuration rejects `K8sRequestTimeout >= WriteTimeout` (prevents connection kill before 504 response) and `< 1s` (prevents operational DoS) |
| **Trusted Proxy RealIP** | Proxy headers (X-Forwarded-For, X-Real-IP, True-Client-IP) only honoured from configured CIDRs; fail-closed when empty (DD-AUTH-003) |

## References

- [DD-GATEWAY-006: Authentication Strategy](../architecture/decisions/DD-GATEWAY-006-authentication-strategy.md)
- [DD-AUTH-014: Middleware-Based SAR Authentication](../../architecture/decisions/DD-AUTH-014-middleware-based-sar-authentication.md)
...
### v1.3.0 (2026-04)
- Application-level hardening: body size limits, ReadHeaderTimeout, generic error responses, identity header stripping, RBAC least-privilege, metrics port fix, K8s API timeout, trusted proxy RealIP (Issue #673)
- Fixed 9x content duplication in this document
