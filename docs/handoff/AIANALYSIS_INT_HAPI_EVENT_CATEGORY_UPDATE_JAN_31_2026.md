...
```
- **Context check:** If `eventType` is a HAPI event → change `eventCategory` from `"analysis"` to `"aiagent"`
...
validators.ValidateAuditEvent(hapiEvent, validators.ExpectedAuditEvent{
    ...
    EventCategory: ogenclient.AuditEventEventCategoryAnalysis,  // ❌ CHANGE TO AiAgent
```
...
EventCategory: ogenclient.NewOptString(eventCategory),
```
- **Context check:** If querying HAPI events → change to `"aiagent"`
...
- "HAPI uses analysis category" → "HAPI uses aiagent category"
- "HolmesGPT API events under analysis" → "HolmesGPT API events under aiagent"
...
└─ NO (aianalysis.* prefix)
   └─ Keep as "analysis" / AuditEventEventCategoryAnalysis
```
...
validators.ValidateAuditEvent(hapiEvent, validators.ExpectedAuditEvent{
    EventType:     string(ogenclient.HolmesGPTResponsePayloadAuditEventEventData),
    EventCategory: ogenclient.AuditEventEventCategoryAiagent,  // ✅ Changed
    EventAction:   "response_sent",
    EventOutcome:  validators.EventOutcomePtr(ogenclient.AuditEventEventOutcomeSuccess),
    CorrelationID: correlationID,
})
...
❌ **DON'T** change AIAnalysis controller events:
```go
...
validators.ValidateAuditEvent(event, validators.ExpectedAuditEvent{
    EventType:     "aianalysis.analysis.completed",  // ← AIAnalysis prefix
    EventCategory: ogenclient.AuditEventEventCategoryAnalysis,  // ← KEEP THIS
})
```

❌ **DON'T** change queries that don't specify event_category:
```go
...
Changes:
- Updated HAPI event queries: event_category='analysis' → 'aiagent'
- Updated HAPI event validations: AuditEventEventCategoryAnalysis → AuditEventEventCategoryAiagent
...
**Good luck! This is straightforward find/replace work. Estimated 1-2 hours.** 🚀
