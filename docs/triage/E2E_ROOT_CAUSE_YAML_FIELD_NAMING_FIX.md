...
database:
  ...
  max_open_conns: 25
  max_idle_conns: 5
  conn_max_lifetime: 5m    # ❌ snake_case
  conn_max_idle_time: 10m  # ❌ snake_case
```
...
type DatabaseConfig struct {
    ...
    MaxOpenConns    int    `yaml:"maxOpenConns"`       // ❌ Expects camelCase
    MaxIdleConns    int    `yaml:"maxIdleConns"`       // ❌ Expects camelCase
    ConnMaxLifetime string `yaml:"connMaxLifetime"`    // ❌ Expects camelCase
    ConnMaxIdleTime string `yaml:"connMaxIdleTime"`    // ❌ Expects camelCase
    SSLMode         string `yaml:"sslMode"`            // ❌ Expects camelCase (not ssl_mode)
}
```
...
3. YAML parser can't map snake_case → camelCase, leaves field empty
...
database:
  ...
  name: action_history
  user: slm_user
  ssl_mode: disable          # ❌ snake_case
  max_open_conns: 25         # ❌ snake_case
  max_idle_conns: 5          # ❌ snake_case
  conn_max_lifetime: 5m      # ❌ snake_case
  conn_max_idle_time: 10m    # ❌ snake_case
...
redis:
  ...
  db: 0
  dlq_stream_name: dlq-stream        # ❌ snake_case
  dlq_max_len: 1000                  # ❌ snake_case
  dlq_consumer_group: dlq-group      # ❌ snake_case
...
database:
  ...
  name: action_history
  user: slm_user
  sslMode: disable          # ✅ camelCase
  maxOpenConns: 25          # ✅ camelCase
  maxIdleConns: 5           # ✅ camelCase
  connMaxLifetime: 5m       # ✅ camelCase
  connMaxIdleTime: 10m      # ✅ camelCase
  secretsFile: "/etc/datastorage/secrets/db-secrets.yaml"
  usernameKey: "username"
  passwordKey: "password"
redis:
  addr: redis.%s.svc.cluster.local:6379
  db: 0
  dlqStreamName: dlq-stream      # ✅ camelCase
  dlqMaxLen: 1000                # ✅ camelCase
  dlqConsumerGroup: dlq-group    # ✅ camelCase
  secretsFile: "/etc/datastorage/secrets/redis-secrets.yaml"
  passwordKey: "password"
logging:
  level: debug
...
### **Changes Made**:
1. `ssl_mode` → `sslMode`
2. `max_open_conns` → `maxOpenConns`
3. `max_idle_conns` → `maxIdleConns`
4. `conn_max_lifetime` → `connMaxLifetime` ⭐ **PRIMARY FIX**
5. `conn_max_idle_time` → `connMaxIdleTime` ⭐ **PRIMARY FIX**
6. `dlq_stream_name` → `dlqStreamName`
7. `dlq_max_len` → `dlqMaxLen`
8. `dlq_consumer_group` → `dlqConsumerGroup`
...
### **5. Code Comparison**:
- `test/infrastructure/datastorage.go:919` ✅ Uses camelCase (works)
- `test/infrastructure/authwebhook_e2e.go:829` ❌ Used snake_case (broken)
...
```go
connMaxLifetime: 1h      # ✅ Correct
connMaxIdleTime: 10m     # ✅ Correct
```
