...
**Status**: ✅ **APPROVED** (V2.6 - Action-Type Primary Matching)
...
### Version 2.6 (2026-02-05) **← CURRENT**
...
2. **Mandatory Label Filtering**: `severity`, `environment`, `priority` used as SQL WHERE filters with wildcard support
...
WHERE status = 'active'
  ...
  AND (severity = $2 OR severity = '*')               -- critical
  AND (component = $3 OR component = '*')             -- pod
...
**Breaking Change (v2.6)**: `action_type` replaces `signal_type` as primary catalog matching key. Workflow registration now requires `action_type` from enforced taxonomy (DD-WORKFLOW-016).
