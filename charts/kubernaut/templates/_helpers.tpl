...
{{/*
Return the in-cluster DataStorage cross-service readiness endpoint URL.
Consumed by every audit-writing service's pkg/audit.DataStorageProber to
gate its own /readyz on DataStorage's real reachability (#1985,
BR-AUDIT-005 v2.0).
DD-PLATFORM-010: this is now the main API port (8080), not the dedicated
kubelet-only health port (8081) -- /readyz is registered as an
unauthenticated top-level route on the same chi router as /api/v1, so it
requires no NetworkPolicy access to 8081 at all. Because 8080 always
serves TLS (Issue #753: inter-service TLS is mandatory, matching
kubernaut.datastorage.url above), this URL is https://, and it reuses
the same default-trust http.Client every other DataStorage HTTPS caller
already uses -- see pkg/audit/openapi_client_adapter.go, which never sets
a custom RootCAs/TLSClientConfig for the equivalent audit-write client.
*/}}
{{- define "kubernaut.datastorage.healthUrl" -}}
https://data-storage-service.{{ .Release.Namespace }}.svc.cluster.local:8080/readyz
{{- end }}
...
{{/*
DataStorage egress rule: allow TCP 8080 (API + cross-service /readyz) to
datastorage pods.

#1985 (BR-AUDIT-005 v2.0) added a DataStorage readiness gate to all 10
callers of this helper. DD-PLATFORM-010 (superseding the original #1985
NetworkPolicy widening) moved the cross-service /readyz check onto the
main API port 8080 -- an unauthenticated top-level route on the same chi
router as /api/v1 -- specifically so this egress rule (and the matching
datastorage NetworkPolicy's ingress side, datastorage/networkpolicy.yaml)
would NOT need a second port opened. Port 8081 stays exclusively
kubelet-facing and is never granted cross-pod ingress/egress; see
DD-PLATFORM-010 for the full rationale (dedicated ports force either
unauthenticated cross-pod exposure of the kubelet health port, or a
NetworkPolicy pinhole per caller -- both worse than one already-open,
already-authenticated port with one unauthenticated path exemption).
