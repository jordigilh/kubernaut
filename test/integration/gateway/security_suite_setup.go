...
func SetupSecurityTokens() *SecurityTestTokens {
	...
	// Create K8s client for controller-runtime operations
	step2Start := time.Now()
	GinkgoWriter.Println("  📋 Step 2: Creating controller-runtime client...")
	k8sClient := SetupK8sTestClient(ctx)
	if k8sClient == nil {
		GinkgoWriter.Println("  ❌ K8s client is nil")
		Expect(k8sClient).ToNot(BeNil(), "K8s client should be available")
	}
	GinkgoWriter.Printf("  ✓ Controller-runtime client created (took %v)\n", time.Since(step2Start))
