...
// FakeSLMClient implements the llm.Client interface for testing
...
func (f *FakeSLMClient) ChatCompletion(ctx context.Context, prompt string) (string, error) {
	...
	return `{"action": "notify_only", "confidence": 0.5, "reasoning": "Fake SLM chat completion for testing"}`, nil
}
