...
class AIAnalysisRegoEvaluationPayload(BaseModel):
    """
    Rego policy evaluation event payload (aianalysis.rego.evaluation)
    """ # noqa: E501
    outcome: StrictStr = Field(description="Evaluation outcome")
    degraded: StrictBool = Field(description="Whether evaluation ran in degraded mode")
    duration_ms: StrictInt = Field(description="Evaluation duration in milliseconds")
    reason: StrictStr = Field(description="Reason for the evaluation outcome")
    __properties: ClassVar[List[str]] = ["outcome", "degraded", "duration_ms", "reason"]
