...
// Issue #2029 Part A: reconcileInvestigating/reconcileAnalyzing must retry
// (with backoff, bounded) instead of fail-closing forever when the live
// cluster's installed AIAnalysis CRD schema rejects a Status().Update() call
// (apierrors.IsInvalid) -- e.g. CRD version skew during a rolling upgrade
// where the Go source already defines an enum value the installed CRD
// doesn't recognize yet. Previously this returned ctrl.Result{}, nil (no
// requeue, no error) on the FIRST rejection, permanently abandoning the
// AIAnalysis with no operator-visible recovery path.
//
...
func (neverCalledRegoEvaluator) Evaluate(_ context.Context, _ *rego.PolicyInput) (*rego.PolicyResult, error) {
	return nil, errors.New("unexpected call (#2029 test double): fixture has no SelectedWorkflow, Handle must short-circuit first")
}
...
// annotation persistence path #2029 Part A relies on) is never intercepted,
			...
			if subResourceName == "status" && atomic.AddInt32(&calls, 1) <= n {
				return apierrors.NewInvalid(
					...
					field.ErrorList{field.NotSupported(field.NewPath("status", "subReason"),
						"SimulatedCRDSchemaLag2029", []string{"TransientError", "PermanentError"})},
				)
			}
...
// InvestigatingHandler/AnalyzingHandler (per #2029 plan spike: both
...
// ----------------------------------------------------------------------------
// UT-AA-2029-001/002/003/005
// ----------------------------------------------------------------------------

var _ = Describe("Schema-rejection retry (#2029 Part A)", func() {
	...
	It("UT-AA-2029-001: reconcileInvestigating retries (not freezes) on apierrors.IsInvalid", func() {
		scheme := newSchemaRejectionTestScheme()
		analysis := newSchemaRejectionTestAnalysis("ut-2029-001", PhaseInvestigating)
	...
	It("UT-AA-2029-002: reconcileAnalyzing retries (not freezes) on apierrors.IsInvalid", func() {
		scheme := newSchemaRejectionTestScheme()
		analysis := newSchemaRejectionTestAnalysis("ut-2029-002", PhaseAnalyzing)
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(result.RequeueAfter).To(BeNumerically(">", 0),
			"must requeue with backoff, not fail-close forever like the pre-#2029 behavior")
	...
	It("UT-AA-2029-003: retry count annotation persists via plain Update() across repeated rejections", func() {
		scheme := newSchemaRejectionTestScheme()
		analysis := newSchemaRejectionTestAnalysis("ut-2029-003", PhaseInvestigating)
	...
	It("UT-AA-2029-005: exceeding the retry cap escalates to a terminal Failed phase with valid enum values", func() {
		scheme := newSchemaRejectionTestScheme()
		analysis := newSchemaRejectionTestAnalysis("ut-2029-005", PhaseInvestigating)
	...
	It("UT-AA-2029-014: a subsequent successful status write clears the leftover retry-count annotation", func() {
		scheme := newSchemaRejectionTestScheme()
		analysis := newSchemaRejectionTestAnalysis("ut-2029-014", PhaseInvestigating)
		analysis.Annotations = map[string]string{
