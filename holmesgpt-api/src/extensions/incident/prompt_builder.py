...
def create_incident_investigation_prompt(
    ...
    # "proactive": Proactive prevention - incident anticipated but not yet occurred
...
"""
    ...
    # BR-AI-084: Add proactive signal mode context if applicable
    if signal_mode == "proactive":
        prompt += f"""
## Proactive Signal Mode (ADR-054)

**IMPORTANT**: This is a PROACTIVE signal. The incident has NOT yet occurred.
...
### Phase 1: Investigate the {'Anticipated Incident' if signal_mode == 'proactive' else 'Incident'}
{'**PROACTIVE MODE**: This incident is anticipated based on resource trend analysis but has NOT yet occurred.' if signal_mode == 'proactive' else ''}
Use available tools to investigate the {'anticipated incident' if signal_mode == 'proactive' else 'incident'}:
...
{'- Assess resource utilization trends and recent deployments' if signal_mode == 'proactive' else ''}
{'- Determine if the prediction is likely to materialize based on current state' if signal_mode == 'proactive' else ''}

**Goal**: {'Evaluate current environment. Assess resource utilization trends, recent deployments, and current state to determine if preemptive action is warranted and how to prevent this incident.' if signal_mode == 'proactive' else 'Understand what actually happened and why.'}
...
{'Provide natural language summary of your prediction assessment + structured JSON with preventive workflow and parameters. Include whether preemptive action is recommended or if the prediction is unlikely to materialize.' if signal_mode == 'proactive' else 'Provide natural language summary + structured JSON with workflow and parameters.'}
