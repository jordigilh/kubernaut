...
// (newAnthropicModel, newVertexAIModel's Claude path), not just that the
...
var _ = Describe("Claude/Vertex-Claude LLM call timeout wiring (#1955, BR-AI-1955)", func() {
	Describe("NewModelFromConfig", func() {
		...
		// IT-AF-1955-004: vertex_ai + a claude-* model. Construction
		// requires ambient GCP ADC, which this environment may or may not
		// have — soft-checked exactly like this package's pre-existing
		// UT-AF-1252-004 (model_test.go), since either outcome still
		// proves the fix as long as a successful construction is wrapped.
		It("IT-AF-1955-004: wraps a Vertex-hosted claude model with the timeout decorator when construction succeeds", func() {
			...
			timeout, wrapped := launcher.IsTimeoutWrappedForTest(m)
			Expect(wrapped).To(BeTrue(), "newVertexAIModel must wrap its result with the #1955 timeout decorator")
			Expect(timeout).To(Equal(time.Duration(config.DefaultLLMTimeoutSeconds) * time.Second))
		})
	})
})
