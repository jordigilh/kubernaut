# DD-PLAYBOOK-004: Parameter Schema Location - Analysis

**Date**: 2025-11-15  
**Status**: Analysis - Awaiting Decision  
**Related**: DD-PLAYBOOK-003

---

## User's Question

> "Provide a confidence assessment on having the playbook images containing a JSON file in its root directory with a predefined name that contains the parameter struct (compatible with Tekton format), so that we can just use as is in the Tekton pipeline and we don't need migration."
...
Each playbook container image includes a JSON file at a predefined location:
...
├── /usr/local/bin/remediate.sh    # Playbook script
...
**Current Flow**:
```
LLM Investigation → Search Playbook Catalog → Get Schema → Populate Parameters → Execute
```
...
**Current Need** (from DD-PLAYBOOK-001):
```python
...
**Example**:
```
50 playbook containers × 100MB each = 5GB just to read schemas
```
...
**Benefits**:
- ✅ Fast playbook discovery (catalog)
...
```
┌─────────────────────────────────────────────────────────────┐
│ 1. Playbook Catalog (Fast Discovery)                        │
│    - Playbook metadata + labels                             │
...
```
┌─────────────────────────────────────────────────────────────┐
│ 1. Operator Builds Playbook Container                       │
...
│    - Updates playbook catalog                               │
└───────────────────────┬─────────────────────────────────────┘
                        │
                        ▼
┌─────────────────────────────────────────────────────────────┐
│ 3. Playbook Catalog (Auto-Updated)                          │
...
- Catalog enables fast playbook search
