...
> **Post-CI corrections (two rounds — see §11 and the §8 correction note above)**: this section was originally written before (a) the PR's CI ran to completion, and (b) without checking `git log` against its own "Not-Implemented" claim. Both produced inaccuracies, now fixed:
>
