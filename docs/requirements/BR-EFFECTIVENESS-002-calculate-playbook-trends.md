...
ADR-033 requires continuous learning feedback loops to optimize playbook selection and identify degrading playbooks. The Effectiveness Monitor must analyze historical success rate data (collected via BR-EFFECTIVENESS-001) to calculate trends, identify statistically significant changes, and generate effectiveness scores for each playbook.
...
**Current Flow**:
```
1. AI queries playbook success rates (5 playbooks)
...
5. ✅ Trend-aware playbook selection
```
...
- ✅ BR-AI-057: AI uses effectiveness scores for playbook selection
