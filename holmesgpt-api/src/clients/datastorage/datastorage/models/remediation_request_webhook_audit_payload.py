...
class RemediationRequestWebhookAuditPayload(BaseModel):
    """
    ...
    rr_name: StrictStr = Field(description="Name of the RemediationRequest")
    namespace: StrictStr = Field(description="Kubernetes namespace")
    modified_by: StrictStr = Field(description="User who modified the timeout configuration")
    modified_at: datetime = Field(description="When the modification occurred")
    old_timeout_config: Optional[TimeoutConfig] = None
    new_timeout_config: Optional[TimeoutConfig] = None
    __properties: ClassVar[List[str]] = ["event_type", "rr_name", "namespace", "modified_by", "modified_at", "old_timeout_config", "new_timeout_config"]

    @field_validator('event_type')
    def event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('webhook.remediationrequest.timeout_modified'):
            raise ValueError("must be one of enum values ('webhook.remediationrequest.timeout_modified')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of old_timeout_config
        if self.old_timeout_config:
            _dict['old_timeout_config'] = self.old_timeout_config.to_dict()
        # override the default output from pydantic by calling `to_dict()` of new_timeout_config
        if self.new_timeout_config:
            _dict['new_timeout_config'] = self.new_timeout_config.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "old_timeout_config": TimeoutConfig.from_dict(obj.get("old_timeout_config")) if obj.get("old_timeout_config") is not None else None,
            "new_timeout_config": TimeoutConfig.from_dict(obj.get("new_timeout_config")) if obj.get("new_timeout_config") is not None else None
        })
        return _obj
