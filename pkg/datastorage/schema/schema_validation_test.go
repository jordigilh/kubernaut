...
func TestSchemaValidation(t *testing.T) {
	...
	RunSpecs(t, "Database Schema Validation Suite")
}

var _ = Describe("Notification Audit Table Schema", func() {
	var db *sql.DB
	var err error
	...
	BeforeEach(func() {
		...
		if err != nil {
			Skip(fmt.Sprintf("Database connection not available: %v", err))
		}

		err = db.Ping()
		if err != nil {
			Skip(fmt.Sprintf("Database not reachable: %v", err))
		}
	})
	...
	Context("Table Columns", func() {
		It("should have all required columns with correct types", func() {
			...
			for rows.Next() {
				...
				columns[colName] = map[string]interface{}{
					"data_type":   dataType,
			...
			for colName, expected := range expectedColumns {
				...
				Expect(col["data_type"]).To(Equal(expected.dataType),
					fmt.Sprintf("Column %s should have type %s", colName, expected.dataType))

				Expect(col["is_nullable"]).To(Equal(expected.isNullable),
					fmt.Sprintf("Column %s nullable should be %s", colName, expected.isNullable))

				if expected.maxLength > 0 {
					maxLen := col["max_length"].(sql.NullInt64)
					Expect(maxLen.Valid).To(BeTrue())
					Expect(maxLen.Int64).To(Equal(expected.maxLength),
