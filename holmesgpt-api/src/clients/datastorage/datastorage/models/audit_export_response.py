...
class AuditExportResponse(BaseModel):
    ...
    """ # noqa: E501
    export_metadata: AuditExportResponseExportMetadata
    events: List[AuditExportResponseEventsInner] = Field(description="Audit events matching the query filters")
    hash_chain_verification: AuditExportResponseHashChainVerification
    detached_signature: Optional[StrictStr] = Field(default=None, description="Detached PEM-encoded signature (if requested)")
    __properties: ClassVar[List[str]] = ["export_metadata", "events", "hash_chain_verification", "detached_signature"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.events:
            ...
            _dict['events'] = _items
        # override the default output from pydantic by calling `to_dict()` of hash_chain_verification
        if self.hash_chain_verification:
            _dict['hash_chain_verification'] = self.hash_chain_verification.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "export_metadata": AuditExportResponseExportMetadata.from_dict(obj.get("export_metadata")) if obj.get("export_metadata") is not None else None,
            "events": [AuditExportResponseEventsInner.from_dict(_item) for _item in obj.get("events")] if obj.get("events") is not None else None,
            "hash_chain_verification": AuditExportResponseHashChainVerification.from_dict(obj.get("hash_chain_verification")) if obj.get("hash_chain_verification") is not None else None,
            "detached_signature": obj.get("detached_signature")
        })
        return _obj
