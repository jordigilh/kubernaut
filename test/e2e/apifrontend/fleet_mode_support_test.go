...
func configureFleetAFMockLLM(ctx context.Context, kubeconfigPath string, writer io.Writer) error {
	...
	config, err := addFleetAFScenarioSelectors(configMap.Data["config.yaml"])
	if err != nil {
...
func addFleetAFScenarioSelectors(config string) (string, error) {
	...
	newSelectors := fleetAFScenarioSelectors()
	...
	selectors.Content = append(selectorNodes, selectors.Content...)
