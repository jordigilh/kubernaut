...
- **API Frontend: auth-only fallback for the console-access gate (#2148, #2150, AC-3/AC-6)** — The coarse-grained `kubernaut.ai/console` SAR gate (#1919) was fail-closed with no permissive default: on an install with no `consoleAccessGroups`/RBAC bindings configured at all, every authenticated user got `403 Forbidden` on `GET /a2a/access` and on every `/mcp`/`/a2a/invoke` call's inline console check, since the SAR decision is entirely delegated to in-cluster RBAC bindings that simply didn't exist yet (a real install blocker, kubernaut-operator#334/#335). New `apifrontend.config.rbac.consoleAccessAuthOnly` **defaults to `true`**, matching `main`/v1.6's behavior (#2150): the console gate is authentication-only out of the box, so any non-empty authenticated user is granted console access with zero RBAC configuration required, giving dev/eval installs a working console immediately. Production installs should configure `apifrontend.config.rbac.personas`/`consoleAccessGroups` and set this to `false` to enforce SAR on the console gate too; `helm install`/`helm upgrade` prints a `NOTES.txt` reminder whenever the flag is left at its default. Per-tool authorization (`Check`) is completely unaffected either way — implemented as a `ConsoleAuthOnlyGate` decorator that embeds `*SARChecker` unchanged, so the actual security boundary is provably untouched by construction. The decision of which concrete authorizer to construct is made once, at process startup, not per-request.
