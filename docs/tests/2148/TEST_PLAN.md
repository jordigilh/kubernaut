...
defaults to `false`, so a fresh install's console works out of the box with
zero RBAC configuration for dev/eval — the exact "haven't wired OIDC groups
yet" case this feature exists to unblock. Production installs are expected
to configure `apifrontend.config.rbac.personas`/`consoleAccessGroups` and
set this to `true` explicitly to enable the authorization check on the
console gate; a NOTES.txt hint reminds operators of this whenever the flag
...
the three production console-gate enforcement points (`console_access.go`'s
`GET /a2a/access` handler, `mcp_bridge.go`'s `checkRBAC`, `agent/root.go`'s
