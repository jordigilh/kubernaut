...
func generateAmbientCATestCert(t *testing.T) string {
	...
	template := &x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "authwebhook-ambient-ca-2276-test"},
...
// IT-AW-2276-001: a tlsCaFile configured via YAML is wired through
// bootstrapAmbientCATrust -- the exact production function run() calls
// immediately after loadAuthWebhookConfig, before
// wireAuthWebhookAuditStore's DataStorage HTTP client -- into ambient env
// vars (Issue #2276, SC-8).
func TestBootstrapAmbientCATrust_AuthWebhook_InjectsFromConfig(t *testing.T) {
	resetAmbientCAEnv(t)
	realCA := generateAmbientCATestCert(t)
	sharedtls.SetSystemCertFileCandidatesForTesting([]string{"/nonexistent/system-bundle.pem"})

	cfg := &awconfig.Config{TLSCAFile: realCA}
...
// IT-AW-2276-002: an unset tlsCaFile leaves ambient env vars untouched
// (fail-open parity).
func TestBootstrapAmbientCATrust_AuthWebhook_NoopWhenUnset(t *testing.T) {
	resetAmbientCAEnv(t)
	cfg := &awconfig.Config{}
