...
class TestDataStorageAPIContract:
    ...
    def test_data_storage_accepts_snake_case_signal_type(self, integration_infrastructure):
        ...
        response = requests.post(
            ...
            json={
                ...
                "filters": {
                    "signal_type": "CrashLoopBackOff"  # snake_case
                },
                "top_k": 3,
                "min_similarity": 0.0
            },
        ...
        assert response.status_code == 200, \
            f"DD-WORKFLOW-001 v1.8: signal_type (snake_case) must work. Got {response.status_code}: {response.text}"
    ...
    def test_data_storage_accepts_custom_labels_structure(self, integration_infrastructure):
        ...
        assert response.status_code == 200, \
            f"DD-HAPI-001: custom_labels structure must be accepted. Got {response.status_code}: {response.text}"
    ...
    def test_data_storage_accepts_detected_labels_with_wildcard(self, integration_infrastructure):
        ...
        assert response.status_code == 200, \
            f"DD-WORKFLOW-001 v1.6: detected_labels with wildcard must work. Got {response.status_code}: {response.text}"
