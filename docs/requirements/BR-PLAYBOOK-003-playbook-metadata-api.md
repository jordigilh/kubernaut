...
ADR-033 requires rich playbook metadata (description, tags, author, incident types) to enable discovery, filtering, and AI selection. The Playbook Catalog Service must provide API to query and update playbook metadata without requiring full playbook re-registration.

**Current Limitations**:
- ❌ No API to update playbook metadata (description, tags) without re-registering
...
**Provide comprehensive playbook metadata API for querying, filtering, and updating playbook metadata to enhance playbook discoverability and management.**

### **Success Criteria**
1. ✅ API to query playbooks by tags
2. ✅ API to query playbooks by author
3. ✅ API to update playbook metadata (description, tags)
4. ✅ API to retrieve full playbook details (metadata + definition)
5. ✅ Supports pagination for large playbook catalogs (500+ playbooks)
...
### **FR-PLAYBOOK-003-02: Update Playbook Metadata**
...
**Acceptance Criteria**:
- ✅ Returns full playbook metadata + definition
- ✅ Returns 404 Not Found if playbook doesn't exist
