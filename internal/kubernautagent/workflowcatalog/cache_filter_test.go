...
// ========================================
// UNIT TESTS: In-memory Go filter/scoring predicates (Issue #1677 Phase 2b)
// ========================================
// Authority: DD-WORKFLOW-019 (KA owns discovery directly). Ported verbatim
// from pkg/datastorage/repository/workflow/cache_filter_test.go (Issue #1661
// Change 6) -- the predicates are unchanged, only their owning package moved.
// Business Requirements: BR-WORKFLOW-006, BR-WORKFLOW-007, BR-FLEET-003.
// ========================================

var _ = Describe("matchesArrayLabel (Issue #1677 Phase 2b)", func() {
	...
	DescribeTable("UT-KA-1677-601: case-insensitive array matching with wildcard and exclusion semantics",
...
var _ = Describe("matchesPriority (Issue #1677 Phase 2b)", func() {
	...
	DescribeTable("UT-KA-1677-602: case-insensitive scalar matching with wildcard",
...
var _ = Describe("matchesMandatoryLabels (Issue #1677 Phase 2b)", func() {
	...
	It("UT-KA-1677-603-001: nil filters match every workflow", func() {
		Expect(matchesMandatoryLabels(baseLabels(), nil)).To(BeTrue())
	})

	It("UT-KA-1677-603-002: matches when every mandatory dimension aligns", func() {
	...
	It("UT-KA-1677-603-003: rejects when severity mismatches", func() {
		filters := &models.WorkflowDiscoveryFilters{
	...
	It("UT-KA-1677-603-004: component matching is case-insensitive (K8s Kind is PascalCase)", func() {
		filters := &models.WorkflowDiscoveryFilters{Component: "V1/POD"}
		Expect(matchesMandatoryLabels(baseLabels(), filters)).To(BeTrue())
	})

	It("UT-KA-1677-603-005: cluster filter absent (non-fleet) never excludes (BR-FLEET-003)", func() {
		labels := baseLabels()
		labels.Cluster = nil
	...
	It("UT-KA-1677-603-006: cluster filter set excludes workflows with no cluster entries", func() {
		labels := baseLabels()
		labels.Cluster = nil
	...
	It("UT-KA-1677-603-007: cluster wildcard opts a workflow into every fleet classification", func() {
		labels := baseLabels()
...
var _ = Describe("matchesDetectedLabelsFilter (Issue #1677 Phase 2b)", func() {
	...
	It("UT-KA-1677-604-001: nil filter DetectedLabels matches every workflow", func() {
		Expect(matchesDetectedLabelsFilter(models.DetectedLabels{}, nil)).To(BeTrue())
	})

	It("UT-KA-1677-604-002: boolean fields never exclude regardless of workflow value", func() {
	...
	It("UT-KA-1677-604-003: string field exact match passes", func() {
	...
	It("UT-KA-1677-604-004: string field mismatch excludes", func() {
		filter := &models.DetectedLabels{GitOpsTool: "argocd"}
		Expect(matchesDetectedLabelsFilter(models.DetectedLabels{GitOpsTool: "flux"}, filter)).To(BeFalse())
	})

	It("UT-KA-1677-604-005: string field absent on workflow side passes (no requirement declared)", func() {
		filter := &models.DetectedLabels{GitOpsTool: "argocd"}
		Expect(matchesDetectedLabelsFilter(models.DetectedLabels{}, filter)).To(BeTrue())
	})

	It("UT-KA-1677-604-006: workflow-side wildcard passes any requested string value", func() {
...
var _ = Describe("detectedLabelsBoost (Issue #1677 Phase 2b)", func() {
	// Mirrors buildDetectedLabelsBoostSQL's per-field CASE WHEN weights
	// (DD-WORKFLOW-004 v1.5).

	It("UT-KA-1677-605-001: nil/empty filter DetectedLabels contributes no boost", func() {
	...
	It("UT-KA-1677-605-002: boolean field full boost when both sides true", func() {
	...
	It("UT-KA-1677-605-003: boolean field contributes nothing when workflow side is false", func() {
		filter := &models.DetectedLabels{GitOpsManaged: true}
		Expect(detectedLabelsBoost(models.DetectedLabels{}, filter)).To(Equal(0.0))
	})

	It("UT-KA-1677-605-004: string field exact match gets full boost", func() {
		filter := &models.DetectedLabels{GitOpsTool: "argocd"}
		Expect(detectedLabelsBoost(models.DetectedLabels{GitOpsTool: "argocd"}, filter)).To(BeNumerically("~", 0.10, 1e-9))
	})

	It("UT-KA-1677-605-005: workflow-side wildcard gets half boost (#215 Gap 2)", func() {
		filter := &models.DetectedLabels{GitOpsTool: "argocd"}
		Expect(detectedLabelsBoost(models.DetectedLabels{GitOpsTool: "*"}, filter)).To(BeNumerically("~", 0.05, 1e-9))
	})

	It("UT-KA-1677-605-006: query-side wildcard gets half boost for any non-empty workflow value", func() {
	...
	It("UT-KA-1677-605-007: multiple fields sum additively", func() {
...
var _ = Describe("detectedLabelsPenalty (Issue #1677 Phase 2b)", func() {
	...
	It("UT-KA-1677-606-001: nil/empty filter DetectedLabels contributes no penalty", func() {
		Expect(detectedLabelsPenalty(models.DetectedLabels{}, nil)).To(Equal(0.0))
	})

	It("UT-KA-1677-606-002: penalizes when query wants GitOps but workflow is not GitOps", func() {
		filter := &models.DetectedLabels{GitOpsManaged: true}
		Expect(detectedLabelsPenalty(models.DetectedLabels{}, filter)).To(BeNumerically("~", 0.10, 1e-9))
	})

	It("UT-KA-1677-606-003: no penalty when both sides agree on GitOps", func() {
		filter := &models.DetectedLabels{GitOpsManaged: true}
		Expect(detectedLabelsPenalty(models.DetectedLabels{GitOpsManaged: true}, filter)).To(Equal(0.0))
	})

	It("UT-KA-1677-606-004: penalizes when workflow declares a different GitOps tool", func() {
		filter := &models.DetectedLabels{GitOpsTool: "argocd"}
		Expect(detectedLabelsPenalty(models.DetectedLabels{GitOpsTool: "flux"}, filter)).To(BeNumerically("~", 0.10, 1e-9))
	})

	It("UT-KA-1677-606-005: no penalty when workflow declares matching tool or wildcard", func() {
		filter := &models.DetectedLabels{GitOpsTool: "argocd"}
...
var _ = Describe("customLabelsBoost (Issue #1677 Phase 2b)", func() {
	// Mirrors buildCustomLabelsBoostSQL's @> JSONB containment check
	// (DD-WORKFLOW-004 v1.7).

	It("UT-KA-1677-607-001: empty filter custom labels contributes no boost", func() {
	...
	It("UT-KA-1677-607-002: exact value match gets full boost per key", func() {
	...
	It("UT-KA-1677-607-003: workflow-side wildcard gets half boost", func() {
		workflow := models.CustomLabels{"team": {"*"}}
		filter := map[string][]string{"team": {"payments"}}
		Expect(customLabelsBoost(workflow, filter)).To(BeNumerically("~", 0.075, 1e-9))
	})

	It("UT-KA-1677-607-004: mismatched value contributes no boost for that key", func() {
		workflow := models.CustomLabels{"team": {"platform"}}
		filter := map[string][]string{"team": {"payments"}}
		Expect(customLabelsBoost(workflow, filter)).To(Equal(0.0))
	})

	It("UT-KA-1677-607-005: multiple keys sum additively", func() {
...
var _ = Describe("finalScore (Issue #1677 Phase 2b)", func() {
	// Mirrors the LEAST((5.0 + boost - penalty) / 10.0, 1.0) formula in
	// discovery.go's selectScoredWorkflows.

	DescribeTable("UT-KA-1677-608: normalizes and caps the raw score at 1.0",
...
var _ = Describe("matchesSearchFilters (Issue #1677 Phase 2b)", func() {
	// Mirrors applyListFilters' SQL WHERE-clause combination: List's
	...
	It("UT-KA-1677-615-001: nil filters match every workflow", func() {
		Expect(matchesSearchFilters(buildRW("wf-a", sharedtypes.CatalogStatusActive), nil)).To(BeTrue())
	})

	It("UT-KA-1677-615-002: workflow_name exact match", func() {
	...
	It("UT-KA-1677-615-003: mandatory label dimensions reuse matchesArrayLabel/matchesPriority wildcard semantics", func() {
		rw := buildRW("wf-a", sharedtypes.CatalogStatusActive)
	...
	It("UT-KA-1677-615-004: status containment -- empty filter matches any status, non-empty filter requires membership", func() {
	...
	It("UT-KA-1677-615-005: every dimension must match (AND semantics)", func() {
		rw := buildRW("wf-a", sharedtypes.CatalogStatusActive)
