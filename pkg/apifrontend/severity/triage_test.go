...
var _ = Describe("Triage Orchestrator", func() {
	...
	Describe("Fleet cluster attribution (#2394)", func() {
		...
		It("UT-AF-2394-003: hub triage selects only alerts attributed to the requested MCP Gateway cluster", func() {
			mockProm := &mockPromClient{
				alerts: []prom.Alert{
					{Labels: map[string]string{
						"alertname": "OtherClusterAlert", "namespace": "prod", "kind": "Deployment", "name": "web-api",
			...
			hubInput.ClusterID = hubClusterID
	...
	Describe("Tier 1: Namespace-scoped and cluster-scoped alert correlation (#1369)", func() {
		...
		It("UT-AF-2021-001: pending resource-specific alert wins over firing cluster-scoped alert (specificity beats state)", func() {
			mockProm := &mockPromClient{
				alerts: []prom.Alert{
					// Cluster-scoped, continuously firing, unrelated to the target (#2018/#2021 repro:
			...
			Expect(result.AlertName).To(Equal("KubePodCrashLooping"),
				"#2018/#2021: resource-specific match must win over cluster-scoped fallback regardless of firing/pending state")
			Expect(result.Severity).To(Equal("critical"))
			Expect(result.Source).To(Equal(severity.SourcePendingAlert))
		})

		It("UT-AF-2021-002: pending namespace-scoped alert wins over firing cluster-scoped alert (specificity beats state)", func() {
			mockProm := &mockPromClient{
				alerts: []prom.Alert{
			...
			Expect(result.AlertName).To(Equal("NamespaceAlert"),
				"#2018/#2021: namespace-scoped match must win over cluster-scoped fallback regardless of firing/pending state")
			Expect(result.Source).To(Equal(severity.SourceNSPendingAlert))
		})
