...
- **Breaking: `kubernautAgent.interactive.enabled` removed, now derived from `apifrontend.enabled`** (DD-PLATFORM-006 Decision Area 11, revised) — API Frontend's `ka.NewSDKMCPClient` is the only caller anywhere in the codebase of KubernautAgent's interactive `/api/v1/mcp` endpoint, so the two settings could never legitimately disagree; the field is removed (`additionalProperties: false` now rejects it) rather than merely schema-guarded against `apifrontend.enabled` as before. KubernautAgent's interactive MCP block now renders automatically whenever `apifrontend.enabled` is true (the default) and is omitted when it's `false` — remove any explicit `kubernautAgent.interactive.enabled` override from your values before upgrading; the other `kubernautAgent.interactive.*` tuning fields (`sessionTTL`, `maxConcurrentSessions`, etc.) are unaffected.
- **Breaking: `server.disableProfiling` renamed to `debug.pprofEnabled`, inverted, and extended to all 12 Go services** (#2275, BR-PLATFORM-012, AC-6) — `apifrontend.config.server.disableProfiling`, `gateway.config.server.disableProfiling`, `datastorage.config.server.disableProfiling`, and `kubernautAgent.runtime.server.disableProfiling` no longer exist. Replaced by a shared `debug.pprofEnabled` boolean (`runtime.debug.pprofEnabled` for `kubernautAgent`), defaulting to `false` (profiling **off**) on every service — note the inverted polarity: `disableProfiling: false` (profiling on) is now `pprofEnabled: true`, and `disableProfiling: true` (profiling off, the prior default) is now `pprofEnabled: false` (still the default, just spelled positively). Update any Helm values or ConfigMaps setting the old field before upgrading. The same knob is now also available on the 8 services that previously had no toggle at all (`aianalysis`, `authwebhook`, `effectivenessmonitor`, `fleetmetadatacache`, `notification`, `remediationorchestrator`, `signalprocessing`, `workflowexecution`) — `fleetmetadatacache` previously ran with profiling unconditionally **on** with no way to disable it; it now defaults to **off** like every other service. For the 7 `controller-runtime`-managed services, enabling the flag opens a dedicated `:6060` pprof listener (`ctrl.Options.PprofBindAddress`) and a matching `containerPort` on the Deployment, since `controller-runtime`'s health-probe mux cannot host custom handlers.

### Changed

- **InvestigationSession terminal-phase closure moved from AIAnalysis to API Frontend (#2214, DD-AA-KA-001 Amendment, BR-INTERACTIVE-010 SC-9)** — AIAnalysis (AA) no longer reads or writes the `InvestigationSession` (IS) CRD at all; RBAC narrowed accordingly (`investigationsessions`/`investigationsessions/status` grants removed). API Frontend (AF) gained a new `AgentSessionTerminalCloseReconciler` that watches `AgentSession` and closes the correlated IS to the matching terminal phase (`Completed`/`Failed` on a KA-written `Status.Phase` update, `Cancelled` on deletion), reusing the existing `CRDSessionService.FinalizeSessionByRR`. AA's external cascade-cancel path (`#1421`, RO patching `AIAnalysis.Status.Reason=ParentCancelled`) now deletes the correlated `AgentSession` (`AgentSessionCreator.DeleteForCascadeCancel`) instead of writing IS directly — this single delete signal is independently observed by both AF's new reconciler (closes IS) and KA's already-proven `Dispatcher.cancelOnDelete` (stops the in-flight investigation goroutine), a capability the old Status-only write never had.
- **Helm chart configuration surface reduction (#1743, DD-PLATFORM-006, BR-PLATFORM-007)** — Reduced the mandatory-field count for a minimal `helm install` and hardened several security-relevant defaults so they can no longer be silently disabled:
  - **PDB/resources consolidation (DA1/2)** — 13 duplicated per-service `pdb.{enabled,minAvailable,maxUnavailable}` blocks replaced with a single `global.podDefaults.pdb`; added `global.defaultResources` as a schema-only fallback for services that don't set their own `resources`.
  - **NetworkPolicies unconditionally mandatory (DA3)** — Removed the `networkPolicies.enabled`/per-service `networkPolicies.<service>.enabled` toggles entirely (`additionalProperties: false` now rejects them at render time); every service gets a default-deny NetworkPolicy with no opt-out.
  - **`values.yaml` trim + auto-generated reference (DA4)** — Removed 95 zero-value-default fields from the example `values.yaml` (schema defaults untouched); `kubernautAgent.llmProfileRef` now infers the sole `global.llmProfiles` entry when exactly one is defined, and requires an explicit value (failing fast, naming the candidates) when zero or 2+ profiles exist (#1987, refined from an earlier fixed `"primary"` default), dropping the mandatory-field count for a minimal install from 8 to 7. Added `hack/gen-helm-config-docs`, a `values.schema.json`-driven Markdown generator producing `docs/generated/helm-values-reference.md` (CI drift-checked), replacing the hand-maintained per-service field tables in `charts/kubernaut/README.md`.
  - **Dead field removal (DA5)** — Removed `tls.interService.enabled` (inter-service mTLS was already unconditionally on; the toggle did nothing) and `gateway.config.cors.allowedMethods` (never read by Go).
  - **Mandatory security-relevant toggles (DA6)** — `datastorage.config.auditHashKey`, `datastorage.config.redis.tls`/replay-cache TLS, `apifrontend.config.auth.replayCache`, and `datastorage.config.server.rateLimit` are now always-on, unconditional features instead of opt-in toggles that could weaken a default deployment.
  - **Audit-HMAC key auto-generation (DA7)** — A `pre-install`/`pre-upgrade` Helm hook now auto-generates and persists (`helm.sh/resource-policy: keep`) the audit hash-chain HMAC key when `datastorage.config.auditHashKey.existingSecret` is unset, instead of requiring a manually pre-created Secret.
  - **Valkey server-side TLS-only (DA8) + APIFrontend replay-cache TLS client (DA13)** — The in-chart Valkey Deployment now serves TLS-only (plaintext port disabled); APIFrontend's replay-cache Valkey client gained full TLS support (cert/key/CA, matching the hardening level of the chart's other inter-service TLS clients) to connect to it.
  - **Console/APIFrontend consistency (DA9)** — `console.enabled=true` now requires `apifrontend.enabled=true` (fails fast at render time); a `NOTES.txt` hint is shown when `console.ingress.enabled=false`.
...
- **`apifrontend.mcp.session_init` audit event only fired once per pod lifetime (#2139, AU-3)** — `NewMCPHandler`'s session-init audit logic deduplicated on the incoming request's (always-empty, pre-assignment) `Mcp-Session-Id` header via a handler-lifetime `sync.Map`, so only the very first session-less `/mcp` request a given API Frontend pod ever received got audited; every subsequent new MCP session was silently skipped. The MCP SDK only invokes this callback when creating a brand-new session in the first place (existing sessions route directly, bypassing it), so the dedup guard was solving a problem that didn't exist and is removed entirely — every new session is now audited.
