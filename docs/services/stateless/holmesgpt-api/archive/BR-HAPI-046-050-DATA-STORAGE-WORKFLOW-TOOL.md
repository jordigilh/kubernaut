...
>
> This document describes a **pre-implementation design** that was superseded by the actual implementation.
> The functionality described here has been implemented with a different architecture.
>
> ### Current Implementation (BR-HAPI-250):
> | Component | Location | Description |
> |-----------|----------|-------------|
> | **Workflow Catalog Toolset** | `src/toolsets/workflow_catalog.py` | HolmesGPT SDK toolset with `search_workflows` tool |
> | **Data Storage Client** | `src/clients/datastorage/client.py` | REST client with `search_workflows()`, `get_workflow_by_uuid()` |
> | **Semantic Search** | Data Storage API | Two-phase search (exact labels + pgvector similarity) |
> | **Hybrid Scoring** | DD-WORKFLOW-004 | `confidence = base_similarity + label_boost - label_penalty` |
>
> ### Key Differences from This Document:
> - Tool name: `search_workflows` (not `get_playbooks`)
> - Endpoint: `POST /api/v1/workflows/search` (not `/api/v1/context/enrich`)
> - No circuit breaker/caching in client (handled by infrastructure)
> - Native HolmesGPT SDK toolset pattern (not custom tool registration)
...
**Priority**: High
**Status**: 🗄️ **ARCHIVED** - Superseded by BR-HAPI-250 (`src/toolsets/workflow_catalog.py`)
