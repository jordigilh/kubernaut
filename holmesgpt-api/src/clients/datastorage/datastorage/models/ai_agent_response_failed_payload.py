...
class AIAgentResponseFailedPayload(BaseModel):
    """
    ...
    """ # noqa: E501
    event_type: StrictStr = Field(description="Event type for discriminator")
    ...
    error_message: StrictStr = Field(description="Error message from the failed investigation")
    phase: StrictStr = Field(description="Phase in which the failure occurred")
    duration_seconds: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Duration of the investigation before failure (seconds)")
    __properties: ClassVar[List[str]] = ["event_type", "event_id", "incident_id", "error_message", "phase", "duration_seconds"]

    @field_validator('event_type')
    def event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('aiagent.response.failed'):
            raise ValueError("must be one of enum values ('aiagent.response.failed')")
        return value
