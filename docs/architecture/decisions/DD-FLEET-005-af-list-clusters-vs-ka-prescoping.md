# DD-FLEET-005: AF's LLM-Facing `list_clusters`/`cluster_id` Is Not an Asymmetry with DD-FLEET-004

**Status**: ✅ Accepted (documentation-only, no code change)
**Date**: 2026-07-29
**Author**: AI Assistant
**Related**: Issue #1768 (Track 3), DD-FLEET-004, ADR-068
...
DD-FLEET-004 made the opposite choice for KubernautAgent (KA): it removed
...
The question: is AF's design a regression against DD-FLEET-004's stated
...
DD-FLEET-004's argument rests entirely on one premise, stated directly in
its Context section:
...
- **AF's agent**, by contrast, is a general-purpose, multi-turn, user-facing
  ...
  "pick a cluster" decision at this layer that DD-FLEET-004's premise says
  never happens in KA's RCA phase.

In other words, DD-FLEET-004's fix targets a *specific, narrow* mismatch
...
tool-list construction in AF's agent, mirroring the refactor DD-FLEET-004
...
asymmetry with DD-FLEET-004 — it is DD-FLEET-004's own premise (no cluster
...
**Negative**:
- The residual prompt-injection/scope-escape surface DD-FLEET-004 called
...
Issue #1768 (Track 3), DD-FLEET-004, ADR-068.
