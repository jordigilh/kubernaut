...
Release images carry SLSA v1.0 **Build Level 3** provenance. Provenance is generated by an isolated reusable workflow (`.github/workflows/slsa-provenance.yml`), not inline in `release.yml` itself -- so the signer identity below is `slsa-provenance.yml`, a workflow `release.yml` cannot influence or forge. That isolation is exactly what distinguishes Build L3 from L2 (see [#1109](https://github.com/jordigilh/kubernaut/issues/1109)):

```bash
cosign verify-attestation \
  --type slsaprovenance \
  --certificate-identity-regexp "^https://github.com/jordigilh/kubernaut/\.github/workflows/slsa-provenance\.yml@.*$" \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  quay.io/kubernaut-ai/<service>:<version>
```

Equivalently, with the GitHub CLI, explicitly pinning to the isolated signer workflow:

```bash
gh attestation verify oci://quay.io/kubernaut-ai/<service>:<version> \
  --owner jordigilh \
  --signer-workflow jordigilh/kubernaut/.github/workflows/slsa-provenance.yml
```
