...
// #2121 (BR-AI-2121, FedRAMP AU-3): sameKindValidationGate and
...
// package's established convention). newTestInvestigator (apiversion_gate_
// test.go) is used instead of investigator.New to satisfy #1677's
// fail-closed catalog-validation hardening (main-only) for the
// workflow-selection phase that follows each gate.
var _ = Describe("#2121: gate retries must recover from (or account for) an undeclared tool call", func() {
	...
	// submit_result during a gate retry — the #2121 failure mode. Real
	...
	Describe("UT-KA-2121-001 (regression): sameKindValidationGate recovers after one undeclared-tool retry", func() {
		It("must accept the reminder-retry's result and record retry_outcome=resolved_after_other_tool_retry (BR-AI-2121 AC1)", func() {
			mockClient.responses = []llm.ChatResponse{
				...
				// Gate retry attempt 1: LLM calls an undeclared tool instead of submit_result (#2121).
			...
			Expect(result.RemediationTarget.Kind).To(Equal("Deployment"),
				"UT-KA-2121-001: final result must reflect the reminder retry's data, not the original Pod target")
			Expect(mockClient.calls).To(HaveLen(4),
				"UT-KA-2121-001: expected RCA + first gate retry + reminder retry + workflow-selection LLM calls")

			gateEvents := auditStore.eventsByAction(audit.ActionSameKindGate)
			Expect(gateEvents).NotTo(BeEmpty(), "UT-KA-2121-001: gate must emit an audit event")
			ev := gateEvents[0]
			Expect(ev.Data["retry_outcome"]).To(Equal("resolved_after_other_tool_retry"),
				"UT-KA-2121-001: audit trail must distinguish a reminder-assisted recovery from a first-try resolution")
			Expect(ev.EventOutcome).To(Equal(audit.OutcomeSuccess),
				"UT-KA-2121-001: a recovered retry is a successful gate outcome")
		})
	})

	Describe("UT-KA-2121-002: sameKindValidationGate keeps the original result when the undeclared tool call persists", func() {
		It("must keep the pre-retry result and record retry_outcome=llm_requested_other_tool (BR-AI-2121 AC2)", func() {
			mockClient.responses = []llm.ChatResponse{
				{Message: llm.Message{Role: "assistant", Content: `{
					"rca_summary":"Pod OOMKilled",
					"confidence":0.85,
			...
			Expect(result.RemediationTarget.Kind).To(Equal("Pod"),
				"UT-KA-2121-002: original (pre-retry) result must be kept when the reminder retry also fails")
			Expect(mockClient.calls).To(HaveLen(4),
				"UT-KA-2121-002: expected RCA + first gate retry + reminder retry + workflow-selection LLM calls")

			gateEvents := auditStore.eventsByAction(audit.ActionSameKindGate)
			Expect(gateEvents).NotTo(BeEmpty(), "UT-KA-2121-002: gate must emit an audit event")
			ev := gateEvents[0]
			Expect(ev.Data["retry_outcome"]).To(Equal("llm_requested_other_tool"),
				"UT-KA-2121-002: audit trail must record that the LLM never answered with submit_result")
			Expect(ev.EventOutcome).To(Equal(audit.OutcomeFailure),
				"UT-KA-2121-002: an unresolved retry must be a failed gate outcome, not the hardcoded success of pre-#2121 code")
		})
	})

	Describe("UT-KA-2121-003: apiVersionValidationGate recovers after one undeclared-tool retry", func() {
		It("must accept the reminder-retry's api_version and record retry_outcome=resolved_after_other_tool_retry (BR-AI-2121 AC1)", func() {
			mockClient.responses = []llm.ChatResponse{
				{Message: llm.Message{Role: "assistant", Content: `{
			...
			Expect(result.RemediationTarget.APIVersion).To(Equal("operators.coreos.com/v1alpha1"),
				"UT-KA-2121-003: final result must reflect the reminder retry's api_version")
			Expect(result.HumanReviewNeeded).To(BeFalse(),
				"UT-KA-2121-003: a recovered retry must not trigger human review")
			Expect(mockClient.calls).To(HaveLen(4),
				"UT-KA-2121-003: expected RCA + first gate retry + reminder retry + workflow-selection LLM calls")

			gateEvents := auditStore.eventsByAction(audit.ActionAPIVersionGate)
			Expect(gateEvents).NotTo(BeEmpty(), "UT-KA-2121-003: gate must emit an audit event")
			ev := gateEvents[0]
			Expect(ev.Data["retry_outcome"]).To(Equal("resolved_after_other_tool_retry"),
				"UT-KA-2121-003: audit trail must distinguish a reminder-assisted recovery from a first-try resolution")
			Expect(ev.EventOutcome).To(Equal(audit.OutcomeSuccess),
				"UT-KA-2121-003: a recovered retry is a successful gate outcome")
		})
	})

	Describe("UT-KA-2121-004: apiVersionValidationGate triggers human review when the undeclared tool call persists", func() {
		It("must trigger human review and record retry_outcome=llm_requested_other_tool (BR-AI-2121 AC2 Security)", func() {
			mockClient.responses = []llm.ChatResponse{
				{Message: llm.Message{Role: "assistant", Content: `{
					"rca_summary":"Subscription etcd needs restart",
					"confidence":0.85,
			...
			Expect(result.HumanReviewNeeded).To(BeTrue(),
				"UT-KA-2121-004: an unresolved ambiguous-kind retry must still trigger human review "+
			...
			Expect(result.HumanReviewReason).To(Equal("rca_incomplete"),
				"UT-KA-2121-004: reason must be rca_incomplete, matching the existing exhaustion convention")

			gateEvents := auditStore.eventsByAction(audit.ActionAPIVersionGate)
			Expect(gateEvents).NotTo(BeEmpty(), "UT-KA-2121-004: gate must emit an audit event")
			ev := gateEvents[0]
			Expect(ev.Data["retry_outcome"]).To(Equal("llm_requested_other_tool"),
				"UT-KA-2121-004: audit trail must record that the LLM never answered with submit_result")
			Expect(ev.EventOutcome).To(Equal(audit.OutcomeFailure),
				"UT-KA-2121-004: gate exhaustion must be a failed outcome, not the hardcoded success of pre-#2121 code")
		})
	})

	// UT-KA-2121-005 is the regression test for the secondary finding: both
	...
	// stores), this test would fail against the pre-#2121 store-then-mutate
	// code and passes once the store call is deferred until every Data
	// mutation has happened (BR-AI-2121 AC3, FedRAMP AU-3).
	Describe("UT-KA-2121-005 (regression): retry_outcome/EventOutcome are captured at StoreAudit call time, not just in a later in-memory read", func() {
		It("must have set retry_outcome=resolved and EventOutcome=success on sameKindValidationGate's audit event before storing it", func() {
			...
			gateEvents := auditStore.eventsByAction(audit.ActionSameKindGate)
			Expect(gateEvents).NotTo(BeEmpty(), "UT-KA-2121-005: gate must emit an audit event")
			ev := gateEvents[0]
			Expect(ev.Data["retry_outcome"]).To(Equal("resolved"),
				"UT-KA-2121-005: retry_outcome must be captured by StoreAudit at call time, not mutated afterward")
			Expect(ev.EventOutcome).To(Equal(audit.OutcomeSuccess),
				"UT-KA-2121-005: EventOutcome must be captured by StoreAudit at call time, not the pre-#2121 "+
		...
		It("must have set retry_outcome=exhausted and EventOutcome=failure on apiVersionValidationGate's audit event before storing it", func() {
			...
			Expect(ev.Data["retry_outcome"]).To(Equal("exhausted"),
				"UT-KA-2121-005: retry_outcome must be captured by StoreAudit at call time — this is the "+
					"pre-existing dead-mutation bug (#2124) this PR fixes alongside #2121")
			Expect(ev.EventOutcome).To(Equal(audit.OutcomeFailure),
				"UT-KA-2121-005: EventOutcome must reflect the actual (failed) outcome, not the pre-#2121 "+
					"unconditional hardcoded success")
		})
	})
})
