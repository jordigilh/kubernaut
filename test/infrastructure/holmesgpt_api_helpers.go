...
func GetHAPIE2ETestWorkflows() []TestWorkflow {
	...
	baseWorkflows := []TestWorkflow{
		{WorkflowID: "oomkill-increase-memory-v1", Name: "OOMKill Remediation - Increase Memory Limits", Description: "Increases memory limits for pods experiencing OOMKilled events", ActionType: "IncreaseMemoryLimits", Severity: "critical", Component: "pod", Priority: "P0", SchemaImage: hapiWorkflowRegistry + "/oomkill-increase-memory:v1.0.0",
		...
		{WorkflowID: "memory-optimize-v1", Name: "OOMKill Remediation - Scale Down Replicas", Description: "Reduces replica count for deployments experiencing OOMKilled", ActionType: "ScaleReplicas", Severity: "high", Component: "deployment", Priority: "P1", SchemaImage: hapiWorkflowRegistry + "/memory-optimize:v1.0.0",
			SchemaParameters: []models.WorkflowParameter{
		...
		{WorkflowID: "crashloop-config-fix-v1", Name: "CrashLoopBackOff - Fix Configuration", Description: "Identifies and fixes configuration issues causing CrashLoopBackOff", ActionType: "RestartDeployment", Severity: "high", Component: "pod", Priority: "P1", SchemaImage: hapiWorkflowRegistry + "/crashloop-config-fix:v1.0.0",
			SchemaParameters: []models.WorkflowParameter{
				{Name: "NAMESPACE", Type: "string", Required: true, Description: "Target namespace"},
		...
		{WorkflowID: "node-drain-reboot-v1", Name: "NodeNotReady - Drain and Reboot", Description: "Safely drains and reboots nodes in NotReady state", ActionType: "DrainNode", Severity: "critical", Component: "node", Priority: "P0", SchemaImage: hapiWorkflowRegistry + "/node-drain-reboot:v1.0.0",
			SchemaParameters: []models.WorkflowParameter{
		...
		{WorkflowID: "image-pull-backoff-fix-credentials", Name: "ImagePullBackOff - Fix Registry Credentials", Description: "Fixes ImagePullBackOff errors by updating registry credentials", ActionType: "RollbackDeployment", Severity: "high", Component: "pod", Priority: "P1", SchemaImage: hapiWorkflowRegistry + "/imagepull-fix-creds:v1.0.0"},
		{WorkflowID: "generic-restart-v1", Name: "Generic Pod Restart", Description: "Generic pod restart for unknown issues", ActionType: "RestartPod", Severity: "medium", Component: "deployment", Priority: "P2", SchemaImage: hapiWorkflowRegistry + "/generic-restart:v1.0.0",
			SchemaParameters: []models.WorkflowParameter{
				{Name: "NAMESPACE", Type: "string", Required: true, Description: "Target namespace"},
