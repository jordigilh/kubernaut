...
type MockAuditStore struct {
	StoredEvents []*pkgaudit.AuditEvent
	StoreError   error
}

func (m *MockAuditStore) StoreAudit(ctx context.Context, event *pkgaudit.AuditEvent) error {
	if m.StoreError != nil {
		return m.StoreError
	}
	...
	Describe("BR-SP-090: RecordError", func() {
		Context("Happy Path", func() {
			...
			It("AC-HP-06: should record error with phase and error message", func() {
				...
				Expect(*event.ErrorMessage).To(Equal("connection timeout"))
			})
		})
	})
