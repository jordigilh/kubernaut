...
var _ = Describe("StoreAdapter", func() {
	...
	Describe("DD-AUDIT-003 v2.2: ClusterID propagation (CC8.1)", func() {
		It("UT-AF-FLEET-001: propagates ClusterID to AuditEventRequest [CC8.1, AU-3]", func() {
			adapter.Emit(context.Background(), &audit.Event{
				Type:      audit.EventRRCreated,
				ClusterID: "prod-east",
				UserID:    "sre-user",
			...
			evt := store.lastEvent()
			Expect(evt).NotTo(BeNil())
			Expect(evt.ClusterID.IsSet()).To(BeTrue(),
				"CC8.1: StoreAdapter must propagate Event.ClusterID to AuditEventRequest.ClusterID")
			Expect(evt.ClusterID.Value).To(Equal("prod-east"),
				"CC8.1: ClusterID value must match the source event")
		})

		It("UT-AF-FLEET-002: leaves ClusterID unset for single-cluster events (backward compat)", func() {
			adapter.Emit(context.Background(), &audit.Event{
				Type:   audit.EventRRCreated,
				UserID: "sre-user",
				Detail: map[string]string{
			...
			evt := store.lastEvent()
			Expect(evt).NotTo(BeNil())
			Expect(evt.ClusterID.IsSet()).To(BeFalse(),
				"Single-cluster events must not set ClusterID on the request")
		})

		It("UT-AF-FLEET-003: propagates ClusterID for deduplicated events [CC8.1]", func() {
			adapter.Emit(context.Background(), &audit.Event{
				Type:      audit.EventRRDeduplicated,
				ClusterID: "prod-west",
				UserID:    "sre-user",
			...
			Expect(evt).NotTo(BeNil())
			Expect(evt.ClusterID.IsSet()).To(BeTrue())
			Expect(evt.ClusterID.Value).To(Equal("prod-west"),
				"CC8.1: ClusterID must be propagated for deduplicated events too")
		})
	})
})
