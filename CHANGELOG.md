...
- **API Frontend: auth-only fallback for the console-access gate (#2148, AC-3/AC-6)** — The coarse-grained `kubernaut.ai/console` SAR gate (#1919) was fail-closed with no permissive default: on an install with no `consoleAccessGroups`/RBAC bindings configured at all, every authenticated user got `403 Forbidden` on `GET /a2a/access` and on every `/mcp`/`/a2a/invoke` call's inline console check, since the SAR decision is entirely delegated to in-cluster RBAC bindings that simply didn't exist yet (a real install blocker, kubernaut-operator#334/#335). New `apifrontend.config.rbac.consoleAccessAuthOnly` (default `false`, unchanged behavior) makes the console gate authentication-only when set: any non-empty authenticated user is granted console access without a SAR call. Per-tool authorization (`Check`) is completely unaffected — implemented as a `ConsoleAuthOnlyGate` decorator that embeds `*SARChecker` unchanged, so the actual security boundary is provably untouched by construction. The decision of which concrete authorizer to construct is made once, at process startup, not per-request.
