...
import (
	...
	config "github.com/jordigilh/kubernaut/pkg/authwebhook/config"
)

// BR-PLATFORM-012, Issue #2275: extend the shared Debug.PprofEnabled toggle
// (secure-by-default, off) to AuthWebhook, a controller-runtime-managed
...
var _ = Describe("BR-PLATFORM-012: Debug.PprofEnabled secure default (Issue #2275)", func() {
	It("UT-AW-2275-001 DefaultConfig disables profiling by default (secure-by-default)", func() {
		cfg := config.DefaultConfig()
		Expect(cfg.Debug.PprofEnabled).To(BeFalse())
	})

	It("UT-AW-2275-002 YAML can opt in to profiling by setting debug.pprofEnabled: true", func() {
		cfg := config.DefaultConfig()
