...
var _ = Describe("Phase 1: AF Standalone (Realistic)", Label("e2e", "phase1"), func() {
	...
	Context("Agent Card", func() {
		It("should serve agent card JSON at /.well-known/agent-card.json", func() {
			...
			Expect(skills).To(HaveLen(14), "agent card should advertise all 14 MCP tools as skills")
		})
	})
