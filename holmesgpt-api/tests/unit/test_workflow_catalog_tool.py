...
class TestInputValidation:
    ...
    def test_empty_query_returns_error_u1_1(self):
        ...
        # ACT
        with patch('src.toolsets.workflow_catalog.requests.post') as mock_post:
    ...
    def test_excessive_top_k_capped_u1_7(self):
        ...
        with patch('src.toolsets.workflow_catalog.requests.post') as mock_post:
            ...
            assert request_json["top_k"] <= 100, \
                f"U1.7: top_k must be capped, got {request_json['top_k']}"
    ...
    def test_invalid_min_similarity_handled_u1_8(self):
        ...
        with patch('src.toolsets.workflow_catalog.requests.post') as mock_post:
            ...
            if mock_post.called:
                ...
                min_sim = request_json.get("min_similarity", 0.3)
...
class TestResponseTransformation:
    ...
    def test_transforms_uuid_workflow_id_u2_1(self, mock_post):
        """
        U2.1: workflow_id UUID is correctly parsed from v3.0 response

        Integration Outcome: UUID preserved without transformation errors
        """
        # ARRANGE - Mock v3.0 flat response
        test_uuid = "1c7fcb0c-d22b-4e7c-b994-749dd1a591bd"
        ...
        data = json.loads(result.data)
        assert data["workflows"][0]["workflow_id"] == test_uuid, \
            "U2.1: workflow_id must be preserved"

        # Validate UUID format
        parsed_uuid = uuid.UUID(data["workflows"][0]["workflow_id"])
        assert str(parsed_uuid) == test_uuid, "U2.1: workflow_id must be valid UUID"

    @patch('src.toolsets.workflow_catalog.requests.post')
    def test_transforms_title_field_u2_2(self, mock_post):
        """
        U2.2: 'title' field parsed correctly (not 'name')

        Integration Outcome: v3.0 'title' field accessible
        """
        # ARRANGE - Mock v3.0 response with 'title'
        mock_post.return_value.status_code = 200
        mock_post.return_value.elapsed.total_seconds.return_value = 0.05
        mock_post.return_value.raise_for_status = Mock()
        mock_post.return_value.json.return_value = {
    ...
    @patch('src.toolsets.workflow_catalog.requests.post')
    def test_transforms_singular_signal_type_u2_3(self, mock_post):
        """
        U2.3: 'signal_type' is string (not array) per v3.0

        Integration Outcome: signal_type is singular string
        """
        # ARRANGE - Mock v3.0 response with singular signal_type
        mock_post.return_value.status_code = 200
        mock_post.return_value.elapsed.total_seconds.return_value = 0.05
        mock_post.return_value.raise_for_status = Mock()
        mock_post.return_value.json.return_value = {
    ...
    @patch('src.toolsets.workflow_catalog.requests.post')
    def test_transforms_confidence_score_u2_4(self, mock_post):
        """
        U2.4: 'confidence' float parsed correctly
    ...
    @patch('src.toolsets.workflow_catalog.requests.post')
    def test_handles_null_optional_fields_u2_5(self, mock_post):
        """
        U2.5: null container_image/container_digest handled gracefully
        ...
        mock_post.return_value.json.return_value = {
            "workflows": [{
                ...
                "signal_type": "OOMKilled",
                "confidence": 0.87,
                "container_image": None,  # Optional - can be null
                "container_digest": None  # Optional - can be null
            }],
            "total_results": 1
        }
...
class TestErrorHandling:
    ...
    @patch('src.toolsets.workflow_catalog.requests.post')
    def test_http_error_returns_structured_error_u3_1(self, mock_post):
        """
        U3.1: HTTP 500 error returns ERROR status with message
    ...
    @patch('src.toolsets.workflow_catalog.requests.post')
    def test_invalid_json_returns_error_u3_2(self, mock_post):
        """
        U3.2: Malformed JSON response returns ERROR status

        Integration Outcome: JSON parse errors produce structured ERROR
        """
        # ARRANGE - Mock response with invalid JSON
        mock_post.return_value.status_code = 200
        mock_post.return_value.elapsed.total_seconds.return_value = 0.05
        mock_post.return_value.raise_for_status = Mock()
        mock_post.return_value.json.side_effect = json.JSONDecodeError("Invalid", "", 0)
        ...
        # ASSERT
        assert result.status == StructuredToolResultStatus.ERROR, \
            "U3.2: Invalid JSON must return ERROR status"
        assert result.error is not None, \
            "U3.2: Error message must be provided"

    @patch('src.toolsets.workflow_catalog.requests.post')
    def test_connection_timeout_returns_error(self, mock_post):
        """
    ...
    @patch('src.toolsets.workflow_catalog.requests.post')
    def test_connection_refused_returns_error(self, mock_post):
        """
