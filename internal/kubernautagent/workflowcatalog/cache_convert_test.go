...
// ========================================
// UNIT TESTS: CRD -> models label/workflow converters (Issue #1677 Phase 2b)
// ========================================
// Authority: DD-WORKFLOW-019. Ported verbatim from
// pkg/datastorage/repository/workflow/cache_convert_test.go (Issue #1661
// Change 6) -- these converters let the cache-backed Step 1/2 discovery path
// (ListActions, ListWorkflowsByActionType) reuse the filter/scoring
// predicates (cache_filter.go) and models.RemediationWorkflow response shape
// without touching Postgres.
// ========================================

var _ = Describe("crdLabelsToMandatoryLabels (Issue #1677 Phase 2b)", func() {
	It("UT-KA-1677-609-001: maps every field 1:1 (CRD and models.MandatoryLabels share the same shape)", func() {
	...
	It("UT-KA-1677-609-002: nil/empty Cluster maps to nil (BR-FLEET-003 exclusion semantics preserved)", func() {
		got := crdLabelsToMandatoryLabels(rwv1alpha1.RemediationWorkflowLabels{Priority: "P2"})
		Expect(got.Cluster).To(BeEmpty())
	})
})

var _ = Describe("crdCustomLabelsToModel (Issue #1677 Phase 2b)", func() {
	It("UT-KA-1677-610-001: wraps each single CRD value in a one-element slice", func() {
	...
	It("UT-KA-1677-610-002: nil input maps to an empty (non-nil) CustomLabels", func() {
...
var _ = Describe("crdDetectedLabelsToModel (Issue #1677 Phase 2b)", func() {
	It("UT-KA-1677-611-001: unmarshals the CRD's raw JSON into models.DetectedLabels", func() {
	...
	It("UT-KA-1677-611-002: nil input maps to a zero-value DetectedLabels without error", func() {
		got, err := crdDetectedLabelsToModel(nil)
		Expect(err).ToNot(HaveOccurred())
		Expect(got.IsEmpty()).To(BeTrue())
	})

	It("UT-KA-1677-611-003: malformed JSON returns an error", func() {
...
var _ = Describe("crdActionTypeToEntry (Issue #1677 Phase 2b)", func() {
	It("UT-KA-1677-613-001: maps spec.name/description and the supplied workflow count", func() {
...
var _ = Describe("crdWorkflowToModel (Issue #1677 Phase 2b)", func() {
	...
	It("UT-KA-1677-612-001: maps identity/execution fields needed by the discovery response", func() {
	...
	It("UT-KA-1677-612-003: maps spec.parameters[] into the {\"schema\":{\"parameters\":[...]}} envelope models.RemediationWorkflow.Parameters has always used (Phase 2b: HandleGetWorkflowByID's documented contract requires it for LLM parameter validation; the envelope matches buildWrappedWorkflowParameters/the OpenAPI object schema, not a bare array -- a bare array breaks the ogen client's map[string]jx.Raw decoder)", func() {
		rw := buildRW()
	...
	It("UT-KA-1677-612-004: no declared parameters maps to a nil (omitted) parameters field", func() {
		got, err := crdWorkflowToModel(buildRW())
		Expect(err).ToNot(HaveOccurred())
		Expect(got.Parameters).To(BeNil())
	})

	It("UT-KA-1677-612-005: catalog-only fields with no CRD equivalent are zero-valued (not a regression -- confirmed nil today for inline registration too)", func() {
		got, err := crdWorkflowToModel(buildRW())
		Expect(err).ToNot(HaveOccurred())
