...
func generateAmbientCATestCert(t *testing.T) string {
	...
	template := &x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "gateway-ambient-ca-2276-test"},
...
// IT-GW-2276-001: a tlsCaFile configured via YAML is wired through
// bootstrapAmbientCATrust -- the exact production function run() calls
// immediately after loadGatewayConfig, before telemetry.Bootstrap's OTel
// exporter -- into ambient env vars (Issue #2276, SC-8).
func TestBootstrapAmbientCATrust_Gateway_InjectsFromConfig(t *testing.T) {
	resetAmbientCAEnv(t)
	realCA := generateAmbientCATestCert(t)
	sharedtls.SetSystemCertFileCandidatesForTesting([]string{"/nonexistent/system-bundle.pem"})

	cfg := &config.ServerConfig{TLSCAFile: realCA}
...
// IT-GW-2276-002: an unset tlsCaFile leaves ambient env vars untouched
// (fail-open parity).
func TestBootstrapAmbientCATrust_Gateway_NoopWhenUnset(t *testing.T) {
	resetAmbientCAEnv(t)
