...
class TestMCPFilterInstructionsFailedDetections:
    ...
    def test_mcp_filter_handles_missing_failed_detections_key(self):
        ...
        detected_labels = {
            # No failedDetections key at all
            "gitOpsManaged": True,
            "pdbProtected": False,
            "stateful": True
        }
...
class TestSearchWorkflowsFailedDetections:
    ...
    def test_search_workflows_strips_failed_detection_fields(self):
        ...
        assert "pdbProtected" not in clean_labels
        # But other fields preserved
        assert clean_labels.get("gitOpsManaged") == True
        assert clean_labels.get("gitOpsTool") == "argocd"
        assert clean_labels.get("stateful") == True
        # failedDetections itself should also be stripped (not needed by DS)
        assert "failedDetections" not in clean_labels
    ...
    def test_search_workflows_strips_multiple_failed_detections(self):
        ...
        assert "pdbProtected" not in clean_labels
        assert "hpaEnabled" not in clean_labels
        assert "networkIsolated" not in clean_labels
        # Reliable fields preserved
        assert clean_labels.get("gitOpsManaged") == True
        assert clean_labels.get("stateful") == True
        assert clean_labels.get("helmManaged") == True
    ...
    def test_search_workflows_preserves_all_when_no_failures(self):
        ...
        assert clean_labels.get("gitOpsManaged") == True
        assert clean_labels.get("pdbProtected") == True
        assert clean_labels.get("stateful") == False
        assert "failedDetections" not in clean_labels
    ...
    def test_search_workflows_handles_missing_failed_detections_key(self):
        ...
        assert clean_labels.get("gitOpsManaged") == True
        assert clean_labels.get("pdbProtected") == False


class TestRecoveryMCPFilterFailedDetections:
    """
    Tests for recovery.py _build_mcp_filter_instructions honoring failedDetections.

    Ensures recovery flow also respects failed detections.
    """

    def test_recovery_mcp_filter_excludes_failed_detections(self):
        """
        Business Outcome: Recovery workflow search also excludes failed detections
        """
        from src.extensions.recovery import _build_mcp_filter_instructions

        detected_labels = {
            "failedDetections": ["pdbProtected", "stateful"],
            "gitOpsManaged": True,
            "pdbProtected": False,
            "stateful": False,
            "helmManaged": True
        ...
        # Business outcome: Failed detections excluded
        assert "pdb_protected" not in instructions.lower()
        assert '"stateful"' not in instructions.lower()
        # But reliable labels included
        assert "gitops_managed" in instructions.lower()
        assert "helm_managed" in instructions.lower()


class TestRecoveryClusterContextFailedDetections:
    """
    Tests for recovery.py _build_cluster_context_section honoring failedDetections.
    """

    def test_recovery_cluster_context_excludes_failed_detections(self):
        """
        Business Outcome: Recovery LLM prompt excludes unreliable characteristics
        """
        from src.extensions.recovery import _build_cluster_context_section

        detected_labels = {
            "failedDetections": ["pdbProtected"],
            "pdbProtected": True,   # Unreliable
            "gitOpsManaged": True,  # Reliable
            "gitOpsTool": "argocd"
        }
        ...
        # Business outcome: PDB not mentioned
        assert "PDB" not in context.upper()
