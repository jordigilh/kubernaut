...
| **v2.0** | 2026-03-04 | Added `aiagent.response.failed` event type to `aiagent` category for failure audit trail. `AIAgentResponseFailedPayload` captures `error_message`, standardized `error_details`, `phase`, and `duration_seconds`. SOC2 CC7.2 compliance: failed investigations now have a reconstructable audit trail. Cross-references: #442, PR #443, DD-AUDIT-005 (hybrid provider audit), DD-ERROR-001. | Architecture Team |
| **v2.1** | 2026-03-25 | Added `aiagent.enrichment.completed` and `aiagent.enrichment.failed` event types to `aiagent` category for Phase 2 enrichment audit trail. New `AIAgentEnrichmentCompletedPayload` captures root_owner, detected_labels, owner_chain_length, remediation_history_fetched. New `AIAgentEnrichmentFailedPayload` captures failure reason, detail, and affected_resource. SOC2 CC8.1: enrichment operations now have complete audit coverage. Cross-references: #533, #529 (three-phase RCA architecture). | Architecture Team |
...
| `aiagent` | AI Agent Provider (Kubernaut Agent) | Autonomous AI agent with tool-calling for investigations, recovery, and effectiveness analysis | `aiagent.llm.request`, `aiagent.llm.response`, `aiagent.llm.tool_call`, `aiagent.workflow.validation_attempt`, `aiagent.response.complete`, `aiagent.response.failed`, `aiagent.enrichment.completed`, `aiagent.enrichment.failed` |
...
| `webhook` | Authentication Webhook Service | Operator attribution for CRD operations (SOC2 CC8.1) | `webhook.workflowexecution.block_cleared`, `webhook.notificationrequest.deleted`, `webhook.remediationapprovalrequest.decided`, `remediationworkflow.admitted.create`, `remediationworkflow.admitted.delete`, `remediationworkflow.admitted.denied` (DD-WEBHOOK-003, ADR-058) |
