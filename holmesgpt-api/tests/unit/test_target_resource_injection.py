...
class TestCanonicalParamValidation:
    ...
    def test_ut_hapi_496_007_rejects_missing_target_resource_name(self):
        """UT-HAPI-496-007: Workflow missing TARGET_RESOURCE_NAME is rejected."""
        ...
        assert any("TARGET_RESOURCE_NAME" in e for e in result.errors)

    def test_ut_hapi_496_008_rejects_missing_target_resource_kind(self):
        """UT-HAPI-496-008: Workflow missing TARGET_RESOURCE_KIND is rejected."""
        workflow = _make_mock_workflow([
        ...
        assert result.is_valid is False
        assert any("TARGET_RESOURCE_KIND" in e for e in result.errors)

    def test_ut_hapi_496_009_rejects_missing_target_resource_namespace(self):
        """UT-HAPI-496-009: Workflow missing TARGET_RESOURCE_NAMESPACE is rejected."""
        workflow = _make_mock_workflow([
            "TARGET_RESOURCE_NAME",
            "TARGET_RESOURCE_KIND",
            "MEMORY_LIMIT_NEW",
        ...
        assert result.is_valid is False
        assert any("TARGET_RESOURCE_NAMESPACE" in e for e in result.errors)
...
class TestPromptAndParser(unittest.TestCase):
    ...
    def test_ut_hapi_496_016_prompt_instructs_get_resource_context(self):
        """UT-HAPI-496-016: Prompt instructs LLM to call get_resource_context."""
        prompt = create_incident_investigation_prompt(STANDARD_REQUEST_DATA)
        self.assertIn("get_resource_context", prompt)
