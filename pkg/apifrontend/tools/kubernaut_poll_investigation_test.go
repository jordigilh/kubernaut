...
var _ = Describe("kubernaut_poll_investigation", func() {
	...
	It("UT-AF-112-001: returns in_progress with user-friendly progress string", func() {
		...
		result, err := tools.HandlePollInvestigation(ctx, kaClient, tools.PollInvestigationArgs{SessionID: "sess-1"}, 1, 100*time.Millisecond, nil)
	...
	It("UT-AF-112-003: blocks polling KA at configured interval", func() {
		...
		kaClient := ka.NewClient(ka.Config{BaseURL: server.URL})
		result, err := tools.HandlePollInvestigation(ctx, kaClient, tools.PollInvestigationArgs{SessionID: "sess-1"}, 3, 50*time.Millisecond, nil)
		Expect(err).NotTo(HaveOccurred())
		Expect(result.Status).To(Equal("in_progress"))
	...
	It("UT-AF-112-004: returns early on status change", func() {
		...
		kaClient := ka.NewClient(ka.Config{BaseURL: server.URL})
		result, err := tools.HandlePollInvestigation(ctx, kaClient, tools.PollInvestigationArgs{SessionID: "sess-1"}, 10, 50*time.Millisecond, nil)
		Expect(err).NotTo(HaveOccurred())
		Expect(result.Status).To(Equal("completed"))
	...
	It("UT-AF-112-005: tracks poll count and returns timeout after max polls", func() {
		...
		kaClient := ka.NewClient(ka.Config{BaseURL: server.URL})
		result, err := tools.HandlePollInvestigation(ctx, kaClient, tools.PollInvestigationArgs{SessionID: "sess-1"}, 3, 10*time.Millisecond, nil)
		Expect(err).NotTo(HaveOccurred())
		Expect(result.Status).To(Equal("in_progress"))
		Expect(result.PollCount).To(Equal(3))
	})
	...
	It("UT-AF-112-006: respects context cancellation during blocking", func() {
		...
		_, err := tools.HandlePollInvestigation(cancelCtx, kaClient, tools.PollInvestigationArgs{SessionID: "sess-1"}, 100, 50*time.Millisecond, nil)
