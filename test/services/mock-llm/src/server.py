...
class MockLLMRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        """Handle GET requests (health checks, model list, metrics)."""
        if self.path == "/health":
            ...
            response = metrics_collector.get_metrics()
            response["service"] = "mock-llm"
            response["version"] = "1.0.0"
        elif self.path == "/api/tags" or self.path == "/v1/models":
    ...
    def _tool_call_response(self, scenario: MockScenario, request_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Record for test validation and metrics
...
class MockLLMServer:
    ...
    def start(self):
        ...
        # Clear tool call tracker and reset metrics
...
def create_mock_llm_fixture(force_text_response: bool = False):
    ...
    Usage in conftest.py:
        from test.services.mock_llm.src import create_mock_llm_fixture
...
if __name__ == "__main__":
    print("Starting mock LLM server on port 11434...")
    with MockLLMServer(port=11434) as server:
        print(f"Mock LLM running at {server.url}")
        print("Health check: GET {}/health".format(server.url))
        print("Metrics: GET {}/metrics".format(server.url))
