# DD-PLAYBOOK-010: MCP Playbook Catalog Integration in HolmesGPT API - Confidence Assessment

**Date**: 2025-11-15  
**Status**: ✅ Approved  
**Target Version**: v1.0 (MVP)  
**Related**: DD-PLAYBOOK-008, DD-PLAYBOOK-009, DD-EMBEDDING-001, DD-STORAGE-008

---

## Proposal

**Integrate MCP playbook catalog search logic directly inside kubernaut-agent instead of developing a separate MCP service wrapper.**
...
└───────────────────────┬─────────────────────────────────────┘
                        │
                        │ HTTP POST /mcp/tools/search_playbook_catalog
                        ▼
┌─────────────────────────────────────────────────────────────┐
│ Mock MCP Server (Python Flask) - TEMPORARY TEST SERVICE     │
│ - In-memory playbook dictionary                             │
...
│ - Playbook catalog table with full schema                   │
...
**Current Problem**:
- **Mock MCP Server** is a temporary test service duplicating playbook search logic
...
- **Estimated Improvement**: 20-100ms saved per playbook search
...
- **Alignment**: Playbook search is another "toolset" calling existing REST API
...
- **Risk**: HolmesGPT API takes on playbook search responsibility
- **Current Responsibilities**: RCA orchestration, LLM interaction
- **Added Responsibility**: MCP tool implementation for playbook search
- **Concern**: "God object" anti-pattern
- **Counter-Argument**: 
  - Playbook search is a "tool" for RCA, not a separate domain
...
- **Risk**: Testing playbook search requires Data Storage Service
...
- **Risk**: Playbook search logic only available to HolmesGPT API
...
- **Counter-Argument**:
  ...
  - No other service currently needs MCP-style playbook search
...
└───────────────────────┬─────────────────────────────────────┘
                        ...
                        ▼
┌─────────────────────────────────────────────────────────────┐
│ MCP Playbook Catalog Service (Separate Service)             │
...
- ✅ **Reusability**: Other services can use MCP Playbook Catalog
...
2. Implement MCP tool: `search_playbook_catalog`
...
class PlaybookMCPTool:
    """
    MCP tool for playbook catalog search.
    ...
    async def search_playbook_catalog(
        self,
        query: str,
        labels: Dict[str, str],
        ...
        """
        Search playbook catalog via Data Storage Service.
...
async def analyze_recovery(request_data: Dict[str, Any], app_config: Dict[str, Any]):
    ...
    # Search playbooks
    playbooks = await playbook_tool.search_playbook_catalog(
...
**Mitigation**: Keep playbook MCP logic in separate module (`playbook_mcp.py`)  
...
### ✅ **RECOMMENDED: Integrate MCP Playbook Tool in HolmesGPT API (Option A)**
...
1. ✅ Keep playbook MCP logic in separate module (`playbook_mcp.py`)
...
- [ ] Implement `PlaybookMCPTool` class with `search_playbook_catalog` method
...
- [ ] Test end-to-end RCA with playbook search
...
**Recommendation**: Integrate MCP playbook tool in kubernaut-agent for v1.0 MVP  
**Confidence**: 92%  
