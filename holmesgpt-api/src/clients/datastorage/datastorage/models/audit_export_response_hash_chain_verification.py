...
class AuditExportResponseHashChainVerification(BaseModel):
    """
    AuditExportResponseHashChainVerification
    """ # noqa: E501
    total_events_verified: StrictInt = Field(description="Total events with hash chain data")
    valid_chain_events: StrictInt = Field(description="Events with valid hash chains")
    broken_chain_events: StrictInt = Field(description="Events with broken hash chains (tampered)")
    chain_integrity_percentage: Optional[Union[Annotated[float, Field(le=100.0, strict=True, ge=0.0)], Annotated[int, Field(le=100, strict=True, ge=0)]]] = Field(default=None, description="Percentage of events with valid chains")
    verification_timestamp: datetime = Field(description="When verification was performed")
    tampered_event_ids: Optional[List[StrictStr]] = Field(default=None, description="String UUIDs of tampered events (if any)")
    __properties: ClassVar[List[str]] = ["total_events_verified", "valid_chain_events", "broken_chain_events", "chain_integrity_percentage", "verification_timestamp", "tampered_event_ids"]
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "total_events_verified": obj.get("total_events_verified"),
            "valid_chain_events": obj.get("valid_chain_events"),
            "broken_chain_events": obj.get("broken_chain_events"),
            "chain_integrity_percentage": obj.get("chain_integrity_percentage"),
            "verification_timestamp": obj.get("verification_timestamp"),
            "tampered_event_ids": obj.get("tampered_event_ids")
        })
        return _obj
