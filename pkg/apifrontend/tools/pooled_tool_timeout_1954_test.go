...
// doesn't complete within bound, instead of letting a regression (no
// timeout applied anywhere in the chain) hang the whole suite until the
// outer `go test` process timeout.
func awaitWithSafetyBound(bound time.Duration, fn func()) {
