...
var _ = Describe("Kubernaut Agent audit coverage 668 (BR-KA-197 DD-AUDIT-002)", func() {
	...
	Describe("toIncidentResponseData human_review_reason mapping (BR-KA-197)", func() {
		...
		It("UT-KA-2020-010: maps decision_expired onto the ogen enum value (#2019/#2020 audit whitelist)", func() {
			recorder := &fakeOgenClient{}
			store := audit.NewDSAuditStore(recorder)
			...
			Expect(payload.ResponseData.NeedsHumanReview.Value).To(BeTrue())
			Expect(payload.ResponseData.HumanReviewReason.Set).To(BeTrue(),
				"AU-2/AU-3: decision_expired must not be silently dropped from the audit record (#2019/#2020)")
