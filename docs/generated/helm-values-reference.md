...
| `config.rbac.consoleAccessAuthOnly` | boolean | Makes the coarse-grained console gate (kubernaut.ai/console) authentication-only, bypassing the SAR call. Defaults to true so dev/eval installs get a working console with zero RBAC configuration (#2150); production installs should configure apifrontend.config.rbac.consoleAccessGroups and set this to false to enforce SAR on the console gate. Per-tool authorization is unaffected either way and remains unconditionally fail-closed. | `true` | No |
