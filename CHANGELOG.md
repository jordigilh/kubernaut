...
- **API Frontend: auth-only fallback for the console-access gate (#2148, #2150, AC-3/AC-6)** — The coarse-grained `kubernaut.ai/console` gate (#1919) was fail-closed with no permissive default: on an install with no `consoleAccessGroups`/RBAC bindings configured at all, every authenticated user got `403 Forbidden` on `GET /a2a/access` and on every `/mcp`/`/a2a/invoke` call's inline console check, since the decision is entirely delegated to in-cluster RBAC bindings that simply didn't exist yet (a real install blocker, kubernaut-operator#334/#335). New `apifrontend.config.rbac.consoleAccessAuthorizationCheckEnabled` **defaults to `false`**, matching `main`/v1.6's behavior (#2150): the console gate is authentication-only out of the box, so any non-empty authenticated user is granted console access with zero RBAC configuration required, giving dev/eval installs a working console immediately. Production installs should configure `apifrontend.config.rbac.personas`/`consoleAccessGroups` and set this to `true` so `consoleAccessGroups` takes precedence and the authorization check (a SAR call) runs on the console gate too; `helm install`/`helm upgrade` prints a `NOTES.txt` reminder whenever the flag is left at its default. Per-tool authorization (`Check`) is completely unaffected either way — implemented as a `ConsoleAccessAuthorizationCheckGate` decorator that embeds `*SARChecker` unchanged, so the actual security boundary is provably untouched by construction. The decision of which concrete authorizer to construct is made once, at process startup, not per-request.
...
- **MCP takeover could reconstruct "0 prior turns" even though the in-flight autonomous investigation had already succeeded (#2156)** — `handleTakeover`'s context-reconstruction read raced ahead of the investigation goroutine's own result write: `TransitionToUserDriving` flips session status and cancels the investigation's context synchronously, then `handleTakeover` immediately read `sess.Result` with no wait, so a successful-but-not-yet-landed investigation was invisible to the takeover response (surfacing as "0 prior turns reconstructed" instead of the real count). Replaced the read-immediately behavior with a real completion signal: `session.Session` gains a `done` channel closed exactly once by the investigation goroutine's own deferred cleanup, and `handleTakeover` now waits on it (bounded only by the request context) via `session.Manager.WaitForCompletionByRemediationID` instead of guessing with a fixed retry budget. Also fixes a latent hang in `IT-KA-1425-001`, uncovered while porting: its test fixture blocked on a `gate` channel that ignored `ctx.Done()`, harmless under the old retry design but a deadlock risk under the new wait — fixed to respect cancellation like real production code.
...
- **`submit_result` schema-repair retry could fabricate a low-confidence RCA instead of surfacing the real gate audit data (#1777, backport tracked as #1980, merged via #1779)** — Gate audit events now populate real `prompt_length`/`prompt_preview` values.
- **`kubernaut_get_approval_request` rejected bare RAR names (#1493, backport merged via #1958)** — Now accepts bare names, matching `kubernaut_get_remediation_request`'s existing behavior.
- **`ParseRARID` diverged from `ParseRRID` in how it handled bare names (#1959, merged via #1962)** — Simplified `ParseRARID` to bare-name-only, matching `ParseRRID`.
- **Pooled MCP tool calls and Claude/Vertex-Claude LLM calls had no client-side timeout, allowing a stalled upstream call to block indefinitely (#1954, #1955, merged via #1960)** — Both call paths are now bounded with client-side timeouts.
...
- **Kind v0.32.0 bump (containerd config v4 support, CI infra) exposed several latent Helm chart / smoke-test gaps** — bumping the CI runner's Kind version (required: `v0.30.0` cannot parse the newer `containerd` config v4 format shipped by current GitHub Actions runners) brought in a newer `kindnet` CNI that, unlike the older one v1.5.3 shipped against, actually enforces `NetworkPolicy` and schedules pods onto a more CPU-constrained single-node control-plane. This surfaced four previously-inert issues, all fixed as part of this release (none are regressions in this release's own code — main had already independently fixed each one over time):
  - **NetworkPolicy silently blocked all API-server egress/ingress (main PR #1571)** — `networkPolicies.apiServerCIDR` defaulted to `""` with no auto-discovery, so no NetworkPolicy-selected pod ever got an explicit allow rule for the API server at all; `authwebhook`'s `patch-cabundle` init container (the only pod making a *hard-required*, non-optional API call at startup) hung indefinitely (`Init:0/1`). Confirmed live that even setting the CIDR manually to the `kubernetes` Service's ClusterIP does not work: NetworkPolicy `ipBlock` rules are evaluated against the *post-DNAT* destination on kindnet (and most CNIs), and the real backend commonly listens on `6443`, not the Service's port `443`. Backported main's fix: `networkPolicies.apiServerCIDR`/`apiServerCIDRs`/`apiServerPort` now auto-discover the API server's real backend endpoint IP(s) and port via a Helm `lookup` against the live `kubernetes` Endpoints object during any real `helm install`/`upgrade` — no manual `--set` needed in the common case. Manual override remains available (required for `helm template`/GitOps rendering, where `lookup` has no live cluster access).
  - **`authwebhook` init container had no retry around `kubectl`, and unreliable in-cluster config auto-detection (main #1542)** — a single non-retrying `kubectl get`/`kubectl patch` call, combined with `kubectl`'s in-cluster config auto-detection being unreliable in the `bitnami/kubectl` image, meant any transient API-server blip (documented by main as commonly 60-90s under load) was fatal. Backported main's `kube()`/`kube_retry()` wrapper, which explicitly configures the service-account token/CA/server and wraps every call in a 20-attempt retry loop.
  - **Concurrent image pulls from GHCR during pod startup starved the control-plane** — unlike `main`'s smoke-test job (which uploads images as workflow artifacts and does a local `kind load image-archive`), this branch's build job pushes to `ghcr.io`, so all ~13 pods pulled their image over the network simultaneously, stacking TLS/decompression CPU cost on top of API-server/etcd/kubelet load and starving `kube-apiserver` (observed silent for ~19 minutes in one CI run). Added a preload step that pulls each image once up front and `kind load image-archive`s it, so pod starts are a pure local containerd lookup with zero network pulls, matching `main`'s effective behavior.
  - **Concurrent controller-runtime startup saturated the single-node control-plane's CPU**, delaying health-server bind past controllers' liveness-probe budgets and causing unrelated `CrashLoopBackOff`. Raised `initialDelaySeconds` 10-30s → 120s across the 7 affected controller charts, and ported main's active pod-recovery loop (`34840e312`) into `scripts/helm-smoke-test.sh`, which deletes crash-looping pods for an immediate clean-slate retry instead of waiting out kubelet's exponential backoff.
...
- **Sibling interactive/autonomous sessions sharing a `remediation_id` left stuck non-terminal (#1654)** — An MCP `action=start` fallback session can be created for a remediation ID before AA's own autonomous investigation session for that same ID leaves `Running`; both share `remediation_id` metadata in `session.Manager`'s store. `ForceCompleteByRemediationID` returned after completing only the first non-terminal match found in (non-deterministic) map iteration, and `CompleteHTTPSession` skipped the force-complete sweep entirely once `FindUserDrivingByRemediationID` found a match — so whichever sibling session AA was actually polling could be left stuck in `Investigating` until its own inactivity timer eventually fired, minutes later (root cause of the E2E-FP-1456-001 flake). `ForceCompleteByRemediationID` now completes every non-terminal sibling and emits a `SessionCompleted` audit event per session (previously this path had no audit trace at all); `CompleteHTTPSession` now always attempts both completion paths.
- **`select_workflow`/`complete_no_action` never cancelled the session's inactivity timer (#1654)** — `buildMCPTools`-equivalent wiring in `main.go` never passed the shared `TimeoutManager` into `CompleteNoActionTool` (the `WithCompleteNoActionTimeoutTracker` option existed but was never called), and `SelectWorkflowTool` had no way to receive one at all. Either path terminating an interactive session left its inactivity timer running to fire later against an already-terminal session, producing a spurious "failed to release expired session" log and an incorrect timeout-completion attempt. Added `WithSelectWorkflowTimeoutTracker` and wired both trackers into their respective tool constructions.
