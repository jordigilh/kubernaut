...
ADR-033 requires playbook versioning with effectiveness-based deprecation. The Playbook Catalog Service must manage playbook lifecycle transitions (draft → active → deprecated) based on effectiveness data from the Effectiveness Monitor, ensuring AI always selects the most effective playbook versions.
...
**Current Flow** (Without BR-PLAYBOOK-002):
```
...
**Desired Flow with BR-PLAYBOOK-002**:
```
1. Team creates v1.3 (status: draft)
...
**Requirement**: Playbook Catalog SHALL enforce playbook lifecycle state machine.
...
### **FR-PLAYBOOK-002-02: Promote Playbook API**

**Requirement**: Playbook Catalog SHALL provide API to promote playbook from draft to active.
...
### **FR-PLAYBOOK-002-03: Deprecate Playbook API**

**Requirement**: Playbook Catalog SHALL provide API to deprecate playbook.
...
**Requirement**: Playbook Catalog SHALL provide API to compare two playbook versions.
...
- ✅ BR-PLAYBOOK-001: Playbook registry (provides playbook storage)
