...
func (s *OllamaIntegrationSuite) SetupSuite() {
	...
	// Setup test Kubernetes environment
	testEnv, err := SetupFakeEnvironment()
	s.Require().NoError(err, "Failed to setup test Kubernetes environment")
	s.testEnv = testEnv

	s.logger.Info("Test Kubernetes environment setup complete")
}
...
func (s *OllamaIntegrationSuite) TearDownSuite() {
	...
	if s.testEnv != nil {
		err := s.testEnv.Cleanup()
		if err != nil {
			s.logger.WithError(err).Error("Failed to cleanup test environment")
		}
	}
		...
		s.Run(testCase.Name, func() {
			...
			if s.Assert().NoError(err, "Alert analysis should succeed") &&
				...
				s.Assert().Contains([]string{"scale_deployment", "restart_pod", "increase_resources", "notify_only",
					"rollback_deployment", "expand_pvc", "drain_node", "quarantine_pod", "collect_diagnostics"},
	...
	for _, incident := range incidents {
		s.Run(fmt.Sprintf("SecurityIncident_%s", incident), func() {
			...
			if s.Assert().NoError(err, "Security incident should be analyzed") &&
				...
				// Security actions should be conservative (notify, quarantine, or restart for isolation)
				expectedActions := []string{"notify_only", "restart_pod", "quarantine_pod"}
				s.Assert().Contains(expectedActions, recommendation.Action,
	...
	for _, resource := range resources {
		s.Run(fmt.Sprintf("ResourceExhaustion_%s", resource), func() {
			...
			if s.Assert().NoError(err, "Resource exhaustion should be analyzed") &&
				...
				// Resource exhaustion should trigger appropriate scaling, restart, or storage actions
				expectedActions := []string{"restart_pod", "scale_deployment", "increase_resources", "notify_only", "expand_pvc"}
				s.Assert().Contains(expectedActions, recommendation.Action,
					"Resource exhaustion should recommend scaling, restart, resources, or storage expansion")
...
// TestEndToEndIntegration runs the complete end-to-end integration tests
func (s *OllamaIntegrationSuite) TestEndToEndIntegration() {
	s.T().Log("Running comprehensive end-to-end integration tests...")

	// Run all end-to-end test categories
	s.TestEndToEndActionExecution()
	s.TestEndToEndFailureScenarios()
	s.TestEndToEndPerformanceScenarios()
	s.TestEndToEndSecurityScenarios()

	s.T().Log("All end-to-end integration tests completed")
}
