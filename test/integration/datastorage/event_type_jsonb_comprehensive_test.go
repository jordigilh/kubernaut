...
var eventTypeCatalog = []eventTypeTestCase{
	...
	{
		Service:       "aianalysis",
		EventType:     "aianalysis.investigation.started",
	...
	{
		Service:       "aianalysis",
		EventType:     "aianalysis.investigation.completed",
		EventCategory: "investigation",
		EventAction:   "completed",
		SampleEventData: map[string]interface{}{
			"analysis_id":        "aa-001",
	...
	{
		Service:       "aianalysis",
		EventType:     "aianalysis.approval.required",
	...
	{
		Service:       "aianalysis",
		EventType:     "aianalysis.error.occurred",
		EventCategory: "error",
		EventAction:   "error_occurred",
		SampleEventData: map[string]interface{}{
...
var _ = Describe("GAP 1.1: Comprehensive Event Type + JSONB Validation", Label("gap-1.1", "p0"), func() {
	...
	Describe("ADR-034 Event Type Catalog Coverage", func() {
		It("should validate all 27 event types are documented", func() {
			...
			Expect(serviceCounts["aianalysis"]).To(Equal(5))
		...
		for _, tc := range eventTypeCatalog {
			...
			Context(fmt.Sprintf("Event Type: %s", tc.EventType), func() {
				...
				It("should accept event type via HTTP POST and persist to database", func() {
					// ARRANGE: Create audit event with service-specific event_data
	...
	Describe("GIN Index Usage for JSONB Queries", func() {
		It("should use idx_event_data_gin for JSONB queries (BR-STORAGE-027 performance)", func() {
			// ARRANGE: Execute EXPLAIN on JSONB query
			explainQuery := `
				EXPLAIN (FORMAT JSON)
			...
			// ASSERT: Query plan uses GIN index
			// Note: GIN index usage appears as "Bitmap Index Scan" or "Index Scan" in plan
			Expect(explainJSON).To(SatisfyAny(
				ContainSubstring("idx_event_data_gin"),
				ContainSubstring("Bitmap Index Scan"),
				ContainSubstring("Index Scan"),
			), "JSONB queries should use GIN index for performance")
