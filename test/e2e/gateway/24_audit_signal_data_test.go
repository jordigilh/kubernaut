...
var _ = Describe("BR-AUDIT-005: Gateway Signal Data for RR Reconstruction", func() {
	...
	Context("Gap #1-3: Complete Signal Data Capture", func() {
		...
		Eventually(func() int {
			...
			Expect(auditEvent.ActorType.Value).To(Equal("authenticated-service"), "Actor type (FedRAMP AU-3)")
			Expect(auditEvent.ActorID.Value).ToNot(BeEmpty(), "Actor ID - authenticated K8s identity")
