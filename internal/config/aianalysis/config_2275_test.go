...
import (
	...
	"github.com/jordigilh/kubernaut/internal/config/aianalysis"
)

// BR-PLATFORM-012, Issue #2275: extend the shared Debug.PprofEnabled toggle
// (secure-by-default, off) to AIAnalysis, a controller-runtime-managed
...
var _ = Describe("BR-PLATFORM-012: Debug.PprofEnabled secure default (Issue #2275)", func() {
	It("UT-AA-2275-001 DefaultConfig disables profiling by default (secure-by-default)", func() {
		cfg := aianalysis.DefaultConfig()
		Expect(cfg.Debug.PprofEnabled).To(BeFalse())
	})

	It("UT-AA-2275-002 YAML can opt in to profiling by setting debug.pprofEnabled: true", func() {
		cfg := aianalysis.DefaultConfig()
		data := []byte("debug:\n  pprofEnabled: true\n")
