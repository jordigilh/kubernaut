...
var _ = Describe("isEventPredicate (#1449)", func() {
	...
	// ═══════════════════════════════════════════════════════════════════════
	// Issue #2029 Part B / FedRAMP SI-4: KACorrelationID-only changes must
	// also wake the controller immediately, not just terminal transitions.
	// Without this, a takeover's correlation write (which only changes
	// KACorrelationID, not Phase) is silently dropped and AA only learns
	// about it on its next scheduled poll (up to sessionPollInterval later)
	...
	Context("#2029 SI-4: KACorrelationID changes trigger reconciliation", func() {
		It("UT-AA-2029-013: passes Update events when only KACorrelationID changes (Phase stays non-terminal)", func() {
			pred := controller.ISEventPredicate()
			...
			Expect(pred.Update(updateEvent)).To(BeTrue(),
				"#2029 Part B: a KACorrelationID change must pass the predicate even when Phase doesn't change")
		})

		It("UT-AA-2029-013b: drops Update events when KACorrelationID and Phase are both unchanged (no regression)", func() {
			pred := controller.ISEventPredicate()
		...
		It("UT-AA-2029-013c: existing terminal-phase cases still pass when KACorrelationID is held at its zero value (no regression)", func() {
			pred := controller.ISEventPredicate()
			oldIS := makeIS(isv1alpha1.SessionPhaseActive)
			...
			Expect(pred.Update(updateEvent)).To(BeTrue(),
				"UT-AA-1449-010's terminal-phase case must be unaffected by the #2029 widening")
		})
	})
