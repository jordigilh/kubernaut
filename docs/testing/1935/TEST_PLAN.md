...
**Follow-up filed with the console team**: `kubernaut-console` should expose a user-facing setting to disable raw-thinking display in the ThinkingPanel (enabled by default), since Anthropic's own guidance treats extended-thinking text as exploratory/unfiltered reasoning rather than a polished user-facing statement. Out of scope for this repo's fix — tracked as [jordigilh/kubernaut-console#TBD](https://github.com/jordigilh/kubernaut-console/issues).
