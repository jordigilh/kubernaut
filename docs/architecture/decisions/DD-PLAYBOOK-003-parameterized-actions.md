...
**Updated**: 2025-11-15  
**Target Release**: v1.1  
**Related**: BR-PLAYBOOK-001, DD-PLAYBOOK-001
...
**Rationale**: Controllers handle audit trail, not playbooks. Playbook parameters should only include what's required for execution.
...
**User Clarification**:
> "Our playbooks are container images, they run in a Tekton pipeline. The image can contain any runnable process, such as an ansible playbook or a shell script. It's up to the operators to implement and define these vetted playbooks."
...
**Key Insight**: The playbook container image needs parameters to know:
...
### 1. Playbook Schema (Defines Expected Parameters)
...
The LLM selects a specific remediation playbook and populates its required parameters:
...
**Key Points**:
- `playbook_id`: Specific remediation playbook (e.g., "oomkill-scale-down")
- `parameters`: Flat parameter object (no conditional logic)
- LLM does NOT see internal playbook steps
- Each playbook has simple, clear parameter requirements


### 2. Operator Flexibility ✅
- Operators define playbook containers (Ansible, Shell, Python, Go, etc.)
...
│  3. Gets playbook recommendations from MCP                   │
│  4. LLM populates parameters based on investigation          │
└───────────────────────┬─────────────────────────────────────┘
                        │
...
┌─────────────────────────────────────────────────────────────┐
│                    Tekton Pipeline                           │
│  1. Validates parameters against playbook schema             │
│  2. Pulls playbook container image                           │
...
└───────────────────────┬─────────────────────────────────────┘
                        ...
                        ▼
┌─────────────────────────────────────────────────────────────┐
│              Playbook Container (Operator-Defined)           │
...
1. Update DD-PLAYBOOK-001 with parameter schema
2. Add `parameters` field to playbook catalog schema
...
4. Create example playbook containers (Ansible + Shell)
...
## Example Playbook Catalog Entry (Complete)
...
│  (PipelineRun)  │  • Runs playbook container images
│                 │  • Injects parameters as env vars
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ Playbook Image  │  Container with remediation logic
...
**Principle**: Each playbook implements ONE remediation strategy. The playbook container may have multiple internal steps/actions, but these are implementation details hidden from the LLM.
...
**Note**: `internal_steps` are implementation details NOT exposed to LLM. The playbook container handles these internally.
...
**Key Points**:
- `playbook-id`: Specific remediation playbook (e.g., "oomkill-scale-down")
...
### 4. Playbook Container Implementation (Operator-Defined)

Operators implement playbook containers with internal remediation logic. The LLM does NOT see these implementation details.
...
**APPROVED**: Each playbook implements ONE remediation strategy with a flat parameter schema.
...
**Accepted**:
- More playbook entries in catalog (3 vs 1 for OOMKill scenarios)
...
- Single playbook with REMEDIATION_ACTION parameter
...
- Enhance playbook descriptions with use-case guidance ✅
- Update DD-PLAYBOOK-003 to reflect single remediation pattern ✅

**v1.1 (Short-term)**:
- Implement catalog tagging system in Data Storage Service
- Add category/incident_type fields to playbook schema
- Update semantic search to support tag filtering
- CRD-based registration via RemediationPlaybook CRD

**v1.2 (Medium-term)**:
- Conduct operator user testing
- Iterate on catalog UX based on feedback
- Refine playbook naming conventions
...
### Playbook Registration: Three Separate Playbooks, One Image
...
3. **Simple LLM Interface** (HIGH VALUE)
   ...
   - Maintains single remediation playbook pattern from LLM perspective
...
The Playbook Registry Controller will validate:
...
```go
// Playbook Registry Controller validation
