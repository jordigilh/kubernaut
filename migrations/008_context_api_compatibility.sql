...
-- - embedding: 384-dimensional vector for semantic similarity search
--
-- These fields enable Context API to provide LLM with rich filtering capabilities
-- and semantic search without requiring complex JSONB extraction from alert_labels.
...
-- Add embedding vector for semantic similarity search
-- Supports Context API's semantic search capabilities
ALTER TABLE resource_action_traces
...
COMMENT ON COLUMN resource_action_traces.embedding IS
'Vector embedding (384 dimensions) for semantic similarity search using pgvector extension';
