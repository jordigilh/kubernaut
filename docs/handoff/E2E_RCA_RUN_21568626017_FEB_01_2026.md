...
- ❌ **HolmesGPT-API (HAPI)**: 401 Unauthorized - Missing Authorization header (27/31 tests failed)
- ❌ **Notification**: Pod readiness timeout - controller pod failed to become ready (BeforeSuite failure)
- ❌ **WorkflowExecution**: tkn CLI `--override` flag error (BeforeSuite failure)

**6 services PASSED**:
- ✅ AIAnalysis, DataStorage, Gateway, AuthWebhook, RemediationOrchestrator, SignalProcessing

**Must-Gather Availability**: ❌ NOT available for any failed service
...
- ✅ 4 tests PASSED (skipped tests marked as SKIPPED)
- ❌ 27 tests FAILED with identical 401 errors

### Evidence from CI Logs

#### HAPI Deployment (Successful)
```
🚀 Deploying HolmesGPT-API (NodePort 30120)...
...
⏳ Waiting for HAPI pod ready...
   ✅ HAPI pod became ready
```

#### ServiceAccount RBAC (Created)
```
🔐 Creating HolmesGPT-API ServiceAccount + RBAC...
...
**Evidence Chain**:
1. ✅ ServiceAccount `holmesgpt-api-sa` created with proper RBAC
2. ✅ Deployment includes `serviceAccountName: holmesgpt-api-sa` (added in commit `001721b93`)
3. ✅ Pod became ready (health check passed)
4. ❌ **All HAPI requests to DataStorage receive 401 Unauthorized**
...
```
🔐 Creating Notification Controller ServiceAccount + RBAC...
...
🔐 Adding DataStorage access for notification-controller SA (DD-AUTH-014)...
✅ RoleBinding created: notification-controller-datastorage-access
```
...
#### Pod Status (FAILED)
```
⏳ Waiting for controller pod to be created...
...
readinessProbe:
  ...
  periodSeconds: 15
  timeoutSeconds: 5
  failureThreshold: 6  # ← Increased from 3 in commit 072e9432f
```

**Maximum Wait Time**: 15s (initial) + (6 × 15s) = **105 seconds**

**Historical Context**:
- **Local execution**: Pod becomes ready in ~38 seconds ✅ (documented in previous RCA)
- **CI execution**: Consistently fails after 105 seconds timeout ❌
...
- Network latency for inter-service communication (Kind NodePort → DS pod)
...
```yaml
failureThreshold: 10  # 15s + (10 × 15s) = 165 seconds
```
...
startupProbe:
  ...
  periodSeconds: 10
  failureThreshold: 18  # 10s + (18 × 10s) = 190 seconds for startup
readinessProbe:
  httpGet:
    path: /readyz
    port: 8081
...
**Evidence**:
1. ✅ tkn CLI installed (version 0.40.0, upgraded in commit `4cae2b862`)
2. ❌ `--override` flag does not exist in tkn v0.39.0, v0.40.0, or any other version
3. ⚠️  **WRONG FILE FIXED** in commit `9d2ddb9e4` (fixed `test/infrastructure/tekton_bundles.go`, but WE E2E doesn't use it!)

**Critical Discovery**: WorkflowExecution E2E uses **`workflow_bundles.go`**, NOT `tekton_bundles.go`

**Code Path Investigation**:
```bash
# WorkflowExecution E2E calls:
test/e2e/workflowexecution/workflowexecution_e2e_suite_test.go
  → infra.BuildAndRegisterTestWorkflows()  # line 132

# Which is defined in:
test/infrastructure/workflow_bundles.go:69
  → buildWorkflowBundle()  # line 192
    → exec.Command("tkn", "bundle", "push", bundleRef, "-f", pipelineYAML, "--override")  # line 211 ❌
```

**Actual Bug Location** (`test/infrastructure/workflow_bundles.go:211`):
```go
buildCmd := exec.Command("tkn", "bundle", "push", bundleRef,
    "-f", pipelineYAML,
    "--override", // ← STILL HERE! This is the ACTUAL code path used by WE E2E
)
```

**Why We Missed It**:
- Commit `9d2ddb9e4` fixed `tekton_bundles.go` ✅
- But `tekton_bundles.go` is **NOT USED** by any E2E tests ❌
- The actual file used is `workflow_bundles.go` which was **NEVER FIXED** ❌
...
**Removed `--override` flag from the ACTUAL file used by WE E2E**:

```go
// test/infrastructure/workflow_bundles.go:209
buildCmd := exec.Command("tkn", "bundle", "push", bundleRef,
    "-f", pipelineYAML,
...
**Verification**:
```bash
# Confirm no --override flags remain in actual code
grep -r "\-\-override" test/infrastructure/ test/e2e/ --include="*.go"
# Result: Only comments, no actual usage ✅
```

**Expected Outcome**: WorkflowExecution E2E BeforeSuite will complete successfully, Tekton bundles will be built and loaded into Kind cluster.

### References
- **WRONG Fix Commit**: `9d2ddb9e4` (fixed unused file `tekton_bundles.go`)
- **CORRECT Fix Commit**: `6196a14ee` (fixed actual file `workflow_bundles.go`)
- **Upgrade Commit**: `4cae2b862` (upgraded tkn to v0.40.0)
- **Code Location**: `test/infrastructure/workflow_bundles.go:211`
...
```
⚠️  No must-gather directory found at /tmp/kubernaut-must-gather
...
**Impact**:
- ✅ Must-gather artifacts will now be created for **all** E2E failures
- ✅ BeforeSuite failures will have Kind cluster logs available
- ✅ RCA will be possible for infrastructure setup issues
...
### 3. WorkflowExecution E2E (FIXED - Wrong File)
**Priority**: P0 → ✅ RESOLVED  
**Action**: ✅ Fixed the ACTUAL file used by WE E2E (commit `6196a14ee`)
**Root Cause**: 
- Commit `9d2ddb9e4` fixed `tekton_bundles.go` (NOT USED)
- WE E2E actually uses `workflow_bundles.go` (which was NEVER FIXED)
- Removed `--override` flag from `workflow_bundles.go:211`

**Expected Outcome**: Tekton bundles build successfully, WE E2E BeforeSuite completes

### 4. CI Infrastructure (COMPLETED)
**Priority**: P2  
**Action**: ✅ Ensure must-gather logs always available
**Steps**:
1. ✅ Commit `322ab9cd1` pushed (manual Kind export on BeforeSuite failure)
...
| Service | Root Cause Confidence | Fix Confidence | Status |
|---------|---------------------|----------------|--------|
| **WorkflowExecution** | 100% (wrong file fixed) | 100% (✅ FIXED) | ✅ commit `6196a14ee` |
| **Notification** | 80% (DS client init slow in CI) | 70% (need startup probe + DS health fix) | ⏳ Needs DS health endpoint fix |
| **HAPI** | 75% (token not being sent) | 50% (Python code issue) | ⏳ Need must-gather logs |

**Overall Confidence**: 85% (WE fixed, NT/HAPI need investigation)

**Must-Gather Status**: 
- ✅ CI now exports logs on BeforeSuite failures (commit `322ab9cd1`)
- Next CI run will provide NT and HAPI pod logs for investigation
