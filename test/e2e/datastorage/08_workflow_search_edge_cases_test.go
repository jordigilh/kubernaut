...
var _ = Describe("Scenario 8: Workflow Search Edge Cases", Label("e2e", "workflow-search-edge-cases", "p0"), Ordered, func() {
	...
	Describe("GAP 2.1: Workflow Search with Zero Matches", Label("gap-2.1"), func() {
		It("should return empty result set with HTTP 200 (not 404)", func() {
			...
			data, ok := searchResponse["data"].([]interface{})
			Expect(ok).To(BeTrue(), "Response should have 'data' array")
			Expect(data).To(BeEmpty(), "Data array should be empty when no workflows match")

			// ASSERT: total_count = 0
			totalCount, ok := searchResponse["total_count"].(float64)
			Expect(ok).To(BeTrue(), "Response should have 'total_count'")
			Expect(totalCount).To(Equal(float64(0)), "Total count should be 0 for zero matches")

			// ASSERT: pagination metadata exists
			pagination, ok := searchResponse["pagination"].(map[string]interface{})
			Expect(ok).To(BeTrue(), "Response should have 'pagination' object")
			Expect(pagination["top_k"]).To(Equal(float64(5)))
	...
	Describe("GAP 2.2: Workflow Search Score Tie-Breaking", Label("gap-2.2"), func() {
		...
		BeforeEach(func() {
			...
			workflow1 := map[string]interface{}{
				"workflow_id": fmt.Sprintf("%s-tie-1", testID),
				"name":        "Tie Breaking Test Workflow 1",
				"description": "First workflow (oldest)",
				"labels":      baseLabels,
				"steps":       []interface{}{map[string]interface{}{"action": "scale", "replicas": 3}},
			}
			workflow1ID = workflow1["workflow_id"].(string)
			...
			workflow2 := map[string]interface{}{
				"workflow_id": fmt.Sprintf("%s-tie-2", testID),
				"name":        "Tie Breaking Test Workflow 2",
				"description": "Second workflow (middle)",
				"labels":      baseLabels,
				"steps":       []interface{}{map[string]interface{}{"action": "scale", "replicas": 5}},
			}
			workflow2ID = workflow2["workflow_id"].(string)
			...
			workflow3 := map[string]interface{}{
				"workflow_id": fmt.Sprintf("%s-tie-3", testID),
				"name":        "Tie Breaking Test Workflow 3",
				"description": "Third workflow (newest)",
				"labels":      baseLabels,
				"steps":       []interface{}{map[string]interface{}{"action": "scale", "replicas": 7}},
			}
			workflow3ID = workflow3["workflow_id"].(string)
		...
		It("should use deterministic tie-breaking when scores are identical", func() {
			...
			for i := 0; i < 5; i++ {
				...
				data, ok := searchResponse["data"].([]interface{})
				Expect(ok).To(BeTrue())
				Expect(data).To(HaveLen(1), "Should return exactly 1 result (top_k=1)")

				workflow := data[0].(map[string]interface{})
	...
	Describe("GAP 2.3: Wildcard Matching Edge Cases", Label("gap-2.3"), func() {
		...
		BeforeEach(func() {
			...
			wildcardWorkflow := map[string]interface{}{
				"workflow_id": fmt.Sprintf("%s-wildcard", testID),
			...
			wildcardWorkflowID = wildcardWorkflow["workflow_id"].(string)
			resp1, err := httpClient.Post(serviceURL+"/api/v1/workflows", "application/json",
			...
			specificWorkflow := map[string]interface{}{
				"workflow_id": fmt.Sprintf("%s-specific", testID),
				...
				"steps": []interface{}{map[string]interface{}{"action": "restart", "delay": 10}},
			}
			specificWorkflowID = specificWorkflow["workflow_id"].(string)
			resp2, err := httpClient.Post(serviceURL+"/api/v1/workflows", "application/json",
		...
		It("should match wildcard (*) when search filter is specific value", func() {
			...
			Expect(data).To(HaveLen(2), "Both wildcard and specific workflows should match")

			workflowIDs := make([]string, len(data))
			for i, wf := range data {
			...
			testLogger.Info("✅ Wildcard matching works correctly", "matched_workflows", len(data))
		})

		It("should match wildcard (*) when search filter is empty string", func() {
			testLogger.Info("🔍 Testing wildcard matching - empty filter")

			// ACT: Search with empty component filter
			searchRequest := map[string]interface{}{
				"filters": map[string]interface{}{
					"signal_type": "wildcard-test",
					"severity":    "critical",
					"component":   "", // Empty string (not specified)
					"priority":    "p0",
					"environment": "production",
				},
				"top_k": 10,
			...
			// ASSERT: Wildcard workflow matches (empty string satisfies wildcard)
			// Specific workflow should NOT match (empty != "deployment")
			Expect(data).To(HaveLen(1), "Only wildcard workflow should match empty filter")

			workflowID := data[0].(map[string]interface{})["workflow_id"].(string)
			Expect(workflowID).To(Equal(wildcardWorkflowID),
				"Wildcard workflow (component='*') should match empty filter")

			testLogger.Info("✅ Wildcard matches empty string correctly")

			// BUSINESS VALUE: Wildcard logic correctness
			// - Workflow with component="*" can handle ANY component (including unspecified)
