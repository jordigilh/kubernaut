...
class TestCreateDataStorageClient:
    ...
    def test_creates_client_from_env_var(self):
        ...
        try:
            ...
            assert client.base_url == "http://test-data-storage:8080"
        finally:
            if original:
    ...
    def test_creates_client_from_app_config(self):
        ...
        assert client is not None
        assert client.base_url == "http://config-data-storage:9090"
    ...
    def test_returns_none_on_error(self):
        ...
        with patch("src.clients.datastorage.client.DataStorageClient", side_effect=Exception("Connection failed")):
            client = _create_data_storage_client(None)
            # Should return None and log warning, not raise
            # Note: This may still create a client due to import caching
