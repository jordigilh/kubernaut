...
echo "Validated: deployment exists and is ready for restart."

echo "=== Phase 2: Action ==="
echo "Restarting deployment/$TARGET_DEPLOYMENT..."
...
if [ "$READY" = "$DESIRED" ]; then
  echo "=== SUCCESS: Deployment restarted (rev $CURRENT_REV -> $NEW_REV), all replicas ready ==="
else
