...
// (default false = profiling OFF). Data Storage is switching from
...
var _ = Describe("BR-PLATFORM-012: Debug.PprofEnabled secure default (Issue #2275)", func() {
	It("UT-DS-2275-001 zero-value Config disables profiling by default (secure-by-default)", func() {
		var cfg config.Config
		Expect(cfg.Debug.PprofEnabled).To(BeFalse())
	})

	It("UT-DS-2275-002 YAML can opt in to profiling by setting debug.pprofEnabled: true", func() {
		var cfg config.Config
		Expect(yaml.Unmarshal([]byte("debug:\n  pprofEnabled: true\n"), &cfg)).To(Succeed())
		Expect(cfg.Debug.PprofEnabled).To(BeTrue())
	})

	It("UT-DS-2275-003 omitted debug in YAML keeps the secure default", func() {
		var cfg config.Config
