# Data Storage Service - Implementation Plan V4.7

**Version**: 4.7 - Critical Decisions & Discovery-First Approach  
**Date**: 2025-11-02  
**Timeline**: 12.5 days (100 hours) ← **Includes 2.5 days Phase 0 discovery for 100% confidence**  
**Status**: ✅ Ready for Implementation (Phase 0 Discovery → 100% Confidence → Implementation)  
**Based On**: V4.6 + Critical decision resolution + 8 confidence gap analysis
...
## 🎯 **UPDATED TIMELINE** (12.5 days, 100 hours) ✅ **V4.7 - INCLUDES PHASE 0 DISCOVERY**

**Phase 0: Pre-Implementation Discovery** (2.5 days, 19.5 hours) ← **NEW in V4.7**

| Day | Focus | Hours | Key Deliverables | Confidence Impact |
|-----|-------|-------|------------------|-------------------|
| **Day 0.1** | P0 Gap Resolution | 8h | Notification schema, DB migrations, Error recovery ADR | 90% → 95% (+5%) |
| **Day 0.2** | P1 Gap Resolution | 6.5h | pgvector requirements, Performance SLA, Effectiveness schema | 95% → 98% (+3%) |
| **Day 0.3** | P2 Gap Resolution + Review | 5h | Service integration checklist, Auth decision doc, Final validation | 98% → 100% (+2%) |

**Phase 1-3: TDD Implementation** (10 days, 80 hours)

| Day | Focus | Hours | Key Deliverables | Changes from V4.3 |
|-----|-------|-------|------------------|-------------------|
| **Day 1** | Models + Interfaces | 8h | Data models, business interfaces | No change |
| **Day 2** | Schema | 8h | PostgreSQL schema + pgvector extension | No change |
| **Day 3** | Validation Layer | 8h | Input validation, RFC 7807 errors ✅ | **+3.5h (RFC 7807 + naming)** |
| **Day 4** | Embedding Generation | 4h | Embedding pipeline for AIAnalysis only ✅ | **-4h (Decision 1a: AIAnalysis only)** |
| **Day 5** | pgvector Storage + DLQ | 6h | Single-transaction writes + DLQ fallback ✅ | **-2h (no dual-write, +2h DLQ)** |
| **Day 6** | Query API | 8h | REST endpoints, filtering, pagination | No change |
| **Day 7** | Integration Tests | 11h | Podman setup, behavior+correctness, DLQ scenarios ✅ | **+4h (Podman + principles + DLQ tests)** |
| **Day 8** | E2E Tests | 8h | Complete workflows, detailed scenarios ✅ | **+0.5h (E2E details)** |
| **Day 9** | Unit Tests + BR Matrix | 8h | Comprehensive unit tests | No change |
| **Day 10** | Metrics + Logging | 8h | Prometheus metrics (audit-specific) ✅ | **+0.5h (audit metrics)** |
| **Day 11** | Production Readiness | 9h | OpenAPI spec, Config, Shutdown ✅ | **+8h (OpenAPI + Config + Shutdown)** |
| **Day 12** | REMOVED | | | **-8h (merged into Day 11)** |

**Phase 0-3 Total**: 12.5 days (105.5 hours) = 19.5h discovery + 86h implementation  
**Comparison**: V4.3 (96h) vs. V4.7 (105.5h) = **+9.5h investment for 100% confidence** ✅  
**Net Value**: +2.5 days upfront prevents 2+ days rework (proven by Context API lessons)
