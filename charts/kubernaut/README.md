...
| `global.llmProfiles.<name>.reasoning.effort` | Unified, provider-agnostic reasoning-depth knob (#1604): `""` (vendor/provider default), `none`, `minimal`, `low`, `medium`, `high`, or `xhigh`. Supported on Anthropic, Kubernaut Agent's Gemini-family client (BR-AI-087), real OpenAI/gpt-5/o-series models, and DeepSeek. Both Anthropic and Gemini have no tier above `high`, so `xhigh` is accepted but clamped down to `high` on those two providers (not an error) | `""` |
