...
// Orchestrates the complete remediation lifecycle by managing RemediationRequest CRDs
// and creating child CRDs (RemediationProcessing, AIAnalysis, WorkflowExecution, KubernetesExecution)
//
// Service: Remediation Orchestrator (05-remediationorchestrator)
...
import (
	...
	remediationcontroller "github.com/jordigilh/kubernaut/internal/controller/remediation"
...
func init() {
	...
	// Register RemediationRequest CRD (owned by this controller)
	utilruntime.Must(remediationv1alpha1.AddToScheme(scheme))

	// Register child CRDs (created by this controller)
...
func main() {
	...
	flag.StringVar(&metricsAddr, "metrics-bind-address", "0.0.0.0:9090", "The address the metric endpoint binds to.")
	flag.StringVar(&probeAddr, "health-probe-bind-address", "0.0.0.0:8080", "The address the probe endpoint binds to.")
	...
	// Register RemediationRequest controller
	if err = (&remediationcontroller.RemediationRequestReconciler{
