...
// UT-KA-2075-002: complete_no_action.Handle's tombstone fallback (#2076,
// v1.6 clone of #2075) -- isolated from the real no_matching_workflows race
// (IT-KA-2075-001 proves the two production call sites are actually wired
// together).
var _ = Describe("kubernaut_complete_no_action tool — #2076 (v1.6 clone of #2075) tombstone fallback", func() {
	It("UT-KA-2075-002a (AC-6, AU-3): returns already_resolved instead of erroring on a tombstone hit", func() {
		...
		Expect(err).NotTo(HaveOccurred(), "#2075/#2076: a tombstone hit must resolve the race, not error")
