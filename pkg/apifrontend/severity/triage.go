...
// the target resource (DD-AF-012, #2027). Unlike ErrSeverityUndetermined
...
func (t *Triager) Triage(ctx context.Context, input TriageInput) (TriageResult, error) {
	...
	// DD-AF-012/#2027: a cluster-scoped-only match is a guess, not a fact --
	// fail closed unless the caller already carries the user's confirmation
	// for this exact candidate (a different candidate does not bypass the
	// gate, even on a later call).
