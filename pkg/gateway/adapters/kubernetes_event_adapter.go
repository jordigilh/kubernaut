...
// severityUnknown is the fallback severity label when a signal source does not
// report one (shared across gateway adapters).
...
func (a *KubernetesEventAdapter) Parse(ctx context.Context, rawData []byte) (*types.NormalizedSignal, error) {
	...
	severity := event.Type
	if severity == "" {
		severity = severityUnknown // Only default if missing entirely
	}
