...
class TestDataStorageClientGetWorkflowByUUID:
    ...
    def mock_http_client(self):
        """Mock HTTP client for Data Storage requests."""
        return Mock()

    @patch('src.clients.datastorage.client.requests')
    def test_get_workflow_by_uuid_returns_workflow_when_exists(
        self, mock_requests, mock_http_client, mock_response_workflow
    ):
        """
        BR-AI-023: Workflow existence validation - workflow found.

        Given: Data Storage has workflow with UUID 'restart-pod-v1'
        When: get_workflow_by_uuid('restart-pod-v1') is called
        Then: Returns RemediationWorkflow object with correct data
        """
        # Arrange
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = mock_response_workflow
        ...
        # Act
        workflow = client.get_workflow_by_uuid("restart-pod-v1")

        # Assert
        assert workflow is not None
        assert workflow.workflow_id == "restart-pod-v1"
        assert workflow.container_image == "ghcr.io/kubernaut/restart-pod:v1.0.0"
        assert workflow.container_digest == "sha256:def456"
        mock_requests.get.assert_called_once()
        call_args = str(mock_requests.get.call_args)
        assert "/api/v1/workflows/restart-pod-v1" in call_args

    @patch('src.clients.datastorage.client.requests')
    def test_get_workflow_by_uuid_returns_none_when_not_found(
        self, mock_requests, mock_http_client
    ):
        """
        BR-AI-023: Hallucination detection - workflow not found.

        Given: Data Storage does NOT have workflow with UUID 'hallucinated-workflow'
        When: get_workflow_by_uuid('hallucinated-workflow') is called
        ...
        # Act
        workflow = client.get_workflow_by_uuid("hallucinated-workflow")

        # Assert
        assert workflow is None

    @patch('src.clients.datastorage.client.requests')
    def test_get_workflow_by_uuid_includes_parameter_schema(
        self, mock_requests, mock_http_client, mock_response_workflow
    ):
        """
        BR-HAPI-191: Parameter schema available for validation.

        Given: Workflow has parameter schema in Data Storage
        When: get_workflow_by_uuid() is called
        Then: Returned workflow includes parsed parameter schema
        """
        # Arrange
        mock_response = Mock()
        mock_response.status_code = 200
        ...
        # Parameter schema should be accessible
