...
class ValidationResult(BaseModel):
    ...
    """ # noqa: E501
    is_valid: StrictBool = Field(description="Whether the RR passes validation (no blocking errors). true = ready to apply, false = missing required fields. ")
    completeness: Annotated[int, Field(le=100, strict=True, ge=0)] = Field(description="Completeness percentage (0-100%) based on field presence. 100% = all fields present (Gaps #1-8 complete). 50-99% = required fields + some optional fields. 0-49% = only required fields or incomplete. ")
    errors: List[StrictStr] = Field(description="Blocking validation errors (missing required fields). Empty array if is_valid = true. ")
    warnings: List[StrictStr] = Field(description="Non-blocking warnings for missing optional fields. Helps identify gaps in reconstruction. ")
    __properties: ClassVar[List[str]] = ["is_valid", "completeness", "errors", "warnings"]
