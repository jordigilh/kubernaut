...
// suite for GitHub issue #2029 Part A: reconcileInvestigating and
// reconcileAnalyzing must retry (bounded, with backoff) instead of
// fail-closing forever when a live cluster's installed AIAnalysis CRD
// rejects a Status().Update() call with apierrors.IsInvalid (e.g. CRD
...
// not just the fake client's approximation of them (see UT-AA-2029-001/002/
...
func TestSchemaRejectionRetryIntegration(t *testing.T) {
	...
	RunSpecs(t, "AIAnalysis Schema-Rejection Retry Integration Suite (#2029 Part A)")
}
