...
var _ = Describe("Degraded Mode", func() {
	...
	Describe("BuildDegradedContext", func() {
		...
		It("should create context from signal labels", func() {
			...
			Expect(ctx.NamespaceLabels).To(HaveKeyWithValue("app", "my-app"))
			Expect(ctx.NamespaceLabels).To(HaveKeyWithValue("environment", "production"))
			Expect(ctx.NamespaceLabels).To(HaveKeyWithValue("team", "platform"))
		})
		...
		It("should set Confidence to 0.5", func() {
			signal := &signalprocessingv1alpha1.SignalData{
				Name:   "test-signal",
				Labels: map[string]string{"key": "value"},
			}
			...
			Expect(ctx.Confidence).To(Equal(0.5))
		})
		...
		It("should handle nil labels gracefully", func() {
			...
			Expect(ctx).NotTo(BeNil())
			Expect(ctx.NamespaceLabels).NotTo(BeNil()) // Should be empty map, not nil
			Expect(ctx.DegradedMode).To(BeTrue())
			Expect(ctx.Confidence).To(Equal(0.5))
		})
		...
		It("should handle empty labels", func() {
			...
			Expect(ctx).NotTo(BeNil())
			Expect(ctx.NamespaceLabels).To(BeEmpty())
			Expect(ctx.DegradedMode).To(BeTrue())
		})
		...
		It("should copy signal annotations if present", func() {
			...
			Expect(ctx.NamespaceAnnotations).To(HaveKeyWithValue("description", "test annotation"))
		})
	})
	...
	Describe("Context Size Validation", func() {
		...
		It("should reject context with label value too long", func() {
			...
			ctx := &signalprocessingv1alpha1.KubernetesContext{
				NamespaceLabels: map[string]string{"key": string(longValue)},
			}
