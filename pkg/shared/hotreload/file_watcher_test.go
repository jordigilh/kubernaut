...
var _ = Describe("FileWatcher", func() {
	...
	Context("NewFileWatcher", func() {
		It("should create watcher with valid parameters", func() {
			...
			watcher, err := NewFileWatcher(testFile, func(string) error { return nil }, logger)

			Expect(err).NotTo(HaveOccurred())
			Expect(watcher).NotTo(BeNil())
			Expect(watcher.path).To(Equal(testFile))
		})

		It("should return error for empty path", func() {
			_, err := NewFileWatcher("", func(string) error { return nil }, logger)
		...
		It("should return error for nil callback", func() {
			_, err := NewFileWatcher("/some/path", nil, logger)
	...
	Context("Start", func() {
		It("should load initial content and call callback", func() {
			...
			watcher, err := NewFileWatcher(testFile, func(content string) error {
		...
		It("should return error if file does not exist", func() {
			watcher, err := NewFileWatcher("/nonexistent/file.txt", func(string) error { return nil }, logger)
			Expect(err).NotTo(HaveOccurred())
		...
		It("should return error if callback fails on initial load", func() {
			...
			watcher, err := NewFileWatcher(testFile, func(string) error {
