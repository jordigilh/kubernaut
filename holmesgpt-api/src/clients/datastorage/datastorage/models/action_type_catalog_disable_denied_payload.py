...
class ActionTypeCatalogDisableDeniedPayload(BaseModel):
    """
    DS audit payload when disable is denied due to active workflow dependencies
    """ # noqa: E501
    event_type: StrictStr
    action_type: StrictStr
    denied_reason: StrictStr
    dependent_workflow_count: StrictInt
    dependent_workflows: List[StrictStr]
    requested_by: StrictStr
    __properties: ClassVar[List[str]] = ["event_type", "action_type", "denied_reason", "dependent_workflow_count", "dependent_workflows", "requested_by"]

    @field_validator('event_type')
    def event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('datastorage.actiontype.disable_denied'):
            raise ValueError("must be one of enum values ('datastorage.actiontype.disable_denied')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "dependent_workflow_count": obj.get("dependent_workflow_count"),
            "dependent_workflows": obj.get("dependent_workflows"),
