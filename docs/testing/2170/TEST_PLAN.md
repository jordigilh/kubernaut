...
| Retire `RecordAIAgentSessionLost`/`EventTypeAIAgentSessionLost` from production call sites but leave the method/interface in place for now | `handleSessionLost` (its sole caller) is deleted; the audit method has zero production callers today (test doubles only implement the interface). Full deletion of the dead method is flagged as a candidate for #2190's cleanup pass rather than done silently in a docs-focused pass |
