...
var _ = Describe("Security and Compliance Validation", Ordered, func() {
	...
	BeforeAll(func() {
		...
		stateManager = shared.NewIsolatedTestSuiteV2("Security Validation").
			WithLogger(logger).
	...
	Context("Data Protection and Privacy", func() {
		It("should detect and handle sensitive data in embeddings", func() {
			...
			sensitivePatterns := []*vector.ActionPattern{
				createSecurityTestPattern(embeddingService, ctx, "pii-email", "User email: john.doe@example.com in alert"),
			...
			for _, pattern := range sensitivePatterns {
				...
				if containsPII {
					...
					Expect(anonymizedPattern.AlertName).ToNot(ContainSubstring("john.doe@example.com"))
...
func processGDPRExport(vectorDB vector.VectorDatabase, request GDPRRequest, ctx context.Context) ([]ExportData, bool) {
	...
	mockData := []ExportData{
		{
			PatternID:  "export-pattern-1",
			ActionType: "scale_deployment",
			AlertName:  "Test Alert 1",
			CreatedAt:  time.Now(),
			Metadata:   map[string]interface{}{"user_id": request.UserID},
		},
	}
