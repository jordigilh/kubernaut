...
Persist session state as a Kubernetes Custom Resource (InvestigationSession CRD) at `kubernaut.ai/v1alpha1` (originally `apifrontend.kubernaut.ai/v1alpha1`, migrated per ADR-001 supersession). AF is fully stateless between requests — the CRD is the only persistent state.
