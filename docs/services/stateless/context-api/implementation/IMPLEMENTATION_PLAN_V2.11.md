...
**Triage Reports**: [CONTEXT_API_V2_TRIAGE_VS_TEMPLATE.md](CONTEXT_API_V2_TRIAGE_VS_TEMPLATE.md), [CONTEXT_VS_NOTIFICATION_GAP_ANALYSIS.md](CONTEXT_VS_NOTIFICATION_GAP_ANALYSIS.md), [GAP_REMEDIATION_COMPLETE.md](GAP_REMEDIATION_COMPLETE.md), [CONTEXT_API_STANDARDS_COMPLIANCE_REVIEW.md](CONTEXT_API_STANDARDS_COMPLIANCE_REVIEW.md), [CONTEXT_API_ADR033_IMPACT_ANALYSIS.md](../../../../CONTEXT_API_ADR033_IMPACT_ANALYSIS.md)

---

## 🎯 **ADR-033 AGGREGATION SERVICE** (Days 10-12)

**✅ APPROVED FOR IMPLEMENTATION**

**Decision**: [ADR-033: Remediation Playbook Catalog](../../../../architecture/decisions/ADR-033-remediation-playbook-catalog.md)

**What**: Context API becomes the **Aggregation Layer** between AI/LLM Service and Data Storage Service

**Architecture**:
```
AI/LLM Service (Playbook Selection)
       ↓
Context API (Aggregation + Caching) ← YOU ARE HERE
       ↓
Data Storage Service (Success Rate APIs)
       ↓
PostgreSQL (resource_action_traces)
```

**Key Changes**:
- **AggregationService**: New service calling Data Storage REST API endpoints
- **Data Storage HTTP Client**: REST client for Data Storage Service (replaces direct PostgreSQL)
- **Caching**: Redis L1 + LRU L2 for aggregation results (configurable TTL, default 5min)
- **HTTP Endpoints**: 3 new endpoints for AI/LLM Service integration
- **Full API Gateway Migration**: ALL queries via Data Storage Service (ADR-032 compliance)
...
**Timeline**: 24 hours (Days 10-12)

**Status**: ⏳ **PENDING** - Data Storage Service ADR-033 endpoints complete ✅
...
- [Context API ADR-033 Impact Analysis](../../../../CONTEXT_API_ADR033_IMPACT_ANALYSIS.md)
...
**Purpose**: Rewrite Days 10-12 to implement ADR-033 Aggregation Service and complete full API Gateway migration

**Changes**:
- 🎯 **Days 10-12 Completely Rewritten** (24 hours)
  - **OLD**: Unit tests, E2E tests, documentation (OBSOLETE - no aggregation features to test)
  - **NEW**: AggregationService implementation + Data Storage HTTP client + Full API Gateway migration
- ✅ **ADR-033 Integration** - Context API becomes Aggregation Layer
  - **Day 10**: AggregationService + Data Storage HTTP Client (8h)
  - **Day 11**: HTTP API Endpoints + Integration Tests (8h)
  - **Day 12**: E2E Tests + Documentation (8h)
- ✅ **Full API Gateway Migration** (ADR-032 compliance)
  - **Decision**: ALL queries via Data Storage Service REST API (no direct PostgreSQL)
  - **Rationale**: Data Storage Service is authoritative owner of schema and data access
  - **Impact**: Replace SQL queries with HTTP client calls (~130 lines net change)
- ✅ **New Business Requirements**
  - **BR-INTEGRATION-008**: Expose Incident-Type Success Rate API
  - **BR-INTEGRATION-009**: Expose Playbook Success Rate API
  - **BR-INTEGRATION-010**: Expose Multi-Dimensional Success Rate API
- ✅ **Configurable Cache TTL**
  - **Default**: 5 minutes (same as current)
  - **Configurable**: Per-endpoint TTL in context-api config structure
  - **Rationale**: Aggregation data changes slowly, allow tuning for performance
- ✅ **Integration Test Infrastructure**
  - **Approach**: Start Data Storage Service in Podman container (consistent with existing tests)
  - **Dependencies**: PostgreSQL + Redis + Data Storage Service
  - **Validation**: Real service integration, no mocks

**Rationale**:
- **ADR-033 Approved**: Remediation Playbook Catalog requires success rate aggregation
- **Data Storage Ready**: ADR-033 endpoints implemented and tested (v5.3 complete)
- **API Gateway Pattern**: DD-ARCH-001 Alternative 2 approved, now implementing
- **User Decision**: Option A (Rewrite Days 10-12 now) + Option B (Full API Gateway migration)

**Impact**:
- Timeline Extended: +24 hours (Days 10-12 rewrite)
- Total Timeline: 12 days → 15 days (96h → 120h implementation)
- Quality Maintained: Phase 3 CRD Controller Level (100%)
- ADR-032 Compliance: 100% (no direct PostgreSQL access)
- ADR-033 Compliance: 100% (full aggregation support)

**Time Investment**: 24 hours (3 days) for ADR-033 implementation
...
### **v2.8.0** (2025-11-02) - API GATEWAY MIGRATION REFERENCE ADDED
...
- ✅ **Future Architectural Change section added** (~20 lines)
  - Documents planned migration from direct PostgreSQL access to Data Storage Service REST API
  ...
  - Timeline: 2-3 days after Data Storage Service Phase 1 complete
  - Impact: Query flow changes, caching unchanged, SQL → HTTP client
- ✅ **Implementation plan created**: [API-GATEWAY-MIGRATION.md](./API-GATEWAY-MIGRATION.md)
  ...
  - Code impact analysis (~130 lines net change)

**Rationale**:
- Architectural decision approved but not yet implemented
- Need to document in authoritative implementation plan to avoid missing future work
- Provides clear reference for Phase 2 implementation when Data Storage Service ready

**Impact**:
- Documentation Completeness: Future work clearly documented and tracked
- Traceability: Links from main plan → migration plan → DD-ARCH-001 decision
- Risk Mitigation: Prevents orphaned implementation plans in service subdirectories

**Time Investment**: 5 minutes (documentation only, no code changes)

**Related**:
- [DD-ARCH-001 Final Decision](../../../../architecture/decisions/DD-ARCH-001-FINAL-DECISION.md)
- [API-GATEWAY-MIGRATION.md](./API-GATEWAY-MIGRATION.md) - Context API Phase 2 plan
- [Data Storage Service API-GATEWAY-MIGRATION.md](../../data-storage/implementation/API-GATEWAY-MIGRATION.md) - Phase 1 dependency
...
### **DAY 10: AGGREGATION SERVICE + DATA STORAGE HTTP CLIENT** (8 hours)

**Objective**: Implement AggregationService with Data Storage Service REST API integration (ADR-033)
...
#### **RED Phase** (2 hours)
**Write failing unit tests for AggregationService**

**Files Created**:
- `test/unit/contextapi/aggregation_service_test.go` (re-enable from `.v1x`)
- `test/unit/contextapi/datastorage_client_test.go` (NEW)

**Test Coverage**:
1. **AggregationService Tests** (15 tests):
   - `GetSuccessRateByIncidentType()` - incident type aggregation (5 tests)
   - `GetSuccessRateByPlaybook()` - playbook aggregation (5 tests)
   - `GetSuccessRateMultiDimensional()` - multi-dimensional aggregation (5 tests)
2. **Data Storage HTTP Client Tests** (10 tests):
   - HTTP request construction (3 tests)
...
#### **GREEN Phase** (4 hours)
**Implement minimal AggregationService + Data Storage HTTP Client**
...
- `pkg/contextapi/datastorage/models.go` (Response models)
...
func (a *AggregationService) GetSuccessRateByIncidentType(
    ...
    // 1. Check cache
    cacheKey := fmt.Sprintf("success_rate:incident_type:%s:%s", params.IncidentType, params.TimeRange)
    if cached, found := a.cache.Get(cacheKey); found {
        return cached.(*IncidentTypeSuccessRate), nil
    }

    // 2. Call Data Storage Service
    result, err := a.dataStorageClient.GetSuccessRateByIncidentType(ctx, params)
    if err != nil {
        return nil, fmt.Errorf("failed to get success rate from Data Storage: %w", err)
    }

    // 3. Cache result (configurable TTL, default 5min)
    a.cache.Set(cacheKey, result, a.getCacheTTL("incident_type"))

    return result, nil
}

// Similar implementations for GetSuccessRateByPlaybook() and GetSuccessRateMultiDimensional()
```

```go
// pkg/contextapi/datastorage/client.go
type Client struct {
    baseURL    string
...
func (c *Client) GetSuccessRateByIncidentType(
    ...
    // Build URL: GET /api/v1/success-rate/incident-type
    u, _ := url.Parse(fmt.Sprintf("%s/api/v1/success-rate/incident-type", c.baseURL))
    q := u.Query()
    q.Set("incident_type", params.IncidentType)
    q.Set("time_range", params.TimeRange)
    q.Set("min_samples", fmt.Sprintf("%d", params.MinSamples))
    u.RawQuery = q.Encode()
    ...
    // Parse response
    var result IncidentTypeSuccessRate
...
**Enhance with production-ready patterns**

**Enhancements**:
1. **Configurable Cache TTL**:
   ```go
   type CacheTTLConfig struct {
       IncidentType      time.Duration // Default: 5min
       Playbook          time.Duration // Default: 5min
       MultiDimensional  time.Duration // Default: 5min
   }
   ```
2. **Structured Logging**: Add request/response logging with zap
3. **Error Handling**: RFC 7807 error parsing from Data Storage Service
4. **Retry Logic**: Exponential backoff for transient failures
5. **Circuit Breaker**: Prevent cascading failures if Data Storage is down

**Deliverables**:
- ✅ `pkg/contextapi/query/aggregation.go` (300 lines)
- ✅ `pkg/contextapi/datastorage/client.go` (400 lines)
- ✅ `pkg/contextapi/datastorage/models.go` (200 lines)
- ✅ `test/unit/contextapi/aggregation_service_test.go` (500 lines, 15 tests)
- ✅ `test/unit/contextapi/datastorage_client_test.go` (400 lines, 10 tests)
- ✅ All 25 unit tests passing
- ✅ Zero lint errors

**Confidence**: 90% (Data Storage endpoints exist and tested)

---

### **DAY 11: HTTP API ENDPOINTS + INTEGRATION TESTS** (8 hours)

**Objective**: Expose aggregation endpoints to AI/LLM Service and validate with real Data Storage Service
...
**Files Created**:
- `test/integration/contextapi/11_aggregation_api_test.go` (NEW)

**Test Coverage** (10 integration tests):
1. **Incident-Type Endpoint** (3 tests):
   - Valid request returns success rate data
   - Invalid incident_type returns 400 Bad Request
   - Data Storage Service unavailable returns cached data (if available)
2. **Playbook Endpoint** (3 tests):
   - Valid request returns playbook success rate
   - Missing playbook_id returns 400 Bad Request
   - Cache hit scenario
3. **Multi-Dimensional Endpoint** (4 tests):
   - All dimensions specified returns combined data
   - Partial dimensions returns filtered data
   - No dimensions returns 400 Bad Request
   - Data Storage Service timeout returns 503 Service Unavailable

**Infrastructure Setup**:
```go
// Start Data Storage Service in Podman container
BeforeSuite(func() {
    // Start PostgreSQL
...
**Expected**: ❌ All tests fail (no HTTP endpoints yet)

#### **GREEN Phase** (4 hours)
**Implement HTTP handlers + route registration**

**Files Created**:
- `pkg/contextapi/server/aggregation_handlers.go` (NEW)
...
func (s *Server) setupRoutes() {
    ...
    s.router.Get("/api/v1/aggregation/success-rate/multi-dimensional", s.HandleGetSuccessRateMultiDimensional)
}
```
...
1. **Prometheus Metrics**:
   ```go
   context_api_aggregation_requests_total{endpoint, status}
   context_api_aggregation_duration_seconds{endpoint}
   context_api_aggregation_cache_hits_total{endpoint}
   ```
2. **RFC 7807 Error Responses**: Consistent error format
3. **Request ID Propagation**: Pass request ID to Data Storage Service
4. **Structured Logging**: Log all aggregation requests with context

**Deliverables**:
- ✅ `pkg/contextapi/server/aggregation_handlers.go` (400 lines, 3 handlers)
- ✅ `test/integration/contextapi/11_aggregation_api_test.go` (600 lines, 10 tests)
...
### **DAY 12: E2E TESTS + DOCUMENTATION** (8 hours)

**Objective**: Validate AI/LLM → Context API → Data Storage flow + complete ADR-033 documentation
...
**Files Created**:
- `test/e2e/contextapi/01_ai_playbook_selection_test.go` (NEW)

**Test Scenarios** (5 E2E tests):
1. **AI Playbook Selection Flow**:
   - AI/LLM Service queries incident-type success rate
   - Context API aggregates from Data Storage Service
   - AI selects best playbook based on success rate
2. **Cache Effectiveness**:
   - First request: Cache miss → Data Storage call
   - Second request: Cache hit → No Data Storage call
   - Verify cache TTL expiration
3. **Data Storage Service Failure**:
   - Data Storage Service unavailable
   - Context API returns cached data (if available)
   - Context API returns 503 if no cache
4. **Multi-Dimensional Query**:
   - AI queries success rate by incident_type + playbook_id
...
**Files Updated**:
1. **Service README** (`docs/services/stateless/context-api/README.md`):
   - Add ADR-033 aggregation features
   - Update architecture diagram (Context API → Data Storage Service)
   - Add API reference for 3 new endpoints
2. **API Specification** (`docs/services/stateless/context-api/api-specification.md`):
   - Document aggregation endpoints (request/response)
   - Add query parameter validation rules
   - Add example requests/responses
3. **Integration Points** (`docs/services/stateless/context-api/integration-points.md`):
   - Add Data Storage Service dependency
   - Update dependency diagram
4. **Design Decision** (`docs/architecture/decisions/DD-CONTEXT-003-aggregation-layer.md`) (NEW):
   - Document aggregation layer architecture
   - Explain caching strategy (configurable TTL)
   - Justify API Gateway pattern (ADR-032 compliance)

**Deliverables**:
- ✅ `test/e2e/contextapi/01_ai_playbook_selection_test.go` (500 lines, 5 tests)
- ✅ All 5 E2E tests passing
- ✅ `docs/services/stateless/context-api/README.md` updated
- ✅ `docs/services/stateless/context-api/api-specification.md` updated
- ✅ `docs/services/stateless/context-api/integration-points.md` updated
