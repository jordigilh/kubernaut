...
var _ = Describe("Session Cancellation Infrastructure — #823", func() {
	...
	Describe("UT-KA-2020-001/002: SetPendingDecisionResult always overwrites on UserDriving, no-ops elsewhere", func() {
		It("UT-KA-2020-001: overwrites Result on a UserDriving session even when a prior result is already set", func() {
			store := session.NewStore(30 * time.Minute)
			id, err := store.Create()
			Expect(err).NotTo(HaveOccurred())
			Expect(store.Update(id, session.StatusRunning, nil, nil)).To(Succeed())
			...
			Expect(sess.Result.WorkflowID).To(Equal("wf-round-2"),
				"#2020: unlike SetResult's first-write-wins guard (#1425, a different concern), "+
					"SetPendingDecisionResult always overwrites while UserDriving -- discover_workflows may legitimately re-run")
		})

		It("UT-KA-2020-002: is a no-op on a Completed session (does not resurrect a terminal result)", func() {
			store := session.NewStore(30 * time.Minute)
			id, err := store.Create()
			Expect(err).NotTo(HaveOccurred())
			Expect(store.Update(id, session.StatusRunning, nil, nil)).To(Succeed())
			...
			Expect(sess.Result.WorkflowID).To(Equal("wf-selected"),
				"#2020: must not overwrite an already-completed session's confirmed result")
		})

		It("UT-KA-2020-003: composition with CompleteUserDriving(id, nil) preserves the pending preview (the actual #2020 bug-fix mechanism)", func() {
			store := session.NewStore(30 * time.Minute)
			id, err := store.Create()
			Expect(err).NotTo(HaveOccurred())
			Expect(store.Update(id, session.StatusRunning, nil, nil)).To(Succeed())
			...
			// Simulates cmd/kubernautagent/routes.go's inactivity-timeout/disconnect
			...
			Expect(sess.Status).To(Equal(session.StatusCompleted))
			Expect(sess.Result).NotTo(BeNil(),
				"#2020: inactivity-timeout completion with a nil result must preserve the previously discovered workflow, not discard it")
