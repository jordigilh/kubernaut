...
// Issue #2029 Part B / FedRAMP SI-4: proves end-to-end that AA adopts a KA
// session AF has correlated onto this RR's InvestigationSession (e.g. after a
// reconnect/takeover) instead of continuing to poll — and potentially finalize
// as expired/terminal — its own now-stale session. Also proves the widened
// ISEventPredicate (see internal/controller/aianalysis/aianalysis_controller.go)
// wakes the reconciler promptly on a KACorrelationID-only change, rather than
// waiting for the next scheduled poll.
var _ = Describe("BR-INTERACTIVE-010: #2029 Part B session correlation adoption", Label("integration", "interactive"), func() {
	const (
	...
	Context("AA adopts a live KA session correlated onto IS after takeover (SI-4)", Serial, func() {
		...
		It("IT-AA-2029-011: adopts a newly-correlated KA session instead of finalizing the stale one, waking promptly via the widened predicate", func() {
			rrName := helpers.UniqueTestName("rr-2029-adopt")
			isName := helpers.UniqueTestName("is-2029-adopt")
			aaName := helpers.UniqueTestName("aa-2029-adopt")
			...
			// #2029 Part B / SI-4 predicate-wake-up proof: the handler's poll
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(analysis.Status.KASession.ID).To(Equal(newSessionID),
					"#2029 Part B / SI-4: AA must promptly adopt the correlated session instead of continuing to track the stale one")
