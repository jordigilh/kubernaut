...
var _ = Describe("present_decision", func() {
	...
	It("UT-AF-2074-001: SI-10 RCAData's ADK-generated JSON schema does not require tool_calls_count/llm_turns", func() {
		// #2074 (v1.6 clone of #2073): prompt.txt never instructs the LLM to
		// compute tool_calls_count or llm_turns, but the ADK-generated schema
		// (github.com/google/jsonschema-go) marks every non-omitempty field as
		// required -- causing every kubernaut_present_decision call to fail
		// schema validation before the handler ever runs.
