#!/bin/bash
# Complete E2E Testing Environment Cleanup Script
...
log_info "Starting Kubernaut E2E Testing Environment Cleanup"
...
# Step 1: Cancel Auto-cleanup
cancel_auto_cleanup() {
    progress "Cancelling Auto-cleanup"

    if [[ -f "/tmp/kubernaut-e2e-cleanup.pid" ]]; then
        CLEANUP_PID=$(cat /tmp/kubernaut-e2e-cleanup.pid)
        if kill -0 "$CLEANUP_PID" 2>/dev/null; then
            kill "$CLEANUP_PID"
            log_success "Auto-cleanup process cancelled (PID: $CLEANUP_PID)"
        else
            log_info "Auto-cleanup process not running"
        fi
        rm -f /tmp/kubernaut-e2e-cleanup.pid
    else
        log_info "No auto-cleanup scheduled"
    fi
}

# Step 2: Cleanup Test Applications
...
# Step 3: Cleanup Kubernaut Application
...
# Step 5: Cleanup Monitoring Stack
cleanup_monitoring() {
...
# Step 6: Cleanup Storage (if not preserving cluster)
cleanup_storage() {
    if [[ "$PRESERVE_CLUSTER" == "true" ]]; then
...
# Step 7: Cleanup Network Resources and RBAC
...
# Step 8: Remove OpenShift Cluster
cleanup_cluster() {
    if [[ "$PRESERVE_CLUSTER" == "true" ]]; then
...
cleanup_temp_files() {
    ...
    # Remove generated scripts if they exist
    GENERATED_SCRIPTS=(
        "setup-litmus-chaos.sh"
...
print_cleanup_summary() {
    log_header "E2E Environment Cleanup Summary"
    ...
    echo -e "  ✓ Temporary files and auto-cleanup"
    ...
    if [[ "$PRESERVE_CLUSTER" == "true" ]]; then
        ...
        echo -e "\n${BLUE}To access preserved cluster:${NC}"
        echo -e "  export KUBECONFIG=$HOME/.kcli/clusters/${CLUSTER_NAME}/auth/kubeconfig"
        echo -e "  oc get nodes"
        echo -e "\n${BLUE}To remove preserved cluster later:${NC}"
        echo -e "  kcli delete cluster ${CLUSTER_NAME}"
    fi

    echo -e "\n${BLUE}To redeploy environment:${NC}"
    echo -e "  ./setup-complete-e2e-environment.sh"

    echo -e "${GREEN}========================================${NC}\n"

    log_success "Kubernaut E2E Testing Environment Cleanup Completed!"
}

# Main execution function
main() {
    log_info "Starting comprehensive cleanup of Kubernaut E2E testing environment"
    echo ""
