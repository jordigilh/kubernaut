...
var _ = Describe("#2013: discover_workflows auto-closes session on no_matching_workflows (main port of #2003)", func() {
	...
	Describe("UT-KA-2020-004: a found workflow is preserved as a pending decision (in case of inactivity timeout)", func() {
		It("should call PersistPendingDecisionResult with decision_expired marked and the discovered fields intact", func() {
			autoMgr := &mockAutoMgrDW2013{
				rcaResult: &katypes.InvestigationResult{RCASummary: "OOM crash"},
			}
			completer := &orderedHTTPCompleter{foundID: "http-sess-2020-004", found: true}
			sessionMgr := &mockSessionManager{
				isActive: true,
				getDriverResult: &mcpinternal.InteractiveSession{
					SessionID:     "sess-2020-004",
					CorrelationID: "rr-2020-004",
					ActingUser:    mcpinternal.UserInfo{Username: "alice"},
				},
			}
			runner := &mockInvestigatorRunner{
			...
			tool := mcptools.NewInvestigateTool(sessionMgr, runner, recon, autoMgr,
				mcptools.WithHTTPCompleter(completer),
				mcptools.WithWorkflowCatalog(&mockWorkflowCatalog{
					workflow: &mcptools.CatalogWorkflow{WorkflowID: "wf-recommended-2020", WorkflowName: "Mock Workflow"},
				}),
			)
			...
			Expect(calls).To(Equal(1), "#2020: a found workflow must be persisted exactly once as a pending decision")
			Expect(id).To(Equal("http-sess-2020-004"))
			Expect(result).NotTo(BeNil())
			Expect(result.WorkflowID).To(Equal("wf-recommended-2020"),
				"#2020: the discovered workflow identity must survive into the pending-decision preview")
			...
			Expect(result.HumanReviewReason).To(Equal(katypes.HumanReviewReasonDecisionExpired),
				"#2020: the preview must be marked decision_expired so a later inactivity-timeout completion "+
					"is distinguishable from a genuine no_matching_workflows outcome")
		})
	})

	Describe("UT-KA-2020-005: no_matching_workflows does not also persist a pending decision (regression guard)", func() {
		It("should not call PersistPendingDecisionResult when the outcome is the #2013 auto-close path", func() {
			order := &orderRecorder{}
			completer := &orderedHTTPCompleter{foundID: "http-sess-2020-005", found: true, order: order}
			autoMgr := &mockAutoMgrDW2013{
				rcaResult: &katypes.InvestigationResult{RCASummary: "OOM crash"},
				order:     order,
			}
			sessionMgr := &mockSessionManager{
				isActive: true,
				getDriverResult: &mcpinternal.InteractiveSession{
					SessionID:     "sess-2020-005",
					CorrelationID: "rr-2020-005",
					ActingUser:    mcpinternal.UserInfo{Username: "alice"},
				},
			}
			runner := &mockInvestigatorRunner{
			...
			Eventually(func(g Gomega) {
				id, result := completer.getCompleted()
				g.Expect(id).To(Equal("http-sess-2020-005"))
				g.Expect(result).NotTo(BeNil())
			}).WithTimeout(2 * time.Second).WithPolling(50 * time.Millisecond).Should(Succeed())

			_, _, calls := completer.getPending()
			Expect(calls).To(BeZero(),
				"#2020: the two branches are mutually exclusive -- no_matching_workflows already auto-closes "+
