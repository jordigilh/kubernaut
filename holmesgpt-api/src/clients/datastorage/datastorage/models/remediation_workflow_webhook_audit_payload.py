...
class RemediationWorkflowWebhookAuditPayload(BaseModel):
    """
    ...
    def event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('remediationworkflow.admitted.create', 'remediationworkflow.admitted.delete', 'remediationworkflow.admitted.denied'):
            raise ValueError("must be one of enum values ('remediationworkflow.admitted.create', 'remediationworkflow.admitted.delete', 'remediationworkflow.admitted.denied')")
        return value

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('create', 'delete', 'denied'):
            raise ValueError("must be one of enum values ('create', 'delete', 'denied')")
        return value
