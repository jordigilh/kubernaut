...
// Step 1 discovery response shape. The workflow count is passed in rather
// than computed here because it depends on the ActionType's sibling
// RemediationWorkflow CRDs in the cache, which this pure converter has no
// access to.
...
// ========================================
// CRD -> MODELS CONVERTERS (Issue #1677 Phase 2b)
// ========================================
// Authority: DD-WORKFLOW-019. Ported verbatim from
// pkg/datastorage/repository/workflow/cache_convert.go (Issue #1661
// Change 6). Lets the cache-backed Step 1/2 discovery path (ListActions,
// ListWorkflowsByActionType) reuse the existing filter/scoring predicates
// (cache_filter.go) and the existing models.RemediationWorkflow response
// shape without a Postgres round-trip.
// ========================================
...
// shape and the Step 3 GetByID/GetWorkflowWithContextFilters full-object
// response -- every field the get_workflow MCP tool documents as part of
// its contract (e.g. spec.parameters[] for LLM parameter validation) must
// be populated here.
//
...
// never captured this Postgres-catalog bookkeeping, and this is not a
// regression -- the DS-side inline/CRD registration path never set them
// either.
...
// has always used. The OpenAPI schema declares `parameters` as `type: object`
// (not `array`) precisely because of this envelope, and the ogen-generated
// client decodes it as `map[string]jx.Raw` -- marshaling the bare parameter
// array here (without the envelope) breaks ogen's decoder for every caller.
