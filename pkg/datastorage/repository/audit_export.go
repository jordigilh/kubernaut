...
func calculateEventHashForVerification(previousHash string, event *AuditEvent) (string, error) {
	// CRITICAL: Clear fields that are set AFTER hash calculation during INSERT
	// 1. EventHash/PreviousEventHash: Set after hash calc (lines 336-337 of audit_events_repository.go)
	// 2. EventDate: Generated by DB trigger from event_timestamp (NOT set during INSERT)
	// We must replicate INSERT-time state to get matching hashes
	eventCopy := *event
	eventCopy.EventHash = ""
	eventCopy.PreviousEventHash = ""
	eventCopy.EventDate = DateOnly{} // Clear DB-generated date field
