...
3. **Fast path — automation proposal links**: `bestpractices.dev` supports pre-filling form fields
  ...
  5. [Vulnerabilities + Static Analysis, part 1](https://www.bestpractices.dev/en/projects/13485/passing/edit?vulnerabilities_fixed_60_days_status=Met&vulnerabilities_fixed_60_days_justification=3+tracked+OSV+entries+%28.govulncheck-ignore.yaml%29+are+upstream+Go-vulndb+false+positives+%28fix+filed+at+golang%2Fvulndb%235797%29+or+a+low-severity+%28CVSS+3.7%29+Tekton+issue+with+no+upstream+fix+yet%3B+each+has+a+dated+2026-10-01+re-review+commitment.&vulnerabilities_critical_fixed_status=Met&vulnerabilities_critical_fixed_justification=No+unaddressed+critical-severity+vulnerabilities+are+currently+open.+The+last+time+a+critical%2Fhigh-severity+issue+was+addressed%2C+it+was+fixed+within+approximately+1+week+of+being+identified+--+a+reasonable+turnaround+for+a+solo-maintainer+project.+govulncheck+is+also+gated+in+CI+on+every+push%2FPR+%28scripts%2Fci%2Fgovulncheck-gated.sh%29+to+catch+new+ones+as+early+as+possible+going+forward.&no_leaked_credentials_status=Met&no_leaked_credentials_justification=gosec+%28hardcoded-credential+detection%29+runs+in+CI+via+golangci-lint.+A+dedicated+gitleaks+secret+scan+also+runs+on+every+push%2C+PR%2C+and+weekly+schedule+%28.github%2Fworkflows%2Fgitleaks.yml%29%2C+covering+full+git+history.+A+one-time+manual+full-history+scan+%288%2C466+commits+across+all+branches%2Ftags%29+found+297+matches+before+this+automation+was+added%3B+every+one+was+triaged+and+confirmed+to+be+a+test%2Ffuzz+fixture%2C+canonical+placeholder%2C+or+regex+false-positive+--+zero+real+credentials+--+and+is+now+tracked+in+.gitleaks.toml+so+only+genuinely+new+findings+fail+CI+going+forward.&static_analysis_status=Met&static_analysis_justification=golangci-lint+%2815%2B+linters%29+%2B+CodeQL.+https%3A%2F%2Fgithub.com%2Fjordigilh%2Fkubernaut%2Fblob%2Fmain%2F.github%2Fworkflows%2Fcodeql.yml&static_analysis_common_vulnerabilities_status=Met&static_analysis_common_vulnerabilities_justification=CodeQL%27s+default+Go+query+suite+targets+known+vulnerability+classes+%28injection%2C+path+traversal%2C+etc.%29%3B+gosec+covers+Go-specific+issues.&static_analysis_fixed_status=Met&static_analysis_fixed_justification=CI-gated%3B+AGENTS.md+GA+Readiness+Audit+mandates+zero+lint%2FSAST+findings+before+merge.&static_analysis_often_status=Met&static_analysis_often_justification=CodeQL+and+golangci-lint+run+on+every+push%2FPR%2C+plus+CodeQL%27s+weekly+scheduled+scan.)
...
- **[vulnerabilities_critical_fixed]** Met — no unaddressed critical-severity vulnerabilities are
currently open. The last time a critical/high-severity issue was addressed, it was fixed within
approximately 1 week of being identified — a reasonable turnaround for a solo-maintainer project.
`govulncheck` is also gated in CI on every push/PR (`scripts/ci/govulncheck-gated.sh`) to catch
new ones as early as possible going forward.
...
`golangci-lint`. A dedicated `gitleaks` secret scan also runs on every push, PR, and weekly
schedule (`.github/workflows/gitleaks.yml`), covering full git history. A one-time manual
full-history scan (8,466 commits across all branches/tags, run before this automation was added)
found 297 matches; every one was individually triaged and confirmed to be a test/fuzz fixture,
canonical placeholder value, or regex false-positive — zero real, currently-valid credentials.
These are now tracked in `.gitleaks.toml` (path-scoped to the exact files involved, not blanket
directory wildcards) so only genuinely new findings fail CI going forward.
