...
class ActionTypeWebhookAuditPayload(BaseModel):
    """
    AW audit payload for ActionType CRD admission events
    """ # noqa: E501
    event_type: StrictStr
    action_type_name: StrictStr = Field(description="PascalCase name from spec.name")
    crd_name: StrictStr = Field(description="K8s metadata.name")
    crd_namespace: StrictStr = Field(description="K8s namespace")
    action: StrictStr
    previously_existed: Optional[StrictBool] = None
    catalog_status: Optional[StrictStr] = None
    denial_reason: Optional[StrictStr] = None
    denial_operation: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["event_type", "action_type_name", "crd_name", "crd_namespace", "action", "previously_existed", "catalog_status", "denial_reason", "denial_operation"]

    @field_validator('event_type')
    def event_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('actiontype.admitted.create', 'actiontype.admitted.update', 'actiontype.admitted.delete', 'actiontype.denied.create', 'actiontype.denied.update', 'actiontype.denied.delete'):
            raise ValueError("must be one of enum values ('actiontype.admitted.create', 'actiontype.admitted.update', 'actiontype.admitted.delete', 'actiontype.denied.create', 'actiontype.denied.update', 'actiontype.denied.delete')")
        return value

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('create', 'update', 'delete', 'denied'):
            raise ValueError("must be one of enum values ('create', 'update', 'delete', 'denied')")
        return value
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            ...
            "action_type_name": obj.get("action_type_name"),
            "crd_name": obj.get("crd_name"),
            "crd_namespace": obj.get("crd_namespace"),
            "action": obj.get("action"),
            "previously_existed": obj.get("previously_existed"),
            "catalog_status": obj.get("catalog_status"),
            "denial_reason": obj.get("denial_reason"),
            "denial_operation": obj.get("denial_operation")
        })
        return _obj
