...
var _ = Describe("kubectl_get", func() {
	...
	// resolveGVR must use the discovery-backed RESTMapper's real plural
	...
	It("UT-AF-1772-001: resolves the discovery-backed plural for an irregular Kind (AIAnalysis)", func() {
		...
		// this test pass against the *buggy* resolveGVR too (both sides
		...
		result, err := tools.HandleKubectlGet(context.Background(), client, mapper, tools.KubectlGetArgs{
			Kind:      "AIAnalysis",
			Name:      "rr-123-analysis",
			Namespace: "prod",
		})
