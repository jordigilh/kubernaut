...
func generateAmbientCATestCert(t *testing.T) string {
	...
	template := &x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "effectivenessmonitor-ambient-ca-2276-test"},
...
// IT-EM-2276-001: a tlsCaFile configured via YAML is wired through
// bootstrapAmbientCATrust -- the exact production function run() calls
// immediately after loadConfigAndNamespace, before initCoreDependencies'
// DataStorage-backed audit store client -- into ambient env vars
// (Issue #2276, SC-8).
func TestBootstrapAmbientCATrust_EffectivenessMonitor_InjectsFromConfig(t *testing.T) {
	resetAmbientCAEnv(t)
...
// IT-EM-2276-002: an unset tlsCaFile leaves ambient env vars untouched
// (fail-open parity).
func TestBootstrapAmbientCATrust_EffectivenessMonitor_NoopWhenUnset(t *testing.T) {
	resetAmbientCAEnv(t)
	cfg := &config.Config{}
