...
var _ = Describe("PostgreSQL Vector Database Integration", Ordered, func() {
	...
	BeforeAll(func() {
		...
		stateManager = shared.NewIsolatedTestSuiteV2("PostgreSQL Vector Integration").
			WithLogger(logger).
	...
	Context("Vector Similarity Search", func() {
		...
		It("should find similar patterns using vector distance", func() {
			...
			similarPatterns, err := vectorDB.FindSimilarPatterns(ctx, searchPattern, 5, 0.8)
			Expect(err).ToNot(HaveOccurred())
		...
		It("should perform semantic search", func() {
			...
			for _, pattern := range patterns {
				if pattern.AlertName == "HighMemoryUsage" || pattern.ActionType == "scale_deployment" {
