...
| `ingress.tls.secretName` | string | Pre-created Secret with a TLS cert for `host`; omit for HTTP-only or an ingress controller that provides its own default cert | `""` | No |
| `nodeSelector` | map[string]object | Kubernetes node selector | `` | No |
| `oauth2Proxy.backendLogoutURL` | string | Optional provider-discovered OIDC end_session_endpoint for OAuth2 Proxy logout. Use the literal {id_token} placeholder where the provider requires an ID token hint. HTTPS is required for production; HTTP is intended only for explicitly configured local/dev providers. | `""` | No |
