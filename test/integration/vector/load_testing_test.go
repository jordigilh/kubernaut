...
var _ = Describe("Load Testing and Scalability", Ordered, func() {
	...
	BeforeAll(func() {
		...
		stateManager = shared.NewIsolatedTestSuiteV2("Load Testing").
			WithLogger(logger).
	...
	Context("High-Volume Pattern Storage", func() {
		...
		It("should handle memory usage efficiently under load", func() {
			By("monitoring initial memory usage")
			var initialMemStats runtime.MemStats
			runtime.GC()
			runtime.ReadMemStats(&initialMemStats)

			By("performing memory-intensive operations")
			const memoryTestPatterns = 2000
			patterns := generateLoadTestPatterns(embeddingService, ctx, memoryTestPatterns, "memory")

			// Store patterns and force garbage collection periodically
			for i, pattern := range patterns {
				err := vectorDB.StoreActionPattern(ctx, pattern)
				Expect(err).ToNot(HaveOccurred())

				// Force GC every 200 patterns to test memory management
				if (i+1)%200 == 0 {
					runtime.GC()
				}
			}

			By("measuring final memory usage")
			var finalMemStats runtime.MemStats
			runtime.GC()
			runtime.ReadMemStats(&finalMemStats)

			By("validating memory efficiency")
			memoryIncrease := finalMemStats.Alloc - initialMemStats.Alloc
			memoryPerPattern := float64(memoryIncrease) / float64(memoryTestPatterns)

			// Memory increase should be reasonable (less than 1KB per pattern)
			Expect(memoryPerPattern).To(BeNumerically("<", 1024))

			By("recording memory metrics")
			loadTestResults.RecordMemoryUsage(initialMemStats.Alloc, finalMemStats.Alloc, memoryTestPatterns)

			logger.WithFields(logrus.Fields{
				"initial_memory_mb":  float64(initialMemStats.Alloc) / 1024 / 1024,
				"final_memory_mb":    float64(finalMemStats.Alloc) / 1024 / 1024,
				"memory_increase_mb": float64(memoryIncrease) / 1024 / 1024,
				"memory_per_pattern": memoryPerPattern,
